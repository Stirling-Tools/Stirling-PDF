...
Before adding or styling an icon, read @frontend/src/editor/core/icons/README.md — `<Icon name="…" />` comes from `@app/ui/Icon`, icon sources live as `.svg` files in `core/icons/svg/`, and inline `<svg>` in TS/TSX is linted out.
