...
const meta: Meta<typeof Button> = {
  ...
  argTypes: {
    variant: { control: 'inline-radio', options: ['gradient', 'outline', 'ghost'] },
    accent: { control: 'inline-radio', options: ['blue', 'purple', 'green', 'amber', 'red'] },
...
export const Disabled: Story = { args: { variant: 'gradient', disabled: true } };
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
      {(['gradient', 'outline'] as const).flatMap((variant) =>
        (['blue', 'purple', 'green', 'amber', 'red'] as const).map((accent) => (
          <Button key={`${variant}-${accent}`} variant={variant} accent={accent}>
            {variant} · {accent}
          </Button>
        )),
      )}
