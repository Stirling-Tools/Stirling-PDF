...
# Root LICENSE: processor is now a single proprietary product. groupX just
# rewrote the two portal entries to frontend/src/processor/proprietary and
# .../saas; collapse them into one entry covering the whole
# frontend/src/processor/ tree (restructure-frontend.sh makes the matching tree
# change: a single LICENSE at src/processor/, none in the children). Keyed on the
# path tokens rather than the surrounding prose, and a no-op once collapsed.
if [ -f LICENSE ]; then
  perl -0777 -pi -e '
    s{[^\n]*"frontend/src/processor/saas/"[^\n]*\n[^\n]*"frontend/src/processor/saas/LICENSE"[^\n]*\n}{}g;
    s{frontend/src/processor/proprietary/}{frontend/src/processor/}g;
  ' LICENSE
  log "collapsed root LICENSE processor entries into a single frontend/src/processor/ entry"
fi
...
perl -pi -e '
  ...
  s{editor/\*\*/\*\.css}{src/**/*.css}g;   # lint:css glob: source css now lives under src/
...
# Group Z: CI workflow `run:` steps and the frontend Dockerfile also execute with
# a frontend/ working dir, so they carry bare 'editor/...' relative paths
# (report-flaky-tests.mts, generate-icons.js, editor/dist, editor/src/assets)
# that groupX's 'frontend/editor' rules never touch. Apply the same bare-relative
# path rewrites here. These files hold no 'text-editor'/'pdf-editor' API paths, so
# the unanchored 'editor/' is safe.
BARE_EDITOR_FILES=()
while IFS= read -r f; do BARE_EDITOR_FILES+=("$f"); done < <(
  git ls-files .github/workflows docker 2>/dev/null | grep -E '\.ya?ml$|Dockerfile'
)
if [ "${#BARE_EDITOR_FILES[@]}" -gt 0 ]; then
  perl -pi -e '
    s{editor/src/portal-saas}{src/processor/saas}g;
    s{editor/src/portal}{src/processor/proprietary}g;
    s{editor/src/(core|proprietary|desktop|saas|cloud|prototypes)}{src/editor/$1}g;
    ...
    s{editor/\.env}{.env}g;
  ' "${BARE_EDITOR_FILES[@]}"
  log "rewrote bare editor/ paths in ${#BARE_EDITOR_FILES[@]} workflow/docker files"
fi
...
echo "== bare 'editor/{scripts,src,dist,...}' left in workflows/docker? (expect 0) =="
git grep -nE 'editor/(scripts|src|public|dist|src-tauri)' -- .github/workflows docker 2>/dev/null | grep -v 'src/editor' || echo "  none"
echo "== LICENSE: leftover processor child entries? (expect none, one frontend/src/processor/ entry) =="
grep -nE 'frontend/src/processor/(proprietary|saas)/' LICENSE || echo "  none"

echo ""
echo "== Fix rules =="
# These edits touch Python AND Java files, so run both repo formatters:
#   - pre-commit:fix   -> Python (ruff) etc.
#   - backend:format   -> Java (spotless / google-java-format). A path-string edit can
#     shorten a line and shift the optimal wrapping, so the edited Java must be
#     re-formatted or backend:format:check (spotlessCheck) fails in CI.
# Both guarded + non-fatal.
if command -v task >/dev/null 2>&1; then
  ...
  if task backend:format >/dev/null 2>&1; then
    echo "  ran task backend:format (spotless)"
  else
    echo "  WARNING: 'task backend:format' failed - run it manually before committing"
  fi
else
  echo "  NOTE: 'task' not on PATH - run 'task pre-commit:fix' and 'task backend:format' before committing"
fi
