...
gmv() {
  ...
  # `git mv src dst` moves src INTO dst when dst already exists (a directory),
  # silently nesting - e.g. a leftover frontend/public turns the public promotion
  # into frontend/public/public. Refuse rather than nest; the tree must be in the
  # clean old layout (see header: reset/clean frontend/ before running).
  [ ! -e "$dst" ] || die "destination '$dst' already exists - refusing to move '$src' into it (would nest). Reset the frontend tree to the clean old layout first (see header)."
  mkdir -p "$(dirname "$dst")"
  git mv "$src" "$dst"
}
...
# The file is renamed to the processor domain now; the @portal alias TOKEN it defines
# is left as-is (rename-frontend-aliases.sh renames the token later).
gmv "$ED/tsconfig.portal.vite.json" "$FE/tsconfig.processor.vite.json"

# frontend/vitest.config.ts already occupies the new vite root: a thin re-export
# of .storybook/vitest.config.ts (added after this script was written) whose only
# job was letting a bare `vitest` / the Storybook Testing UI discover the browser
# project from the conventional root. Post-restructure that slot belongs to the
# promoted editor config - Phase 3 below and the committed `test:editor`
# (`vitest run --root .`) both assume it - and the storybook suite is still run
# explicitly via `--config .storybook/vitest.config.ts`, so the re-export is
# obsolete. Drop it (guarded, in case a prior partial run already did) to clear
# the way for the editor vitest.config.ts move in the loop below.
if git ls-files --error-unmatch "$FE/vitest.config.ts" >/dev/null 2>&1; then
  git rm --quiet "$FE/vitest.config.ts"
  log "removed obsolete root vitest.config.ts (storybook re-export; slot now holds the editor config)"
fi
...
# processor is a single proprietary product: collapse the two identical LICENSE
# files carried over from src/portal and src/portal-saas into one at the
# processor root, with none in the children. The root LICENSE enumerates
# frontend/src/processor/ as a whole - restructure-frontend-external-refs.sh
# makes the matching change there.
gmv "$FE/src/processor/proprietary/LICENSE" "$FE/src/processor/LICENSE"
if git ls-files --error-unmatch "$FE/src/processor/saas/LICENSE" >/dev/null 2>&1; then
  # -f: the enclosing dir was just `git mv`d in, so this file is staged; plain
  # `git rm` refuses to remove a file that has changes staged in the index.
  git rm -f --quiet "$FE/src/processor/saas/LICENSE"
fi
log "consolidated processor LICENSE -> src/processor/LICENSE (none in children)"

for f in index.tsx global.d.ts output.css; do
...
# Import alias TOKENS (@app, @portal) are intentionally NOT renamed here - see the
# header. Files/folders/labels ARE renamed to the processor/editor naming; the aliases
# just keep their names and get re-pointed at the new folders (3b), so every
# `@app/*` / `@portal/*` importer resolves unchanged. rename-frontend-aliases.sh renames
# the @app->@editor / @portal->@processor tokens later.
...
perl -pi -e '
  ...
  s{tsconfig\.portal\.vite\.json}{tsconfig.processor.vite.json}g;   # file renamed in Phase 1
' \
...
# vitest project label -> processor (a rename; the @portal alias token stays @portal).
...
# vite.config also walks UP to the repo root and to the hoisted node_modules from
# the old editor/ root; promoted to frontend/ (one level up), each such walk needs
# one fewer '..': the viteStaticCopy backend-font src and the per-mode cacheDir.
perl -pi -e '
  s{\.\./\.\./app/}{../app/}g;               # viteStaticCopy backend-font src
  s{root \(editor/\)}{root (frontend/)}g;    # stale cacheDir comment
' "$FE/vite.config.ts"
perl -0777 -pi -e 's{import\.meta\.dirname,\s*"\.\.",(\s*)"node_modules"}{import.meta.dirname,$1"node_modules"}g;' "$FE/vite.config.ts"
...
# .storybook/vitest.config.ts (the browser-test config; stays put, run via
# --config) still names the old layout. Its bare 'editor/src/**' stories glob is
# rewritten by the Phase 3j mop-up, but its prose reference to the editor
# unit-test config - promoted from editor/vitest.config.ts up to the frontend/
# root by Phase 1 - is not, so correct that here.
perl -pi -e 's{\beditor/vitest\.config\.ts\b}{vitest.config.ts}g;' "$FE/.storybook/vitest.config.ts"
log "fixed .storybook/vitest.config.ts editor-config reference"
...
' "${SRC_FILES[@]}"
# The live specs' fixture() helper resolves test-fixtures via a multi-line
# path.resolve(process.cwd(), ... "src", "core", "tests", "test-fixtures" ...),
# not the single-line join above. cwd is frontend/ now, so insert the "editor"
# layer segment; anchored on "test-fixtures" so only the fixture builders match.
perl -0777 -pi -e 's{"src",(\s+)"core",(\s+)"tests",(\s+)"test-fixtures"}{"src",$1"editor",$1"core",$2"tests",$3"test-fixtures"}g;' "${SRC_FILES[@]}"
# ogMetadata.test.ts reads the shipped index.html via new URL("../../../index.html").
# index.html was promoted from the editor/ root up to frontend/ - one level farther
# from the test than before - so the walk gains one '..'.
perl -pi -e 's{new URL\("\.\./\.\./\.\./index\.html"}{new URL("../../../../index.html"}g;' "${SRC_FILES[@]}"
log "fixed escaping relative paths + hardcoded root-path literals (assets/scripts/public)"

# --- 3g. Cross-domain relative imports/@imports: processor code reaching an editor ---
# layer (saas/portal/, proprietary/billing/, ...). processor and the editor layers are
# no longer siblings, so each such path needs +1 '../' and an inserted 'editor/'. In the
# old-layout input, portal/portal-saas code only ever reached editor layers this way
# (they were siblings under editor/src/) and no portal subdir is named after a layer, so
# matching any editor-layer segment is safe. Includes .css @imports (e.g. billing.css).
PROC_FILES=()
while IFS= read -r f; do PROC_FILES+=("$f"); done < <(
  git ls-files "$FE/src/processor" | grep -E '\.(ts|tsx|js|jsx|mjs|mts|cjs|css)$'
)
if [ "${#PROC_FILES[@]}" -gt 0 ]; then
  perl -pi -e 's{((?:\.\./)+)(core|proprietary|desktop|saas|cloud|prototypes)/}{../${1}editor/${2}/}g;' "${PROC_FILES[@]}"
  # Those bridge imports carry an `oxlint-disable-next-line no-restricted-imports`
  # on the line above. Lengthening the path pushes the line over the print width,
  # so Phase 5's formatter wraps `await import(...)` onto its own line - away from
  # the disable comment, which then suppresses the wrong line and the ban fires.
  # Move the comment INSIDE the parens, directly above the import, so it stays
  # attached however the formatter wraps; Phase 5 then canonicalises the block.
  perl -0777 -pi -e 's{^([ \t]*)// oxlint-disable-next-line no-restricted-imports -- resolve the real SaaS module past the mocked \@app alias\n[ \t]*(\w+): \(await import\((.+?)\)\)}{$1$2: (\n$1  // oxlint-disable-next-line no-restricted-imports -- resolve the real SaaS module past the mocked \@app alias\n$1  await import($3)\n$1)}gm;' "${PROC_FILES[@]}"
  log "fixed cross-domain processor->editor imports (incl. css @imports)"
fi

# --- 3h. oxlint.config.ts (stays at frontend/): path globs ---
# Order matters: portal/layer-specific paths before the broad 'editor/src/**'.
# Alias mentions (@app/@portal) are left for rename-frontend-aliases.sh.
perl -pi -e '
  s{editor/src/portal-saas}{src/processor/saas}g;
  s{editor/src/portal}{src/processor/proprietary}g;
  s{editor/src/(core|proprietary|desktop|saas|cloud|prototypes)}{src/editor/$1}g;
...
' "$FE/oxlint.config.ts"
log "rewrote oxlint.config.ts path globs"
...
if [ "${#SCRIPT_DOC_FILES[@]}" -gt 0 ]; then
  perl -pi -e '
    ...
    # Bare layer segment joined onto a src-root variable (icon-lint / usedIcons do
    # path.join(SRC|srcDir, "core/icons")); no src/ prefix, so the rules above miss
    # it. core/ is now under editor/.
    s{(path\.join\([A-Za-z_]\w*, ")core/}{${1}editor/core/}g;
  ' "${SCRIPT_DOC_FILES[@]}"
fi
...
# theme-lint.mjs allowlists paths as regex literals with backslash-escaped
# slashes (editor\/src\/...); the plain-path passes above only match unescaped
# 'editor/src/', so rewrite the escaped layer paths here.
perl -pi -e 's{editor\\/src\\/(core|proprietary|desktop|saas|cloud|prototypes)}{src\\/editor\\/$1}g;' "$FE/scripts/lint/theme-lint.mjs"
...
if [ "${#MOP_FILES[@]}" -gt 0 ]; then
  perl -pi -e '
    ...
    s{\beditor/src\b}{src}g;
    s{\beditor/(scripts|public|dist|src-tauri|playwright-report|test-results)\b}{$1}g;
    s{\beditor/\.env}{.env}g;
  ' "${MOP_FILES[@]}"
fi
...
# ===========================================================================
# Structural invariant: the folders moved and no old-vite-root path refs remain.
# @app/@portal aliases are INTENTIONALLY still present here - rename-frontend-aliases.sh
# does that rename in its own pass, so they are not checked for.
residual=$(git grep -lE 'frontend/editor|editor/src' -- "$FE" 2>/dev/null || true)
if [ -n "$residual" ]; then
  log "WARNING: residual old-layout path references (frontend/editor or editor/src):"
  printf '%s\n' "$residual" | sed 's/^/    /'
else
  log "no residual frontend/editor or editor/src path references remain"
fi
log "new tree:"
