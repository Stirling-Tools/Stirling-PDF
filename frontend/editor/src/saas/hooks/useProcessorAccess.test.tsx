...
const { useProcessorAccess, useProcessorAccessState } =
  await import("@app/hooks/useProcessorAccess");

function meReturning(processorAccess: boolean) {
...
describe("useProcessorAccess", () => {
  ...
  it("reports the backend's answer for the signed-in user", async () => {
    ...
    const { result } = renderHook(() => useProcessorAccess());
  ...
  it("does not report a settled denial while the incoming session is loading", async () => {
    authLoading = true;
    const { result, rerender } = renderHook(() => useProcessorAccessState());
  ...
  it("does not give a signed-out mount access from the previous browser cache", async () => {
    currentUserId = "admin-1";
    get.mockResolvedValue(meReturning(true));
    const first = renderHook(() => useProcessorAccessState());
    await waitFor(() =>
    ...
    currentUserId = null;
    client.clear();
    const signedOut = renderHook(() => useProcessorAccessState());
  ...
  it("marks a cached answer unsettled while the incoming view revalidates it", async () => {
    ...
    const next = renderHook(() => useProcessorAccessState());
  ...
  it("re-asks the backend when a different user signs in without a reload", async () => {
    ...
    currentUserId = "admin-1";
    get.mockResolvedValue(meReturning(true));
    const { result, rerender } = renderHook(() => useProcessorAccess());
    await waitFor(() => expect(result.current).toBe(true));
  ...
  it("ignores a response that lands after unmount", async () => {
    ...
    const { result, unmount } = renderHook(() => useProcessorAccess());
    unmount();
