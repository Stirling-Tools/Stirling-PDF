...
async function fetchProcessorAccess(): Promise<boolean> {
  const res = await apiClient.get<{ user?: { processorAccess?: boolean } }>(
    "/api/v1/auth/me",
  );
  return res.data.user?.processorAccess === true;
}
