...
import { tryGetProcessorQueryClient } from "@processor/queryClient";
 ...
 * mapped onto the shared processor `Member` / `UsersResponse` types. Same paths the
...
async function resolveTeam(): Promise<TeamDetailsDTO | null> {
  ...
  // direct fetch when no processor client is mounted (e.g. a unit test).
  const client = tryGetProcessorQueryClient();
  const teams = client
...
/** Map a SaasTeamController member onto the processor Member. */
function toMember(dto: TeamMemberDTO, team: TeamDetailsDTO): Member {
  ...
  return {
    ...
    teamId: team.teamId,
    teamName: team.name,
    // The processor Users page is leader-only on SaaS, so when the viewer leads this
    ...
    // Leaders hold processor access via the role-based default policy;
    // members don't by default. Drives the roster's access chip.
    canAccessProcessor: isLeader,
    status: "active",
...
/** Map a SaasTeamController invitation onto the processor PendingInvitation. */
