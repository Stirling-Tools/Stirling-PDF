...
/** Desktop Downloads belong to this machine, never the connected server's home directory. */
export async function fetchDownloadsSuggestion(): Promise<server.DownloadsSuggestion> {
  await operationRouter.getConnectedServerBaseUrl();
  const directory = await resolveDownloadsDirectory();
  const listing = directory ? await listDirectory(directory) : null;
  return {
    directory: directory ?? "",
    available: listing !== null,
    pdfCount:
      listing?.files.filter((file) => file.name.toLowerCase().endsWith(".pdf"))
        .length ?? 0,
    limit: CLASSIFICATION_DEMO_BATCH_SIZE,
  };
}
