...
function Probe() {
  ...
  return (
    <>
      <span data-testid="access">{String(v?.processorAccess)}</span>
      {/* Raw, un-defaulted value: this is what SaasProcessorGate reads to decide
          "access not known yet" vs "denied". undefined = spinner forever. */}
      <span data-testid="raw">{String(v?.user?.processorAccess)}</span>
    </>
  );
}
...
describe("supabase provider processorAccess lookup", () => {
  ...
  it("calls /me on the configured API base, not the page origin", async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ user: { processorAccess: true } }),
    });
    mount();
    await waitFor(() =>
  ...
  // processorAccess undefined forever - SaasProcessorGate reads that as "still
  // loading" and hangs on a spinner instead of falling back.
  it("resolves the raw processorAccess when /me returns non-ok", async () => {
    h.apiBase = "https://api.example.com";
    fetchMock.mockResolvedValue({
      ok: false,
      status: 401,
  ...
  it("leaves the raw processorAccess defined when the request rejects", async () => {
    h.apiBase = "https://api.example.com";
