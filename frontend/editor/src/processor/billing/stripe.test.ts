 ...
 * unconfigured Supabase, and the processor-session path.
 */
...
describe("createCheckoutSession", () => {
  ...
  it("short-circuits already-subscribed to the processor URL (no client secret)", async () => {
    invoke.mockResolvedValue({
      data: {
        success: true,
        already_subscribed: true,
        portal_url: "https://processor",
      },
      error: null,
    });
    const s = await createCheckoutSession(req);
    expect(s.alreadySubscribed).toBe(true);
    expect(s.redirectUrl).toBe("https://processor");
...
describe("createPortalSession", () => {
  it("returns the processor URL", async () => {
    invoke.mockResolvedValue({
