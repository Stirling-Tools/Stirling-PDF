...
interface CheckoutSessionRequest {
  teamId: number;
  /** Where Stripe redirects on success — typically the processor billing page. */
  successUrl: string;
  /** Where Stripe redirects on cancel/close. */
  cancelUrl: string;
  /** ISO 4217 lower-case. Defaults to "usd"; processor uses the wallet's currency when set. */
...
export async function createCheckoutSession(
  ...
  const res = await invoke<CheckoutResponse>("create-checkout-session", {
    ...
    // The processor drives an in-page onComplete handler (the checkout modal stays open to
