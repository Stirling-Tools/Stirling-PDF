...
vi.mock("@processor/contexts/AccountLinkContext", () => ({
...
vi.mock("@processor/components/account-link/ConnectCallbackHost", () => ({
  ConnectCallbackHost: () => null,
}));
vi.mock("@processor/hooks/useFreeTierExhaustedPrompt", () => ({
  useFreeTierExhaustedPrompt: () => {},
}));
vi.mock("@processor/components/ProcessorChrome", () => ({
  ProcessorChrome: () => <ServerLicenseSection onSaved={() => {}} />,
}));
...
describe("ProcessorProviders license loading", () => {
  beforeEach(() => {
    fetchAppConfig
      .mockReset()
  ...
  it("loads local config before fetching the license on a direct processor visit", async () => {
