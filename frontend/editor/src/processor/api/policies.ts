 ...
 *
 * The processor calls the real Stirling policy API (`/api/v1/policies`);
...
/* Catalogue model + definitions live in @app/policies/catalog - shared with the
   editor's folder-processing setup - and are re-exported here for the processor. */
...
function policyDisplayName(entry: CatalogueEntry, t: TFunction): string {
  return t("processor.policies.defaultName", {
 ...
 * real endpoint is multipart; the processor sends no files, relying on whatever
