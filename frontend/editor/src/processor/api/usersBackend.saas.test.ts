...
// The SaaS usersBackend lives under src/saas; the processor vitest project resolves
...
import { usersBackend } from "../../saas/processor/usersBackend";
...
describe("saas usersBackend — fetchUsers mapping", () => {
  it("maps SaasTeamController members + invitations onto UsersResponse", async () => {
    ...
    // Leader is the viewer on SaaS → self (self-remove disabled) + processor access.
    expect(leader.isSelf).toBe(true);
    expect(leader.canAccessProcessor).toBe(true);
    expect(leader.teamId).toBe(1);

    const member = res.members.find((m) => m.email === "priya@acme.com")!;
    expect(member.role).toBe("member");
    expect(member.isSelf).toBeFalsy();
    expect(member.canAccessProcessor).toBe(false);
