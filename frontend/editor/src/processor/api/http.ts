/**
 * Processor API client — explicit per-backend, per-credential routing.
 *
 * ## Domains
 *
 ...
 *                             USE FOR: attended processor→SaaS reads —
 ...
 * entitlement calls. It never enters the processor — the browser is the human
...
import { reportAccountLinkBlock } from "@processor/services/accountLinkBlock";
...
export class SaasUnconfiguredError extends Error {
  constructor() {
    super(
      "SaaS API not configured — set VITE_SAAS_API_URL to enable processor→SaaS reads.",
    );
...
async function saasJson<T>(
  ...
  const token = await getProcessorSaasToken();
