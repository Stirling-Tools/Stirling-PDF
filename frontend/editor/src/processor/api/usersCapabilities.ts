 ...
 * selects the client + credential (`apiClient.local` in @processor/api/http, via the
 ...
 * `src/proprietary/processor/usersCapabilities.ts`
 * (self-hosted, org-admin: everything) and `src/saas/processor/usersCapabilities.ts`
...
export interface UsersCapabilities {
  ...
  seats: boolean;
  /**
   * Manage Processor access grants. The grant endpoints are ADMIN-only,
   ...
   * processor-eligible session can call.
   */
