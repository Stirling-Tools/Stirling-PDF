...
/**
 * Effective processor access for a member:
...
export interface Member {
  ...
  role: RoleId;
  status: MemberStatus;
  /** Effective processor access; set by the view from the grant list. */
  processorAccess?: ProcessorAccessState;
  /** Authoritative server-side processor access (roster DTO); drives whether a chip shows at all. */
  canAccessProcessor?: boolean;
  /** The explicit PORTAL grant's id, for revoke (present when access = granted). */
  processorGrantId?: number;
...
export const ROLES: Role[] = [
  {
    id: "admin",
    label: "processor.users.roles.admin.label",
    summary: "processor.users.roles.admin.summary",
    permissions: [
      "processor.users.roles.admin.permissions.0",
      "processor.users.roles.admin.permissions.1",
      "processor.users.roles.admin.permissions.2",
      "processor.users.roles.admin.permissions.3",
    ],
    tone: "purple",
  },
  {
    id: "team_owner",
    label: "processor.users.roles.team_owner.label",
    summary: "processor.users.roles.team_owner.summary",
    permissions: [
      "processor.users.roles.team_owner.permissions.0",
      "processor.users.roles.team_owner.permissions.1",
      "processor.users.roles.team_owner.permissions.2",
      "processor.users.roles.team_owner.permissions.3",
    ],
    tone: "blue",
  },
  {
    id: "member",
    label: "processor.users.roles.member.label",
    summary: "processor.users.roles.member.summary",
    permissions: [
      "processor.users.roles.member.permissions.0",
      "processor.users.roles.member.permissions.1",
      "processor.users.roles.member.permissions.2",
      "processor.users.roles.member.permissions.3",
    ],
    tone: "green",
  },
  {
    id: "guest",
    label: "processor.users.roles.guest.label",
    summary: "processor.users.roles.guest.summary",
    permissions: [
      "processor.users.roles.guest.permissions.0",
      "processor.users.roles.guest.permissions.1",
      "processor.users.roles.guest.permissions.2",
      "processor.users.roles.guest.permissions.3",
    ],
    tone: "neutral",
  },
];
...
interface AdminUserSummaryDto {
  ...
  /** Authoritative server-side processor access (honors the configured default policy). */
...
/**
 * GET /api/v1/proprietary/ui-data/admin-settings adapted onto the processor's
...
export async function fetchUsers(tier: Tier): Promise<UsersResponse> {
  ...
  const members: Member[] = (data.users ?? []).map((u) => ({
    ...
    canAccessProcessor: u.processorAccess === true,
...
export interface AdminAuthConfig {
   ...
   * create is hidden there - and the processor can only reach a self-hosted backend
