 ...
 * `src/proprietary/processor/usersBackend.ts` (self-hosted) and
 * `src/saas/processor/usersBackend.ts` (SaaS). This module is just the shared
 ...
 * team) stay in `@processor/api/{users,teams}` and are gated off on SaaS via
 * `usersCapabilities`.
 */
