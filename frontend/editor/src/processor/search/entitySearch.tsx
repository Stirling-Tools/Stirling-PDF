...
import { fetchPipelines, type PipelineView } from "@processor/api/pipelines";
...
function policyResultTitle(entry: CatalogueEntry, t: Translate) {
  ...
  return entry.policy
    ? t("processor.policies.defaultName", { category })
    : category;
}
...
export function buildProcessorEntityGroups(
  ...
  if (policies.length > 0) {
    groups.push({
      id: "portal-policies",
      label: t("processor.nav.policies"),
      results: policies,
    });
  }
  ...
  const pipelines = includeScope("portal-pipelines")
    ? rankPortalPipelineResults(
        ...
        policyPipelineIds,
        (pipelineId) =>
          navigate(`${toProcessorPath(VIEW_PATHS.pipelines)}/${pipelineId}`),
        ENTITY_GROUP_LIMIT,
      )
    : [];
  if (pipelines.length > 0) {
    groups.push({
      id: "portal-pipelines",
      label: t("processor.nav.pipelines"),
      results: pipelines,
    });
  }
  ...
  const sources = includeScope("portal-sources")
    ? rankByFuzzy(entities.sources, trimmed, [
        ...
        .map(({ item, score }) => ({
          ...
          score,
          onSelect: () =>
            navigate(`${toProcessorPath(VIEW_PATHS.sources)}/${item.id}`),
        }))
    : [];
  if (sources.length > 0) {
    groups.push({
      id: "portal-sources",
      label: t("processor.nav.sources"),
      results: sources,
    });
  }
