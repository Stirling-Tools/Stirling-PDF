...
/** Home processor visualiser, backed by the global processor MSW handlers.
 *  Particles animate via rAF (paused for hidden tabs — view in a focused tab). */
const meta: Meta<typeof ProcessorFlow> = {
  title: "Processor/Components/ProcessorFlow",
...
const CATEGORY_LABEL = (id: string) =>
  `processor.policies.categories.${id}.label`;
...
function buildModel(a: PlaygroundArgs): ProcessorFlowModel {
  ...
  const outcomes: FlowOutcome[] = [
    {
      key: "success",
      labelKey: "processor.processorFlow.outcomes.success",
      count24h: a.delivered,
    },
    {
      key: "failed",
      labelKey: "processor.processorFlow.outcomes.failed",
      count24h: a.failed,
    },
  ];
  const comingSoonSources = [
    {
      key: "apiMcp",
      labelKey: "processor.processorFlow.sources.comingSoon.apiMcp",
    },
    {
      key: "cloud",
      labelKey: "processor.processorFlow.sources.comingSoon.cloud",
    },
    {
      key: "email",
      labelKey: "processor.processorFlow.sources.comingSoon.email",
    },
  ];
