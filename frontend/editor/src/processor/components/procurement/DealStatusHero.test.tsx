...
import type { ProcurementSnapshot } from "@processor/api/procurement";

vi.mock("react-i18next", () => ({
  useTranslation: () => ({
    t: (key: string, value?: string | { count?: number }) => {
      if (typeof value === "string") return value;
      if (key === "processor.procurement.journey.daysLeft")
        return `${value?.count} days left`;
      return key;
    },
  }),
}));
vi.mock("@processor/components/procurement/CalendlyInline", () => ({
  warmCalendly: vi.fn(),
}));
