...
// We therefore keep the widget on one light, high-contrast palette in both processor themes; it reads as a
...
export function CalendlyInline({
  ...
  if (failed) {
    return (
      <p className="processor-sidemodal__text">
        {t("processor.procurement.schedule.fallback")}{" "}
        <a href={url} target="_blank" rel="noopener noreferrer">
          {t("processor.procurement.schedule.fallbackLink")}
        </a>
      </p>
    );
  }
  ...
  return (
    <div className="processor-calendly" style={{ minWidth: 320, height }}>
      {loading && (
        <div className="processor-calendly__loading">
          <Spinner
            size="lg"
            label={t("processor.procurement.schedule.loading")}
          />
        </div>
      )}
      {/* Calendly injects its iframe here; the spinner overlays until the widget is initialised. */}
      <div ref={containerRef} className="processor-calendly__embed" />
    </div>
  );
}
