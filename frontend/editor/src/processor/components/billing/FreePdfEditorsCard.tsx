...
export function FreePdfEditorsCard() {
  ...
  return (
    <Card padding="loose">
      <div className="processor-billing__fleet-row">
        <div className="processor-billing__editors-id">
          <span className="processor-billing__editors-icon" aria-hidden>
            <GroupsIcon sx={{ fontSize: 26 }} />
          </span>
          <div>
            <h3 className="processor-billing__section-title">
              {t("processor.billing.freeEditors.title", "Free PDF Editors")}
            </h3>
            <p className="processor-billing__section-sub">
              {t(
                "processor.billing.freeEditors.subtitle",
                "Deploy anywhere, for your whole team.",
              )}
            </p>
          </div>
        </div>
        <MetricStrip className="processor-billing__fleet-metrics">
          <MetricCard
            label={t(
              "processor.billing.freeEditors.editorsDeployed",
              "Editors deployed",
            )}
          ...
          <MetricCard
            label={t(
              "processor.billing.freeEditors.activeThisMonth",
              "Active this month",
            )}
            value={fmtMetric(data?.activeThisMonth, loading)}
          />
          <MetricCard
            label={t("processor.billing.freeEditors.pdfsEdited", "PDFs edited")}
            value={fmtMetric(data?.pdfsProcessed, loading)}
          />
          <MetricCard
            label={t("processor.billing.freeEditors.cost", "Cost")}
            value="$0"
          />
        </MetricStrip>
        ...
        >
          {t(
            "processor.billing.freeEditors.inviteTeammates",
            "Invite teammates",
          )}
        </Button>
      </div>
