...
export function InvoicesList() {
  ...
  const columns: TableColumn<Invoice>[] = [
    {
      key: "date",
      header: t("processor.billing.invoices.columnDate", "Date"),
      render: (inv) =>
    ...
    },
    {
      key: "pdfs",
      header: t(
        "processor.billing.invoices.columnPdfsProcessed",
        "PDFs processed",
      ),
      align: "right",
    ...
    {
      key: "amount",
      header: t("processor.billing.invoices.columnAmount", "Amount"),
      align: "right",
      render: (inv) =>
        inv.totalMinor == null
          ? "—"
    ...
    {
      key: "status",
      header: t("processor.billing.invoices.columnStatus", "Status"),
      render: (inv) => (
    ...
    {
      key: "description",
      header: t("processor.billing.invoices.columnDescription", "Description"),
      render: (inv) => (
        <span className="processor-billing__invoice-desc">
          {inv.description ??
            t("processor.billing.invoices.descriptionFallback", "Invoice")}
        </span>
      ),
    },
    {
      ...
      align: "right",
      render: (inv) => (
        <div className="processor-billing__invoice-actions">
          {inv.hostedInvoiceUrl && (
            <a
              className="processor-billing__invoice-link"
              href={inv.hostedInvoiceUrl}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={t(
                "processor.billing.invoices.viewAriaLabel",
            ...
            >
              {t("processor.billing.invoices.viewLink", "View ↗")}
            </a>
          )}
          {inv.invoicePdf && (
            <a
              ...
              rel="noopener noreferrer"
              aria-label={t(
                "processor.billing.invoices.downloadAriaLabel",
            ...
            >
              {t("processor.billing.invoices.pdfLink", "PDF ↓")}
            </a>
          )}
        </div>
      ),
  ...
  return (
    <Card padding="loose">
      <h3 className="processor-billing__section-title">
        {t("processor.billing.invoices.title", "Invoice history")}
      </h3>
      ...
      {error && (
        <p className="processor-billing__error" role="alert">
          {t(
            "processor.billing.invoices.loadError",
      ...
      {invoices !== null && invoices.length === 0 && !error && (
        <EmptyState
          size="compact"
          title={t("processor.billing.invoices.emptyTitle", "No invoices yet")}
          description={t(
            "processor.billing.invoices.emptyDescription",
      ...
      {invoices !== null && invoices.length > 0 && (
        <>
          <Table
            className="processor-billing__flush-table"
            columns={columns}
            rows={visibleRows}
            rowKey={(inv) => inv.id}
          />
          {hasMore && (
            <div className="processor-billing__invoice-footer">
              <Button
                variant="tertiary"
                size="sm"
              ...
              >
                {showAll
                  ? t(
                      "processor.billing.invoices.showFewer",
                      "Show fewer (top {{count}})",
                      { count: DEFAULT_VISIBLE },
                    )
                  : atFetchLimit
                    ? t(
                        "processor.billing.invoices.showMostRecent",
                        "Show {{count}} most recent",
                        { count: total },
                      )
                    : t(
                        "processor.billing.invoices.showAll",
                        "Show all {{count}}",
                        {
                          count: total,
                        },
              ...
              {showAll && atFetchLimit && (
                <span className="processor-billing__invoice-note">
                  {t(
                    "processor.billing.invoices.fetchLimitNote",
