...
export function SpendLimitCard({
    ...
    return (
      <Card padding="loose">
        <span className="processor-billing__eyebrow">
          {t("processor.billing.spendLimit.eyebrow", "Spend limit")}
        </span>
        <h3 className="processor-billing__section-title">
          {t(
            "processor.billing.spendLimit.editTitle",
        ...
        <SharedSpendCapControl
          ...
          note={t(
            "processor.billing.spendLimit.capControlNote",
        ...
        {proj && draftCap !== proj.suggestedMajor && (
          <Button
            variant="quiet"
            type="button"
            className="processor-billing__suggested"
            onClick={() => setDraftCap(proj.suggestedMajor)}
          >
            {t(
              "processor.billing.spendLimit.useSuggested",
        ...
        <div className="processor-billing__guardrail">
          <strong>
            {t(
              "processor.billing.spendLimit.guardrailLabel",
              "Your guardrail:",
            )}
          </strong>{" "}
          {t(
            "processor.billing.spendLimit.guardrailBody",
        ...
        {error && (
          <Banner
            tone="danger"
            title={t(
              "processor.billing.spendLimit.saveError",
              "Couldn't save limit",
            )}
          >
            {error}
        ...
        <div className="processor-billing__edit-actions">
          <Button
            variant="secondary"
            size="sm"
            onClick={() => onAdjustingChange(false)}
          >
            {t("processor.billing.spendLimit.cancel", "Cancel")}
          </Button>
          <Button size="sm" loading={saving} onClick={save}>
            {t("processor.billing.spendLimit.save", "Save limit")}
          </Button>
        </div>
      </Card>
    );
  ...
  return (
    <Card padding="loose" id="processor-spend-limit">
      <div className="processor-billing__subscription-head">
        <div>
          ...
          <p className="processor-billing__section-sub">
            {t(
              "processor.billing.spendLimit.displaySub",
        ...
        {isLeader && (
          ...
          >
            {t("processor.billing.spendLimit.adjustLimit", "Adjust limit")}
          </Button>
        )}
      </div>

      <div className="processor-billing__spend-meter">
        <MeterBar
          state={capActive ? state : "FULL"}
          pct={pct}
          barLabel={t("processor.billing.spendLimit.eyebrow", "Spend limit")}
          figure={
          ...
          capSuffix={
            capActive
              ? docEstimate != null
                ? t(
                    "processor.billing.spendLimit.capSuffixWithDocs",
                    "/ month · ≈ {{documents}} documents",
                    {
                      documents: docEstimate.toLocaleString(),
                    },
                  )
                : t("processor.billing.spendLimit.capSuffix", "/ month")
              : t("processor.billing.spendLimit.noCap", "no cap")
          }
          statusLabel={
            capActive
              ? t("processor.billing.spendLimit.pctUsed", "{{pct}}% used", {
                  pct: Math.round(pct),
                })
              : null
          }
          ...
          meta={
            capActive ? (
              <>
                <span>
                  {t(
                    "processor.billing.spendLimit.usedThisMonth",
                ...
                <span>
                  {t(
                    "processor.billing.spendLimit.remaining",
                    "{{amount}} remaining",
                    {
              ...
              <span>
                {t(
                  "processor.billing.spendLimit.thisPeriodUncapped",
      ...
      {proj && (
        <p className="processor-billing__projection">
          <strong>
            {t(
              "processor.billing.spendLimit.projection.label",
              "Projected to exceed.",
            )}
          </strong>{" "}
          {t(
            "processor.billing.spendLimit.projection.body",
