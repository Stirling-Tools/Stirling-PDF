import { useTranslation } from "react-i18next";
import { Card } from "@app/ui";
import { formatMinor } from "@app/billing";
import type { Wallet } from "@processor/api/billing";
import { EnterpriseUpsell } from "@processor/components/billing/EnterpriseUpsell";

/**
 * Actual metered spend this period, with the Enterprise upsell tacked onto the
 * foot (matching marketing — not a separate full-width row). Flex column so the
 * upsell pins to the bottom and the card matches the spend-limit card's height.
 */
export function SpendThisMonthCard({ wallet }: { wallet: Wallet }) {
  const { t } = useTranslation();
  const rateLabel =
    wallet.pricePerDocMinor != null && wallet.pricePerDocMinor > 0
      ? formatMinor(wallet.pricePerDocMinor, wallet.currency)
      : null;

  return (
    <Card padding="loose" className="processor-billing__spend-this-month">
      <span className="processor-billing__eyebrow">
        {t("processor.billing.spendThisMonth.eyebrow", "Spend this month")}
      </span>
      <div className="processor-billing__bignum-row">
        <span className="processor-billing__bignum">
          {formatMinor(wallet.estimatedBillMinor ?? 0, wallet.currency)}
        </span>
      </div>
      <p className="processor-billing__section-sub">
        {rateLabel
          ? t(
              "processor.billing.spendThisMonth.processedWithRate",
              "{{formattedCount}} PDFs processed, at {{rate}} each.",
              {
                count: wallet.billableUsed,
                formattedCount: wallet.billableUsed.toLocaleString(),
                rate: rateLabel,
              },
            )
          : t(
              "processor.billing.spendThisMonth.processed",
              "{{formattedCount}} PDFs processed.",
              {
                count: wallet.billableUsed,
                formattedCount: wallet.billableUsed.toLocaleString(),
              },
            )}
      </p>
      {wallet.freeRemaining > 0 ? (
        <p className="processor-billing__section-sub processor-billing__free-remaining">
          {t(
            "processor.billing.spendThisMonth.freeRemaining",
            "{{formatted}} free PDFs remaining",
            {
              count: wallet.freeRemaining,
              formatted: wallet.freeRemaining.toLocaleString(),
            },
          )}
        </p>
      ) : null}

      <div className="processor-billing__spend-foot">
        <EnterpriseUpsell bare />
      </div>
    </Card>
  );
}
