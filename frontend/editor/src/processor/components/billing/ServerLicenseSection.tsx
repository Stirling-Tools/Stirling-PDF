...
export function ServerLicenseSection({ onSaved }: { onSaved: () => void }) {
  ...
  const certificate = licenseKey?.startsWith("file:")
    ? licenseKey.slice(5)
    : null;
  const label = t("processor.billing.license.label", "License key");
  const updateLabel = licenseKey
    ? t("processor.billing.license.update", "Update")
    : t("processor.billing.license.add", "Add");
    ...
    return (
      <Banner
        ...
        action={
          ...
          >
            {t("processor.accountLink.gate.retry", "Try again")}
          </Button>
        }
      />
    );
      ...
      <KvRow
        ...
        value={
          licenseKey ? (
            ...
            t("processor.billing.license.empty", "No license installed")
          )
        }
        door={
          <span className="billing-license__actions">
            {licenseKey && (
              <button type="button" onClick={() => setDialog("view")}>
                {t("processor.billing.license.view", "View")}
              </button>
            )}
      ...
      <Modal
        ...
        title={
          dialog === "view"
            ? label
            : licenseKey
              ? t("processor.billing.license.updateTitle", "Update license")
              : t("processor.billing.license.addTitle", "Add license")
        }
        width="lg"
        footer={
          dialog === "view" ? (
