...
export function DownloadEditorModal({ open, onClose }: Props) {
  ...
  const guideLink = (id: keyof typeof GUIDES) => (
    <button
      type="button"
      className="processor-install__guide"
      onClick={() => openUrl(GUIDES[id])}
    >
      {t("processor.home.download.guide", {
        name: t(`processor.home.download.${id}.title`),
      })}
  ...
  const note = (id: OptionId) => (
    <p className="processor-install__note">
      {t(`processor.home.download.${id}.note`)}
    </p>
  );
      ...
      case "mac": {
        ...
        return (
          <>
            ...
            >
              {t(`processor.home.download.${id}.downloadBtn`)}
            </Button>
            <p className="processor-install__eyebrow">
              {t(`processor.home.download.${id}.altLabel`)}
            </p>
      ...
      case "linux":
        return (
          <>
            ...
            >
              {t("processor.home.download.linux.guideBtn")}
            </Button>
          </>
        );
      case "docker":
        return (
          <>
            <SegmentedControl<DockerVariant>
              ...
              options={[
                {
                  label: t("processor.home.download.docker.variantStandard"),
                  value: "latest",
                },
                {
                  label: t("processor.home.download.docker.variantFat"),
                  value: "latest-fat",
                },
                {
                  label: t("processor.home.download.docker.variantLite"),
      ...
      case "manual":
        return (
          <>
            ...
            >
              {t("processor.home.download.manual.downloadBtn")}
            </Button>
            <p className="processor-install__eyebrow">
              {t("processor.home.download.manual.runLabel")}
            </p>
  ...
  const renderRow = (id: OptionId) => {
    ...
    return (
      <button
        key={id}
        type="button"
        className="processor-install__option"
        onClick={() => setSelected(id)}
      >
        <span className="processor-install__option-icon" aria-hidden>
          <Icon sx={{ fontSize: 20 }} />
        </span>
        <span className="processor-install__option-text">
          <strong>{t(`processor.home.download.${id}.title`)}</strong>
          <span>{t(`processor.home.download.${id}.tagline`)}</span>
        </span>
        <ChevronRightRounded
          className="processor-install__option-chevron"
          sx={{ fontSize: 20 }}
          aria-hidden
        />
      </button>
  ...
  return (
    <Modal
      ...
      title={
        detail
          ? t(`processor.home.download.${selected}.title`)
          : t("processor.home.download.title")
      }
      subtitle={
        detail
          ? t(`processor.home.download.${selected}.detailBody`)
          : t("processor.home.download.body")
      }
      footer={
        detail ? (
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <Button variant="secondary" onClick={() => setSelected(null)}>
              {t("processor.home.download.back")}
            </Button>
            <Button variant="primary" onClick={done}>
              {t("processor.home.download.done")}
            </Button>
          </div>
        ) : (
          <Button
          ...
          >
            {t("processor.home.download.openInBrowser")}
          </Button>
        )
      }
    >
      {detail ? (
        <div className="processor-install__detail">
          {renderDetail(selected)}
        </div>
      ) : (
        <div className="processor-install__list">
          <p className="processor-install__section">
            {t("processor.home.download.sectionDesktop")}
          </p>
          {DESKTOP.map(renderRow)}
          <p className="processor-install__section">
            {t("processor.home.download.sectionSelfHosted")}
          </p>
