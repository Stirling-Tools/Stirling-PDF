 ...
 * workbench shows, fed by the processor-wired results provider. Cmd/Ctrl+K
 * focuses it (the bar registers its own shortcut). App config (the
 * admin/login gates) comes from ProcessorChrome's shared provider. The distinct
...
export function ProcessorSearchBar() {
  const scopes = useProcessorSearchScopes();
