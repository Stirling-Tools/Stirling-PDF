 ...
 * processor. The processor deliberately lives outside the editor's AppProviders, so
 ...
 * Spring on self-hosted, Supabase on SaaS — same underlying session the processor
 * is already signed in with), and the editor ThemeProvider (which also carries
 * the Mantine theme + toasts the sections expect). URL sync is off — the processor
 ...
 * the editor theme wiring never runs for processor sessions that never open
 * settings.
 */
export function ProcessorSettingsHost() {
  ...
  // Processor-only sections, appended after the build's registry sections. The
  ...
  const extraSections = useMemo<ConfigNavSection[]>(() => {
    ...
    return [
      {
        title: t("processor.settings.groups.admin", "Admin"),
        items: [
          {
            key: navKey,
        ...
        <PreferencesProvider>
          <ThemeProvider>
            <AppConfigModalLazy
              ...
              // TODO: opening Keyboard Shortcuts in the processor white-screens
