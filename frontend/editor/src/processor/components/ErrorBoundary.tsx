...
function DefaultErrorFallback({ onRetry }: { onRetry: () => void }) {
  ...
  return (
    <EmptyState
      title={t("processor.errorBoundary.title")}
      description={t("processor.errorBoundary.description")}
      actions={
        <Button onClick={onRetry}>{t("processor.errorBoundary.retry")}</Button>
      }
    />
  );
}
 ...
 * white-screen the whole processor. The app shell + navigation live OUTSIDE this
...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo): void {
    ...
    console.error("Processor view crashed:", error, info.componentStack);
  }
