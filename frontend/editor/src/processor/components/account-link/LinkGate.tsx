...
import { useLink } from "@processor/contexts/LinkContext";
  ...
  return (
    <Banner
      ...
      title={
        feature
          ? t(
              "processor.accountLink.gate.titleFeature",
              "Link to unlock {{feature}}",
              {
                feature,
              },
            )
          : t("processor.accountLink.gate.title", "Link to unlock")
      }
      description={t(
        "processor.accountLink.gate.description",
        "Link this org's Stirling account to use billable features.",
      )}
      action={
        <Button size="sm" onClick={() => openLinkModal()}>
          {t("processor.accountLink.gate.action", "Link account")}
        </Button>
      }
    />
  );
