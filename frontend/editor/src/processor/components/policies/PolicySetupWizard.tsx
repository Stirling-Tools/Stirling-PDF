...
function PolicySetupWizardBody({
  ...
  const availableSources = useMemo(() => {
    ...
    const editorSource = backendSources.find((s) => s.id === "editor") ?? {
      id: "editor",
      name: t("processor.sources.types.editor.label"),
      type: "editor",
  ...
  async function submit() {
    ...
    try {
      ...
      setError(
        errorMessage(e) || t("processor.policies.wizard.errors.saveFailed"),
      );
    }
  }
  ...
  return (
    <Modal
      ...
      width="lg"
      title={
        <span className="processor-policies__wizard-title">
      ...
      footer={
        <div className="processor-policies__wizard-foot">
          ...
          {step === "workflow" ? (
            ...
            >
              {t("processor.policies.wizard.actions.continue")}
            </Button>
          ) : (
            <>
              <Button
              ...
              >
                {t("processor.policies.wizard.actions.back")}
              </Button>
    ...
    >
      <Tabs
        ...
        items={[
          {
            key: "workflow",
            label: t("processor.policies.wizard.tabs.workflow"),
          },
          {
            key: "settings",
            label: t("processor.policies.wizard.tabs.settings"),
          },
        ]}
      />
      ...
      {error && (
        <Banner
          tone="danger"
          description={error}
          className="processor-policies__wizard-banner"
        />
      )}
      ...
      {step === "workflow" && !isClassification && (
        <div className="processor-policies__wizard-section">
          ...
          <Card padding="none">
            <div className="processor-policies__capabilities">
              {visibleTools.map((tl) => {
                ...
                return (
                  <div
                    key={tl.toolId}
                    className="processor-policies__capability"
      ...
      {step === "settings" && (
        <div className="processor-policies__wizard-section">
          {config.fields.length > 0 && (
            <>
              <h3 className="processor-policies__wizard-heading">
                {t("processor.policies.wizard.settings.heading")}
              </h3>
          ...
          <h3 className="processor-policies__wizard-heading">
            {t("processor.policies.wizard.sources.heading")}
          </h3>
          {sourcesAsync.loading && !sourcesAsync.data ? (
            <p className="processor-policies__sources-loading">
              {t("processor.policies.wizard.sources.loading")}
            </p>
          ) : (
              ...
              {availableSources.map((src) => {
                ...
                return (
                  <Button
                    ...
                    fullWidth
                    className={
                      "processor-policies__source" +
                      (on ? " processor-policies__source--on" : "")
                    }
                    ...
                    aria-pressed={on}
                  >
                    <span className="processor-policies__source-label">
                      {sourceIcon(src.type)}
                      {src.name}
                    </span>
                  </Button>
            ...
            {sources.includes("editor") && (
              <>
                <FormField
                  ...
                  helperText={t(
                    "processor.policies.wizard.output.runOn.helper",
                  )}
                >
                  <Select
                    inputSize="sm"
                    ...
                    options={[
                      {
                        value: "upload",
                        label: t(
                          "processor.policies.wizard.output.runOn.upload",
                        ),
                      },
                      {
                        value: "export",
                        label: t(
                          "processor.policies.wizard.output.runOn.export",
                        ),
                      },
                    ]}
                  />
                ...
                <FormField
                  label={t("processor.policies.wizard.output.outputAs.label")}
                >
                  <Select
                    inputSize="sm"
                    value={outputMode}
                    ...
                    options={[
                      {
                        value: "new_version",
                        label: t(
                          "processor.policies.wizard.output.outputAs.newVersion",
                        ),
                      },
                      {
                        value: "new_file",
                        label: t(
                          "processor.policies.wizard.output.outputAs.newFile",
                        ),
                      },
                    ]}
                  />
                ...
                <FormField
                  label={t(
                    "processor.policies.wizard.output.filenameRule.label",
                  )}
                >
                  <div className="processor-policies__name-row">
                    <Select
                      inputSize="sm"
                      ...
                      options={[
                        {
                          value: "prefix",
                          label: t(
                            "processor.policies.wizard.output.filenameRule.prefix",
                          ),
                        },
                        {
                          value: "suffix",
                          label: t(
                            "processor.policies.wizard.output.filenameRule.suffix",
                          ),
                        },
                        ...(outputMode === "new_file"
                          ? [
                              {
                                value: "auto-number",
                                label: t(
                                  "processor.policies.wizard.output.filenameRule.autoNumber",
                                ),
                              },
                            ]
                          : []),
                    ...
                    {outputNamePosition !== "auto-number" && (
                      <Input
                        ...
                        value={outputName}
                        placeholder={t(
                          "processor.policies.wizard.output.filenameRule.placeholder",
                        )}
