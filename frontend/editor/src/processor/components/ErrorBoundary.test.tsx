...
describe("ErrorBoundary", () => {
  it("renders the default fallback when a child throws", () => {
    ...
    expect(
      screen.getByText("processor.errorBoundary.title"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("processor.errorBoundary.description"),
    ).toBeInTheDocument();
    spy.mockRestore();
  });
  ...
  it("recovers when retry is clicked and the child stops throwing", () => {
    ...
    shouldThrow = false;
    fireEvent.click(screen.getByText("processor.errorBoundary.retry"));

    expect(screen.getByText("recovered content")).toBeInTheDocument();
    expect(
      screen.queryByText("processor.errorBoundary.title"),
