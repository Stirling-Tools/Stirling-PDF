...
export function FlowSankey({ sources, outcomes, policies }: FlowSankeyProps) {
  ...
  if (!srcSum) {
    return (
      <div className="processor-pf__sankey-empty">
        <EmptyState
          size="compact"
          title={t("processor.processorFlow.sankey.empty.title")}
          description={t("processor.processorFlow.sankey.empty.description")}
        />
      </div>
    );
  }
  ...
  flows.forEach((s, i) => {
    ...
    texts.push(
      <text
        ...
        y={accL + lt[i] / 2 + 4}
        textAnchor="end"
        className="processor-pf__sankey-label"
      >
        {label} · {s.docs24h}
      </text>,
    );
  ...
  texts.push(
    <text
      ...
      y={midY - 8}
      textAnchor="middle"
      className="processor-pf__sankey-caption"
    >
      {t("processor.processorFlow.sankey.waist", { n: activeCount })}
    </text>,
  );
  ...
  return (
    <div className="processor-pf__sankey">
