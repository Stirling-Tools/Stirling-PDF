...
export function ProcessorFlow({ dataOverride }: ProcessorFlowProps = {}) {
  ...
  const openPolicySetup = (key: string) =>
    navigate(
      `${toProcessorPath(VIEW_PATHS.pipelines)}?setup=${encodeURIComponent(key)}`,
    );

  /** Deep-link to Infrastructure with the audit-log tab open. */
  const openAuditLog = () =>
    navigate(`${toProcessorPath(VIEW_PATHS.infrastructure)}?tab=audit`);
  ...
  const statsLabel = t("processor.processorFlow.stats", {
    ...
    <Card padding="loose" className="processor-pf">
      ...
      <div className="processor-pf__head">
        <div className="processor-pf__head-text">
          <span
            className={
              "processor-pf__live" + (animate ? " processor-pf__live--on" : "")
            }
            aria-hidden
          />
          <h2 className="processor-pf__title">
            {t("processor.processorFlow.title")}
          </h2>
          <span className="processor-pf__connected">{statsLabel}</span>
        </div>
        <div className="processor-pf__head-actions">
          <StatusBadge tone={animate ? "success" : "neutral"} size="sm">
            {t("processor.processorFlow.liveBadge")}
          </StatusBadge>
          <SegmentedControl<Lens>
            size="xs"
            value={lens}
            onChange={setLens}
            ariaLabel={t("processor.processorFlow.lens.ariaLabel")}
            options={[
              { label: t("processor.processorFlow.lens.flow"), value: "flow" },
              {
                label: t("processor.processorFlow.lens.sankey"),
                value: "sankey",
              },
            ]}
          />
      ...
      {isLoading ? (
        ...
        <div className="processor-pf__stage" ref={wrapRef}>
          <svg className="processor-pf__wires" aria-hidden>
            {wires}
          </svg>

          <div className="processor-pf__cols">
            <FlowSources
              sources={sources}
          ...
          <svg className="processor-pf__particles" aria-hidden>
            <g ref={pGroupRef} />
          </svg>
        </div>
      )}

      <p className="processor-pf__foot">
        {t("processor.processorFlow.footnote")}
      </p>
    </Card>
  );
}
