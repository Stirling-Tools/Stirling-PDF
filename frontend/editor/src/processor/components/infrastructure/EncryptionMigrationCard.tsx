...
export function EncryptionMigrationCard({
  ...
  return (
    <section>
      <Card padding="loose">
        <div className="processor-enc__head">
          <SectionHeader
            title={t("processor.infrastructure.encryption.migration.heading")}
            hint={t("processor.infrastructure.encryption.migration.hint")}
            hintLabel={t("processor.infrastructure.encryption.hintLabel")}
          />
          <StatusBadge tone={STATE_TONE[state]} size="sm">
            {t(`processor.infrastructure.encryption.migration.state.${state}`)}
          </StatusBadge>
        </div>
        {status?.startedAt ? (
          <p className="processor-enc__note">
            {t("processor.infrastructure.encryption.migration.startedAt", {
        ...
        {state === "FAILED" ? (
          <Banner
            tone="danger"
            title={t(
              "processor.infrastructure.encryption.migration.failed.title",
            )}
            description={t(
              "processor.infrastructure.encryption.migration.failed.description",
            )}
          />
        ) : null}
        ...
        {state === "RUNNING" || state === "COMPLETED" || state === "FAILED" ? (
          <>
            <ProgressBar
              ...
              label={t(
                "processor.infrastructure.encryption.migration.progressLabel",
                { processed, total },
              )}
            />
            <div className="processor-enc__migration-stats">
              <StatTile
                label={t(
                  "processor.infrastructure.encryption.migration.encrypted",
                )}
              ...
              <StatTile
                label={
                  <>
                    {t("processor.infrastructure.encryption.migration.skipped")}
                    <InfoHint
                      content={t(
                        "processor.infrastructure.encryption.migration.skippedNote",
                      )}
                      label={t(
                        "processor.infrastructure.encryption.migration.skippedHelp",
                      )}
                    />
                  </>
                }
              ...
              <StatTile
                label={t(
                  "processor.infrastructure.encryption.migration.failedCount",
                )}
          ...
          <p className="processor-enc__note">
            {plaintextFiles > 0
              ? t("processor.infrastructure.encryption.migration.backlog", {
              ...
              : t("processor.infrastructure.encryption.migration.noBacklog")}
          </p>
        )}
        ...
        {!writeEnabled && plaintextFiles > 0 ? (
          <p className="processor-enc__note">
            {t(
              "processor.infrastructure.encryption.migration.requiresEncryptionOn",
            )}
          </p>
        ) : null}
        ...
        <div className="processor-enc__actions">
          ...
          >
            {state === "FAILED" || state === "COMPLETED"
              ? t("processor.infrastructure.encryption.migration.runAgain")
              : t("processor.infrastructure.encryption.migration.start")}
          </Button>
        </div>
        ...
        <Modal
          ...
          width="md"
          title={t(
            "processor.infrastructure.encryption.migration.confirm.title",
          )}
          footer={
            <>
              <Button variant="secondary" onClick={() => setConfirming(false)}>
                {t(
                  "processor.infrastructure.encryption.migration.confirm.cancel",
                )}
              </Button>
              <Button
                variant="primary"
              ...
              >
                {t(
                  "processor.infrastructure.encryption.migration.confirm.confirm",
                )}
              </Button>
            </>
          }
        >
          <ul className="processor-enc__consequences">
            <li>
              {t(
                "processor.infrastructure.encryption.migration.confirm.rewrites",
                {
                  count: plaintextFiles,
                  formatted: plaintextFiles.toLocaleString(),
                },
            ...
            <li>
              {t(
                "processor.infrastructure.encryption.migration.confirm.noCancel",
              )}
            </li>
            <li>
              {t(
                "processor.infrastructure.encryption.migration.confirm.throttled",
              )}
            </li>
          </ul>
        </Modal>
