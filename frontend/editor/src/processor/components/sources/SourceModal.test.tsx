...
describe("SourceModal", () => {
  ...
  it("creates a folder source through the staged flow and closes", async () => {
    ...
    fireEvent.change(
      screen.getByLabelText(/processor\.sources\.wizard\.name/),
      {
  ...
  it("offers a Folder Access settings link when the folder is outside allowed roots", async () => {
    ...
    expect(
      await screen.findByText("processor.sources.builder.folderAccess.title"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("processor.sources.builder.folderAccess.openSettings"),
    ).toBeInTheDocument();
  });
  ...
  it("shows a plain error banner (no settings link) for other save failures", async () => {
    ...
    expect(
      screen.queryByText("processor.sources.builder.folderAccess.openSettings"),
  ...
  it("gates the s3 type on a chosen connection", async () => {
    ...
    expect(
      screen.getByText("processor.sources.builder.create").closest("button"),
    ).toBeDisabled();
  });
  ...
  it("reveals the delivery URL and signing secret once after creating a webhook", async () => {
    ...
    fireEvent.click(
      screen.getByText("processor.sources.types.webhook.reveal.done"),
    );
    expect(onClose).toHaveBeenCalled();
  });
  ...
  it("edits an existing source prefilled and saves with its id", async () => {
    ...
    fireEvent.click(screen.getByText("processor.sources.builder.save"));
  ...
  it("deletes an existing source after the inline confirm", async () => {
    ...
    fireEvent.click(
      await screen.findByText("processor.sources.builder.delete"),
    );
    fireEvent.click(
      await screen.findByText("processor.sources.delete.confirm"),
    );
