  ...
  async function save() {
    ...
    try {
      ...
      const saved = connection
        ...
        : // TEAM scope suits the team-based processor (the backend defaults the team to the
          // caller's own). A teamless single-operator self-hosted deployment would need a
          // USER/SERVER scope choice here - follow-up if the processor ships there.
    ...
    <Modal
      ...
      title={
        isEdit
          ...
          : mode === "pick"
            ? t("processor.connections.picker2.heading")
      ...
      footer={
        ...
        mode === "form" ? (
          <div className="processor-sources__connection-create-actions">
            <Button
              variant="tertiary"
              size="sm"
              disabled={saving}
      ...
      {mode === "pick" ? (
        ...
        <>
          ...
          {type ? (
            ...
            <Banner
              tone="warning"
              description={t("processor.connections.unsupportedType")}
            />
          )}
