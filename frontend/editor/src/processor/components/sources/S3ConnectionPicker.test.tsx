...
describe("S3ConnectionPicker", () => {
  ...
  it("creates a connection inline and selects it", async () => {
    ...
    fireEvent.click(
      await screen.findByText("processor.connections.picker.createNew"),
    );
    fireEvent.change(
    ...
    fireEvent.change(
      screen.getByLabelText(
        /processor\.connections\.types\.s3\.fields\.bucket\.label/,
      ),
      { target: { value: "inbox" } },
    );
    fireEvent.change(
      screen.getByLabelText(
        /processor\.connections\.types\.s3\.fields\.accessKeyId\.label/,
      ),
      { target: { value: "AKIA" } },
    );
    fireEvent.change(
      screen.getByLabelText(
        /processor\.connections\.types\.s3\.fields\.secretAccessKey\.label/,
      ),
      { target: { value: "shh" } },
    );
    fireEvent.click(screen.getByText("processor.connections.picker.save"));
