...
describe("ConnectionModal", () => {
  ...
  it("creates a Purview connection with its tenant id", async () => {
    ...
    await fill("processor.integrations.typedName", "Corp Purview");
    await fill(
      "processor.connections.types.purview.fields.tenantId",
      "cb46c030-1825-4e81-a295-151c039dbf02",
    );
    await userEvent.click(
      screen.getByText("processor.connections.picker.save"),
    );
  ...
  it("bakes the ConsignO preset into the saved config", async () => {
    ...
    await fill("processor.integrations.typedName", "Notarius");
    await fill(
      "processor.connections.types.consigno.fields.baseUrl",
      "https://acme.consignocloud.com/api/v1",
    );
    await fill("processor.connections.types.consigno.fields.clientId", "cid");
    await fill(
      "processor.connections.types.consigno.fields.clientSecret",
      "csecret",
    );
    await fill(
      "processor.connections.types.consigno.fields.username",
      "api@acme.test",
    );
    await fill("processor.connections.types.consigno.fields.password", "pw");
    await userEvent.click(
      screen.getByText("processor.connections.picker.save"),
    );
  ...
  it("cannot be saved until the required fields are filled", async () => {
    ...
    const save = screen.getByText("processor.connections.picker.save");
    expect(save.closest("button")).toBeDisabled();

    await fill("processor.integrations.typedName", "Corp");
  ...
  it("offers custom API only to admins, and only under Advanced", async () => {
    ...
    expect(
      screen.queryByText("processor.connections.categories.advanced.label"),
    ).toBeNull();
    ...
    await waitFor(() =>
      expect(
        screen.getByText("processor.connections.types.api.label"),
      ).toBeInTheDocument(),
    );
    expect(
      screen.getByText("processor.connections.categories.advanced.label"),
    ).toBeInTheDocument();
  });
  ...
  it("pins the custom API form even without capabilities, rather than blanking", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByLabelText(
          /processor\.connections\.types\.api\.fields\.baseUrl/,
          {
            exact: false,
          },
        ),
    ...
    expect(
      screen.queryByPlaceholderText(
        "processor.connections.picker2.searchPlaceholder",
      ),
    ).toBeNull();
  });
  ...
  it("searches across vendor aliases rather than just the label", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("processor.connections.types.splunk.label"),
      ).toBeInTheDocument(),
    );
    // A flat result list, not every category heading it happens to fall under.
    expect(
      screen.queryByText("processor.connections.types.s3.label"),
    ).toBeNull();
  });
  ...
  it("picking a vendor opens its form, and the header back arrow returns to the grid", async () => {
    ...
    await userEvent.click(
      screen.getByText("processor.connections.types.purview.label"),
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByText("processor.connections.types.s3.label"),
      ).toBeInTheDocument(),
    );
  });
  ...
  it("says so when a search matches nothing", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("processor.connections.picker2.noResultsTitle"),
      ).toBeInTheDocument(),
    );
