...
vi.mock("@processor/api/fileRunEvents", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@processor/api/fileRunEvents")>()),
...
>(() => null);
vi.mock("@processor/queries/sources", () => ({
  ...
  useTranslation: () => ({
    ...
    t: (
      ...
      const known: Record<string, string> = {
        ...
        "processor.failures.kind.unknown.title": "Unrecognised failure",
        "processor.failures.action.acknowledge": "Acknowledge",
        "processor.failures.action.dismiss": "Dismiss",
        "processor.failures.empty.title": "Nothing needs your attention",
        "processor.failures.occurrences": "occurrences",
        "processor.failures.scope.open": "Open",
        "processor.failures.scope.closed": "Closed",
        "processor.failures.outcome.dismissed": "Dismissed",
        "processor.failures.outcome.fileRemoved": "File deleted",
        "processor.failures.origin.tool": "Editor tool",
        "processor.failures.origin.policy": "Policy run",
      };
