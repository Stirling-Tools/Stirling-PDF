  ...
  useTranslation: () => ({
    ...
    t: (key: string, options?: { defaultValue?: string } | string) => {
      const known: Record<string, string> = {
        ...
        "processor.failures.action.dismissSkipFile": "Skip this file",
        "processor.failures.action.confirm": "Are you sure?",
        "processor.failures.disabled.closed": "This failure is already closed.",
      };
...
describe("FailureActionButtons", () => {
    ...
    render(
      <FailureActionButtons
        event={event([
          ...
          offer({
            id: "DISMISS",
            labelKey: "processor.failures.action.dismissSkipFile",
          }),
        ])}
        onAction={vi.fn()}
      />,
  ...
  it("falls back to the generic label when the server's key is unknown to this build", () => {
    render(
      <FailureActionButtons
        event={event([
          offer({ labelKey: "processor.failures.action.brandNewKey" }),
        ])}
        onAction={vi.fn()}
      />,
    );
