...
import "@processor/components/pipelines/PipelineEditHeader.css";
...
export function PipelineEditHeader({
  ...
  return (
    <section className="processor-pipeline-edit-header">
      <div className="processor-pipeline-edit-header__identity">
        <ActionIcon
          variant="quiet"
          size="sm"
          onClick={onBack}
        ...
        {renaming ? (
          <Input
            ref={inputRef}
            className="processor-pipeline-edit-header__name-input"
            value={draft}
          ...
          <>
            <h1 className="processor-pipeline-edit-header__title">{name}</h1>
            <ActionIcon
              variant="quiet"
              size="sm"
              onClick={startRename}
              aria-label={t("processor.pipelines.builder.rename")}
            >
      ...
      <div className="processor-pipeline-edit-header__actions">
        ...
        >
          {enabled
            ? t("processor.pipelines.builder.pause")
            : t("processor.pipelines.builder.activate")}
        </Button>
        ...
        >
          {t("processor.pipelines.detail.run")}
        </Button>
        ...
        <Dropdown.Root align="end">
          <Dropdown.Trigger>
            <ActionIcon
              variant="tertiary"
              size="sm"
              aria-label={t("processor.pipelines.builder.moreActions")}
            >
          ...
          <Dropdown.Menu>
            ...
            >
              {t("processor.pipelines.detail.clearHistory")}
            </Dropdown.Item>
            <Dropdown.Divider />
            <Dropdown.Item
              onSelect={onDelete}
              className="processor-pipeline-edit-header__delete-item"
              leading={
                <DeleteOutlineRoundedIcon style={{ fontSize: "1.125rem" }} />
              }
            >
              {t("processor.pipelines.detail.delete")}
            </Dropdown.Item>
          </Dropdown.Menu>
        </Dropdown.Root>
        ...
        <PipelineBlockerTooltip
          heading={t("processor.pipelines.builder.blocker.saveHeading")}
          blockers={blockers}
        >
          <span className="processor-pipeline-edit-header__save">
            <Button
              size="sm"
              onClick={onSave}
              loading={saving}
            ...
            >
              {t("processor.pipelines.composer.save")}
            </Button>
          </span>
        </PipelineBlockerTooltip>
      </div>
