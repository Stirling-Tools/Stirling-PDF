...
import { PolicyExternalApiConfig } from "@processor/components/policies/PolicyExternalApiConfig";
import { isIntegrationStep } from "@processor/components/pipelines/integrationStep";
import type { ExternalApiStepParams } from "@processor/components/policies/stepOperations";
import "@processor/components/pipelines/PipelineStepSettings.css";
...
export function PipelineStepSettings({
  ...
  function toolBody() {
    if (step.support === "noSettings") {
      return (
        <Banner
          tone="info"
          description={t("processor.pipelines.composer.noToolSettings")}
        />
      );
    }
    ...
    if (!Settings) {
      return (
        <Banner
          tone="warning"
          description={t("processor.pipelines.composer.editingUnsupported")}
        />
      );
    }
    return (
  ...
  return (
    <>
      {chips.length > 0 && (
        <div className="processor-step-settings__files">
          <span className="processor-step-settings__files-label">
            {t("processor.pipelines.builder.supportingFiles")}
          </span>
          <div className="processor-step-settings__files-chips">
            {chips.map((chip) => (
              <Chip
                key={chip.field}
                leadingIcon={
