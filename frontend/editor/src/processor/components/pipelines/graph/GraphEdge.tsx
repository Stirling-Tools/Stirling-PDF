...
import type { LaidOutEdge } from "@processor/components/pipelines/graph/pipelineLayout";
import { useEdgeDrop } from "@processor/components/pipelines/graph/useChainDragDrop";
import "@processor/components/pipelines/graph/GraphEdge.css";
...
export function GraphEdge({
  ...
  const insertButton = open ? (
    <ActionIcon
      ...
      size="sm"
      shape="circle"
      className="processor-graph-edge__insert"
      aria-label={t("processor.pipelines.graph.insertHere")}
  ...
  return (
    ...
    >
      <span className="processor-graph-edge__line" aria-hidden />
      {warning ? (
        <span className="processor-graph-edge__note">
          <span className="processor-graph-edge__warning" title={warning.text}>
            <WarningAmberRoundedIcon style={{ fontSize: "0.875rem" }} />
            <span className="processor-graph-edge__warning-label">
              {warning.text}
            </span>
          </span>
          {insertButton}
