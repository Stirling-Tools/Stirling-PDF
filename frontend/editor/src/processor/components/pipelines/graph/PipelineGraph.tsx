...
import {
  GraphNode,
  type NodeRunState,
} from "@processor/components/pipelines/graph/GraphNode";
import {
  GraphEdge,
  type ChainWarning,
} from "@processor/components/pipelines/graph/GraphEdge";
import { GraphPlaceholderNode } from "@processor/components/pipelines/graph/GraphPlaceholderNode";
import {
  NODE_HEIGHT,
  NODE_WIDTH,
  layoutChain,
...
import { useStepDraggable } from "@processor/components/pipelines/graph/useChainDragDrop";
import "@processor/components/pipelines/graph/PipelineGraph.css";
...
export function PipelineGraph({
  ...
  function onBackgroundClick(event: ReactMouseEvent<HTMLDivElement>) {
    ...
    if (
      target.closest(
        "[data-graph-node], .processor-graph-edge, .processor-graph-placeholder",
      )
    ) {
      return;
    }
  ...
  return (
    <div
      ref={graphRef}
      className="processor-graph"
      onKeyDown={onKeyDown}
      onClick={onBackgroundClick}
    >
      <div
        className="processor-graph__canvas"
      ...
      >
        ...
        {draggingIndex !== null && edges.length > 0 && (
          <p
            className="processor-graph__drag-hint"
            aria-live="polite"
            style={{
              left: `${edges[0].x}px`,
          ...
          >
            {t("processor.pipelines.graph.dragHint")}
          </p>
        )}
        ...
        {nodes.map((node) => {
          ...
          if (node.kind === "placeholder") {
            return (
              <div
                className="processor-graph__slot"
                key={node.id}
                style={style}
              >
                <GraphPlaceholderNode
                  label={t("processor.pipelines.graph.addFirstTool")}
          ...
          if (node.kind === "input" || node.kind === "output") {
            ...
            return (
              ...
              >
                {content === null ? (
                  <GraphPlaceholderNode
                    label={t(`processor.pipelines.graph.add.${kind}`)}
                    onAdd={() => onAddEnd(kind)}
                  />
                ) : (
                  <GraphNode
          ...
          return (
            <div className="processor-graph__slot" key={node.id} style={style}>
              <ChainStepNode
                index={index}
                step={steps[index]}
