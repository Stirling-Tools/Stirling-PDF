...
describe("PipelineHeader", () => {
  ...
  it("offers run, clear history and delete only once the pipeline exists", () => {
    ...
    expect(
      screen.queryByText("processor.pipelines.detail.run"),
    ...
    expect(
      screen.getByText("processor.pipelines.builder.testRun"),
    ).toBeInTheDocument();
  });
  ...
  it("labels the save action for what it will do", () => {
    renderHeader({ isEdit: false });
    expect(
      screen.getByText("processor.pipelines.composer.create"),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("processor.pipelines.composer.save"),
    ).not.toBeInTheDocument();
  });
  ...
  it("leaves the page through cancel and back", () => {
    ...
    fireEvent.click(screen.getByText("processor.pipelines.builder.back"));
  ...
  it("keeps the occasional actions out of the row, behind a tray", () => {
    ...
    expect(
      screen.queryByText("processor.pipelines.builder.viewDefinition"),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("opens the definition from the tray", () => {
    ...
    fireEvent.click(
      screen.getByText("processor.pipelines.builder.viewDefinition"),
    );
