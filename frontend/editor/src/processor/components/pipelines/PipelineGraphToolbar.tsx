...
import { type RunOutputFile } from "@processor/api/pipelines";
import "@processor/components/pipelines/PipelineGraphToolbar.css";
...
export function PipelineGraphToolbar({
  ...
  return (
    <div className="processor-pipeline-toolbar">
      <FilePicker
        variant="secondary"
        size="sm"
      ...
      >
        {t("processor.pipelines.builder.testRun")}
      </FilePicker>
      ...
      <Tooltip
        label={t("processor.pipelines.builder.viewDefinition")}
        position="bottom"
        withinPortal
      >
        <ActionIcon
          variant="tertiary"
          size="sm"
          className="processor-pipeline-toolbar__definition"
          onClick={onViewDefinition}
          aria-label={t("processor.pipelines.builder.viewDefinition")}
        >
...
function RunResultStrip({ result, onDownload }: RunResultStripProps) {
  ...
  return (
    <div className="processor-pipeline-toolbar__result">
      <div className="processor-pipeline-toolbar__result-status">
        {result.status === "running" && <Spinner size="sm" />}
        {result.status === "completed" && (
          <CheckCircleOutlineRoundedIcon
            className="processor-pipeline-toolbar__result-icon is-ok"
            style={{ fontSize: "1.25rem" }}
          />
        )}
        {result.status === "failed" && (
          <ErrorOutlineRoundedIcon
            className="processor-pipeline-toolbar__result-icon is-bad"
            style={{ fontSize: "1.25rem" }}
          />
        )}
        <span>
          {t(`processor.pipelines.inspector.status.${result.status}`, {
      ...
      {result.status === "failed" && result.error && (
        <span className="processor-pipeline-toolbar__result-error">
          {result.error}
        </span>
      )}
