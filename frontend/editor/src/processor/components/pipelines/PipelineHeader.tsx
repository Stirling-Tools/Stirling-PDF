...
import "@processor/components/pipelines/PipelineHeader.css";
...
export function PipelineHeader({
  ...
  return (
    <section className="processor-pipeline-header">
      <div className="processor-pipeline-header__top">
        <Button
          variant="quiet"
          size="sm"
          className="processor-pipeline-header__back"
          onClick={onBack}
          leftSection={
            <ArrowBackRoundedIcon style={{ fontSize: "1.125rem" }} />
          }
        >
          {t("processor.pipelines.builder.back")}
        </Button>
        <div className="processor-pipeline-header__save">
          <Button
            variant="tertiary"
            size="sm"
            onClick={onCancel}
            disabled={saving}
          >
            {t("processor.pipelines.composer.cancel")}
          </Button>
          <Button
            size="sm"
            onClick={onSave}
      ...
      <div className="processor-pipeline-header__identity">
        <Input
          className="processor-pipeline-header__name"
          value={name}
        ...
        <Checkbox
          className="processor-pipeline-header__enabled"
          checked={enabled}
          onChange={(e) => onEnabledChange(e.target.checked)}
          label={t("processor.pipelines.builder.enabled")}
        />
      </div>

      <div className="processor-pipeline-header__actions">
        <FilePicker
          variant="secondary"
          size="sm"
        ...
        <Dropdown.Root align="start">
          ...
          <Dropdown.Menu>
            ...
            >
              {t("processor.pipelines.builder.viewDefinition")}
            </Dropdown.Item>
            {isEdit && (
              <Dropdown.Item
        ...
        {isEdit && (
          <Button
            ...
            size="sm"
            accent="danger"
            className="processor-pipeline-header__delete"
            onClick={onDelete}
            leftSection={
...
function RunResultStrip({ result, onDownload }: RunResultStripProps) {
  ...
  return (
    <div className="processor-pipeline-header__result">
      <div className="processor-pipeline-header__result-status">
        {result.status === "running" && <Spinner size="sm" />}
        {result.status === "completed" && (
          <CheckCircleOutlineRoundedIcon
            className="processor-pipeline-header__result-icon is-ok"
            style={{ fontSize: "1.25rem" }}
          />
        )}
        {result.status === "failed" && (
          <ErrorOutlineRoundedIcon
            className="processor-pipeline-header__result-icon is-bad"
            style={{ fontSize: "1.25rem" }}
          />
        )}
        <span>
      ...
      {result.status === "failed" && result.error && (
        <span className="processor-pipeline-header__result-error">
          {result.error}
        </span>
      )}
