...
import type { SourcesResponse } from "@processor/api/sources";
...
describe("Sources view", () => {
  ...
  it("opens the edit modal on row click", async () => {
    ...
    expect(
      await screen.findByText("processor.sources.builder.save"),
    ).toBeInTheDocument();
  ...
  it("opens the create modal from the connect button", async () => {
    ...
    fireEvent.click(
      screen.getAllByText("processor.sources.actions.connectSource")[0],
    );
    // Stage 1 of the modal: the connector catalogue with coming-soon entries.
    expect(
      await screen.findByText("processor.sources.types.sharepoint.label"),
    ).toBeInTheDocument();
  });
  ...
  it("does not open the modal when the virtual editor row is clicked", async () => {
    ...
    expect(
      screen.queryByText("processor.sources.builder.save"),
