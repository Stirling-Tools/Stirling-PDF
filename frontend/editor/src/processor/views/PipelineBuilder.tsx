...
import {
  ...
  uploadPipelineAsset,
  type PolicyAsset,
} from "@processor/api/pipelineAssets";
...
import { RoutingSection } from "@processor/components/policies/RoutingRules";
...
import { PipelineGraphToolbar } from "@processor/components/pipelines/PipelineGraphToolbar";
import { PipelineInspector } from "@processor/components/pipelines/PipelineInspector";
import { PipelineDefinitionModal } from "@processor/components/pipelines/PipelineDefinitionModal";
import {
  PipelineGraph,
  selectedSteps,
  type ChainEnd,
...
import {
  ...
  type ParamsUpdate,
} from "@processor/components/pipelines/PipelineStepSettings";
...
import {
  ...
  type ScheduleUnit,
  type WorkingInput,
} from "@processor/components/pipelines/PipelineInputTrigger";
...
export function PipelineBuilder() {
  ...
  function triggerOptionsFor(sourceId: string) {
    const options = [
      {
        value: MANUAL_OPTION,
        label: t("processor.pipelines.composer.triggerManual"),
      },
    ];
    ...
    if (type) {
      for (const trigger of triggers) {
        if (triggerFitsType(trigger, type)) {
          options.push({
            value: trigger.type,
            label: t(`processor.pipelines.trigger.${trigger.type}`, {
  ...
  function stepLabel(step: WorkingToolStep): string {
    ...
    if (isIntegrationStep(step))
      return t("processor.pipelines.builder.sendToSystem");
  ...
  function diagnosticNote(diagnostic: ToolDiagnostic): string {
    ...
    return t(`processor.pipelines.builder.diagnostic.${diagnostic.code}`, {
  ...
  if (name.trim() === "")
    blockers.push(t("processor.pipelines.builder.blocker.name"));
  ...
  if (!sourceChosen)
    blockers.push(t("processor.pipelines.builder.blocker.source"));
  else if (!scheduleValid)
    blockers.push(t("processor.pipelines.builder.blocker.schedule"));
  if (!outputValid)
    blockers.push(t("processor.pipelines.builder.blocker.destination"));
  if (!routingValid)
    blockers.push(
      t(
        "processor.pipelines.builder.blocker.routing",
  ...
  if (!routingHasVerdict)
    blockers.push(
      t(
        "processor.pipelines.builder.blocker.routingNeedsClassify",
  ...
  if (classifies && !aiAvailabilityLoading && !aiClassificationEnabled)
    blockers.push(
      t(
        "processor.pipelines.builder.blocker.aiClassification",
        "Enable AI classification in Settings, or remove the Classify step",
      ),
    );
  if (hasUnconfiguredSteps)
    blockers.push(
      t("processor.pipelines.builder.blocker.setup", {
        tools: unconfiguredStepLabels.join(", "),
      }),
    );
  if (hasIncompatibleSteps)
    blockers.push(
      t("processor.pipelines.builder.blocker.incompatible", {
  ...
  async function handleTest(file: File) {
    ...
    try {
      ...
      const { runId } = await runPipelineTest(
        {
          name: name.trim() || t("processor.pipelines.builder.testRun"),
          steps: testSteps,
  ...
  function emptySweepResult(outcome: TriggerOutcome): RunResult {
    if (outcome.parked > 0) {
      return {
        tone: "warning",
        text: t("processor.pipelines.run.parked", { count: outcome.parked }),
      };
    }
    if (outcome.inFlight > 0) {
      return { tone: "info", text: t("processor.pipelines.run.inFlight") };
    }
    if (outcome.alreadyProcessed > 0) {
      return {
        tone: "info",
        text: t("processor.pipelines.run.allProcessed", {
          count: outcome.alreadyProcessed,
        }),
      };
    }
    return { tone: "info", text: t("processor.pipelines.run.empty") };
  }
  ...
  async function reportRun(policyId: string) {
    ...
    if (failed) {
      setRunResult({
        tone: "danger",
        text: t("processor.pipelines.run.failed", {
          error: failed.error ?? "",
        }),
      });
      ...
      setRunResult({
        tone: "warning",
        text: t("processor.pipelines.run.timeout"),
      });
    } else if (finals.every((r) => r?.status === "COMPLETED")) {
      setRunResult({
        tone: "success",
        text: t("processor.pipelines.run.completed", { count: finals.length }),
      });
    } else {
      setRunResult({
        tone: "info",
        text: t("processor.pipelines.run.running"),
      });
    }
  }
  ...
  if (aiAvailabilityLoading || (isEdit && !seeded)) {
    return (
      <div className="processor-builder__loading">
        <Spinner />
      </div>
    );
  }
  ...
  function triggerSummary(): string {
    ...
    if (isEditorInput)
      return runOn === "export"
        ? t("processor.pipelines.builder.runOnExport", "Every export")
        : t("processor.pipelines.builder.runOnUpload", "Every upload");
    if (input.triggerType === MANUAL)
      return t("processor.pipelines.composer.triggerManual");
    if (input.triggerType === "schedule")
      ...
      return t(
        `processor.pipelines.composer.runsEvery.${input.scheduleUnit.toLowerCase()}`,
        { count: Number(input.scheduleCount) || 1 },
      );
    return t(`processor.pipelines.trigger.${input.triggerType}`, {
  ...
  function stepWarning(step: WorkingToolStep): string | undefined {
    if (isIntegrationStep(step)) {
      if (!stepOperation(step))
        return t("processor.pipelines.builder.chooseOperation");
      if (!integrationStepConfigured(step))
        return t("processor.pipelines.builder.chooseAccount");
      return undefined;
    }
    if (stepNeedsConfiguring(step, allTools))
      return t("processor.pipelines.builder.needsConfiguring");
    return undefined;
  }
  ...
  function stepDetail(step: WorkingToolStep): string | undefined {
    if (step.support === "unsupported")
      return t("processor.pipelines.builder.usesDefaults");
    if (step.support === "unknown")
      return t("processor.pipelines.builder.unknownStep");
    return undefined;
  }
  ...
  function inspectorBody() {
    if (selected === "input") {
      ...
      return (
        <>
          {hasSources && (
            <>
              <FormField label={t("processor.pipelines.builder.inputSource")}>
                <div className="processor-builder__input-row">
                  <div className="processor-builder__input-field">
                    <Select
                      inputSize="sm"
                      aria-label={t("processor.pipelines.builder.inputSource")}
                      placeholder={t(
                        "processor.pipelines.builder.chooseSource",
                      )}
    ...
    if (selected === "output" && isEditorInput) {
      return (
        <p className="processor-builder__muted">
          {t(
            "processor.pipelines.builder.editorDestinationHelp",
  ...
  return (
    <div className="processor-builder">
      ...
      {hasUnconfiguredSteps && (
        <Banner
          tone="warning"
          description={t("processor.pipelines.builder.stepsNeedSetup", {
            tools: unconfiguredStepLabels.join(", "),
          })}
        />
      )}
      {hasIncompatibleSteps && (
        <Banner
          tone="danger"
          description={t("processor.pipelines.builder.stepsIncompatible", {
            tools: blockingSteps.join(", "),
          })}
        />
      )}

      <div className="processor-builder__grid">
        <div className="processor-builder__canvas">
            ...
            input={
              inputAsked || inputValid
                ? {
                    label:
                      chosenSource?.name ??
                      t("processor.pipelines.builder.chooseSource"),
                    detail: chosenSource ? triggerSummary() : undefined,
                    warning: inputValid
                      ? undefined
                      : t("processor.pipelines.builder.needsSource"),
                  }
                : null
            }
            output={
              isEditorInput
                ? {
                    label: t(
                      "processor.pipelines.builder.editorDestination",
                      "Editor",
                    ),
                    detail: t(
                      "processor.pipelines.builder.editorDestinationDetail",
                ...
                : outputAsked || outputValid
                  ? {
                      label:
                        ...
                        t("processor.pipelines.builder.chooseDestination"),
                      ...
                      detail:
                        routingRules.length > 0
                          ? t(
                              "processor.pipelines.builder.routing.nodeSummary",
                              {
                      ...
                      warning: outputValid
                        ? undefined
                        : t("processor.pipelines.builder.needsDestination"),
                    }
                  : null
            }
            steps={graphSteps}
        ...
        <PipelineInspector
          title={
            selected === "input"
              ? t("processor.pipelines.builder.inputs")
              : selected === "output"
                ? t("processor.pipelines.composer.output")
                : selectedStep
          ...
          message={
            chosenSteps.length > 1
              ? t("processor.pipelines.inspector.multipleSelected", {
      ...
      <Modal
        ...
        width="md"
        title={t("processor.pipelines.composer.addTool")}
        className="processor-pipelines__picker-modal"
      >
        <ToolPicker
          ...
          unavailableReason={(tool) =>
            isClassifyTool(tool) && !aiClassificationEnabled
              ? t(
                  "processor.pipelines.builder.routing.aiToolDisabled",
      ...
      <Modal
        ...
        width="sm"
        title={t("processor.pipelines.delete.title")}
        footer={
          <div className="processor-builder__composer-footer">
            <Button
              variant="tertiary"
              size="sm"
              disabled={deleting}
            ...
            >
              {t("processor.pipelines.delete.cancel")}
            </Button>
            <Button
              size="sm"
              accent="danger"
            ...
            >
              {t("processor.pipelines.delete.confirm")}
            </Button>
          </div>
        }
      >
        <p>{t("processor.pipelines.delete.body", { name: name || "" })}</p>
      </Modal>
      ...
      <Modal
        ...
        width="sm"
        title={t("processor.pipelines.builder.unsavedTitle")}
        footer={
          <div className="processor-builder__composer-footer">
            <Button
              variant="tertiary"
            ...
            >
              {t("processor.pipelines.builder.keepEditing")}
            </Button>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              >
                {t("processor.pipelines.builder.discard")}
              </Button>
              <Button
                size="sm"
                loading={submitting}
      ...
      >
        <p>{t("processor.pipelines.builder.unsavedBody")}</p>
      </Modal>
