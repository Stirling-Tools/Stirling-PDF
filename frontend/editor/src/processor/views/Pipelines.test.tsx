...
import { parseSimplePolicy } from "@processor/api/policies";
...
vi.mock("@processor/queries/policies", () => ({
...
describe("Pipelines view", () => {
  ...
  it("opens template settings directly without the usage details modal", async () => {
    ...
    expect(
      screen.queryByText("processor.policies.detail.recentActivity"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("processor.policies.stats.docsEnforced"),
    ).not.toBeInTheDocument();
  });
  ...
  it("opens settings directly for an existing template setup link", async () => {
    ...
    fireEvent.click(
      screen.getByText("processor.policies.wizard.actions.cancel"),
    );
    await waitFor(() =>
  ...
  it("prevents a non-manager from saving template settings", async () => {
    ...
    const save = await screen.findByRole("button", {
      name: "processor.policies.wizard.actions.saveChanges",
    });
