...
function SectionHead({ title, sub }: { title: string; sub: string }) {
  return (
    <header className="processor-editor__section-head">
      <h2 className="processor-editor__section-title">{title}</h2>
      <p className="processor-editor__section-sub">{sub}</p>
    </header>
  );
}
...
export function EditorAdmin() {
  ...
  return (
    <div className="processor-editor">
      <header className="processor-editor__head">
        <div>
          <h1 className="processor-editor__title">
            {t("processor.editorAdmin.title")}
          </h1>
          <p className="processor-editor__sub">
            {t("processor.editorAdmin.subtitle")}
          </p>
        </div>
      </header>
      ...
      <section>
        <SectionHead
          title={t("processor.editorAdmin.sections.targets.title")}
          sub={t("processor.editorAdmin.sections.targets.sub")}
        />
        {isLoading || !data ? (
          <div className="processor-editor__targets">
      ...
      <section>
        <SectionHead
          title={t("processor.editorAdmin.sections.pairing.title")}
          sub={t("processor.editorAdmin.sections.pairing.sub")}
        />
        {data && (
      ...
      <section>
        <SectionHead
          title={t("processor.editorAdmin.sections.health.title")}
          sub={t("processor.editorAdmin.sections.health.sub")}
        />
        {data && <InstanceHealthTable instances={data.instances} />}
      </section>

      <section className="processor-editor__ops">
        {data && (
