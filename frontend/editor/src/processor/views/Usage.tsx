...
} from "@processor/api/http";
import "@processor/views/Usage.css";
    ...
    fetchWallet()
      ...
      .catch((e) => {
        ...
        if (e instanceof SaasNotLinkedError) {
          ...
          setError(
            t(
              "processor.usage.error.walletUnavailable",
    ...
    <BillingScreen
      ...
      notices={
        <>
          {procurement.loadError && (
            <Banner
              tone="danger"
              title={t("processor.procurement.error.title")}
              action={
      ...
      activateLabel={
        bundleFlow.status === "invoice"
          ? t(
              "processor.billing.freePlan.payInvoice",
              "Pay invoice to complete",
            )
