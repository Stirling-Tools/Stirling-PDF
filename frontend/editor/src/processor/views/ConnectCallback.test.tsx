...
vi.mock("@processor/api/link", () => ({ completeConnect, startConnect }));
...
import ConnectCallback from "@processor/views/ConnectCallback";
...
describe("account-link callback", () => {
  ...
  it("lands on the processor rather than leaving the result on a bare page", async () => {
    landOn(`#type=link&nonce=${NONCE}&access_token=at&refresh_token=rt`);

    const { getByTestId } = renderFlow();

    await waitFor(() => expect(getByTestId("processor")).toBeTruthy());
  });
  ...
  it("hands the result to the dialog rather than rendering its own", async () => {
    ...
    expect(container.querySelector(".processor-connect-callback")).toBeNull();
  });
