...
import type { Policy, TriggerOutcome } from "@processor/api/pipelines";
...
) => baseRender(ui, { wrapper: ProcessorTestProviders, ...options });
...
vi.mock("@processor/hooks/useAiEngineEnabled", () => ({
  useAiEngineEnabled: () => ({
    enabled: true,
...
vi.mock("@processor/api/pipelineAssets", () => ({
...
vi.mock("@processor/api/policies", () => ({
...
vi.mock("@processor/components/pipelines/DestinationPicker", () => ({
  DestinationPicker: ({
    value,
    onChange,
    onCreateNew,
...
vi.mock("@processor/components/sources/SourceModal", () => ({
  SourceModal: ({
    open,
    sourceId,
  }: {
...
describe("PipelineBuilder", () => {
  ...
  function endOpener(end: "input" | "output"): Promise<HTMLElement> {
    return waitFor(() => {
      const placeholder = screen.queryByText(
        `processor.pipelines.graph.add.${end}`,
      );
  ...
  async function pickInputSource(sourceName: string) {
    ...
    fireEvent.click(
      await screen.findByRole("textbox", {
        name: "processor.pipelines.builder.inputSource",
      }),
    );
  ...
  async function addTool(toolName: string) {
    const placeholder = screen.queryByText(
      "processor.pipelines.graph.addFirstTool",
    );
    if (placeholder) {
  ...
  async function openTray() {
    fireEvent.click(
      await screen.findByLabelText("processor.pipelines.builder.moreActions"),
    );
  }
  ...
  it("greets a new pipeline with places to fill, not problems to fix", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.graph.add.input"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("processor.pipelines.graph.add.output"),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("processor.pipelines.builder.needsSource"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("processor.pipelines.builder.needsDestination"),
    ).not.toBeInTheDocument();
    // Nothing is on the chain, so there is nothing to remove either.
    expect(
      screen.queryByLabelText(/processor.pipelines.graph.removeNode/),
    ).not.toBeInTheDocument();
  });
  ...
  it("warns on a step whose tool cannot run on its defaults", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.builder.needsConfiguring"),
    ).toBeInTheDocument();
    expect(
  ...
  it("stays quiet on the wires around a step that still needs setting up", async () => {
    ...
    expect(
      screen.queryByText(
        "processor.pipelines.builder.diagnostic.undeclared-operation",
      ),
    ).not.toBeInTheDocument();
  });
  ...
  it("clears the warning once the step is configured", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText("processor.pipelines.builder.needsConfiguring"),
      ).not.toBeInTheDocument(),
    );
  });
  ...
  it("only asks for a source once the user has asked for the node", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.builder.needsSource"),
    ).toBeInTheDocument();
  ...
  it("puts an end back to a placeholder when it is removed", async () => {
    ...
    fireEvent.click(
      screen.getByLabelText("processor.pipelines.graph.removeNode"),
    );
  ...
  it("edits a node's settings only once it is selected", async () => {
    ...
    expect(
      screen.getByText("processor.pipelines.inspector.noSelectionTitle"),
    ).toBeInTheDocument();
  ...
  it("blocks saving a chain whose steps can't run on each other", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.builder.stepsIncompatible"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("processor.pipelines.composer.create").closest("button"),
    ).toBeDisabled();
  ...
  it("says why on the wire arriving at the step that cannot run", async () => {
    ...
    const note = await screen.findByText(
      /processor\.pipelines\.builder\.diagnostic\./,
    );
    expect(note.closest(".processor-graph-edge")).toHaveClass("is-blocking");
  });
  ...
  it("allows a chain whose steps line up", async () => {
    ...
    expect(
      screen.queryByText("processor.pipelines.builder.stepsIncompatible"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText("processor.pipelines.composer.create").closest("button"),
    ).not.toBeDisabled();
  ...
  it("creates and edits sources in place through the modal", async () => {
    ...
    fireEvent.click(
      screen.getByText("processor.sources.actions.connectSource"),
    );
    ...
    fireEvent.click(
      screen.getByLabelText("processor.pipelines.composer.editSource"),
    );
  ...
  it("makes a destination created from the picker the pipeline's output", async () => {
    ...
    expect(
      screen.getByText("processor.pipelines.builder.chooseSource"),
    ).toBeInTheDocument();
  });
  ...
  it("saves an editor pipeline as its own flag, not as a wire input", async () => {
    ...
    renderBuilder("/processor/pipelines/new");
    fireEvent.change(
      await screen.findByLabelText("processor.pipelines.composer.name"),
  ...
  it("carries a saved pipeline's routes through an unrelated edit", async () => {
    ...
    fireEvent.click(
      await screen.findByLabelText("processor.pipelines.builder.rename"),
    );
    fireEvent.change(
      await screen.findByLabelText("processor.pipelines.composer.name"),
      { target: { value: "Renamed" } },
    );
    fireEvent.click(screen.getByText("processor.pipelines.composer.save"));
  ...
  it("runs an existing pipeline and reports success", async () => {
    ...
    fireEvent.click(await screen.findByText("processor.pipelines.detail.run"));

    await waitFor(() => expect(triggerPipeline).toHaveBeenCalledWith("plc-1"));
    expect(
      await screen.findByText("processor.pipelines.run.completed"),
    ).toBeInTheDocument();
  });
  ...
  it("explains an empty trigger when files are parked by a failed run", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.run.parked"),
    ).toBeInTheDocument();
  });
  ...
  it("explains an empty trigger when everything is already processed", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.run.allProcessed"),
    ).toBeInTheDocument();
  });
  ...
  it("keeps a step's stored file binding on save when the tool can't be probed", async () => {
    ...
    fireEvent.click(
      await screen.findByText("processor.pipelines.composer.save"),
    );
  ...
  it("blocks saving an integration step with no account chosen", async () => {
    ...
    await addTool("processor.policies.operations.discordNotify.label");

    // Operation chosen, account not: still not saveable.
    expect(
      await screen.findByText("processor.pipelines.builder.stepsNeedSetup"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("processor.pipelines.composer.create").closest("button"),
    ).toBeDisabled();
  ...
  it("deletes an existing pipeline after confirmation", async () => {
    ...
    await openTray();
    fireEvent.click(
      await screen.findByText("processor.pipelines.detail.delete"),
    );
    fireEvent.click(
      await screen.findByText("processor.pipelines.delete.confirm"),
    );
  ...
  it("pauses a live pipeline at once, in place, and re-saves the persisted policy", async () => {
    ...
    fireEvent.click(
      await screen.findByText("processor.pipelines.builder.pause"),
    );
    ...
    expect(
      await screen.findByText("processor.pipelines.builder.activate"),
    ).toBeInTheDocument();
    expect(screen.queryByText("pipelines list")).not.toBeInTheDocument();
  });
  ...
  it("prompts to save or discard when leaving with unsaved edits", async () => {
    ...
    expect(
      await screen.findByText("processor.pipelines.builder.unsavedTitle"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByText("processor.pipelines.builder.discard"));
    expect(await screen.findByText("pipelines list")).toBeInTheDocument();
  });
  ...
  it("keeps the account chosen for an integration step", async () => {
    ...
    fireEvent.click(
      await screen.findByPlaceholderText(
        "processor.connections.picker.placeholder",
      ),
    );
