...
const Redact = (props: BaseToolProps) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        lastRequestedMode.current !== null &&
        lastRequestedMode.current !== "manual"
      )
        return;
  ...
  const handleModeChange = (mode: RedactMode) => {
    console.log(
      `[Redact] Mode switch requested: ${base.params.parameters.mode} -> ${mode}`,
    );
    ...
    if (hasAnyFiles) {
      console.log(
        "[Redact] Updating redaction config and navigating to viewer",
      );
  ...
  const buildSteps = () => {
    ...
    if (base.params.parameters.mode === "automatic") {
      ...
      steps.push({
        title: t(
          "redact.searchAndRedact.controlsTitle",
          "Search & Redact Controls",
        ),
        isCollapsed: false,
        onCollapsedClick: () => {},
