 ...
 * Encryption and plain AcroForm fields ARE preserved by SaveAsCopy, so they
 * are deliberately not flagged - warning on them would be crying wolf.
 */
...
export function detectSaveRisks(doc: EditorDocument): SaveRisks {
  ...
  return { signatures, xfaForm };
}

export function hasSaveRisks(r: SaveRisks): boolean {
  return r.signatures > 0 || r.xfaForm;
}
