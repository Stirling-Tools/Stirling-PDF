...
const h = vi.hoisted(() => ({
  toolRoute: vi.fn(() => "/compress"),
  editorHomeRoute: vi.fn(() => null),
}));

vi.mock("@app/utils/urlRouting", () => ({
  parseToolRoute: () => ({ workbench: "fileEditor", toolId: null }),
  toolRoute: h.toolRoute,
  editorHomeRoute: h.editorHomeRoute,
}));
...
/** The hook navigates through the router, so it needs one to render under. */
...
describe("useNavigationUrlSync — startup-view selections", () => {
  beforeEach(() => h.toolRoute.mockClear());
  ...
  it("never writes the URL for the startup-applied tool", () => {
    ...
    expect(h.toolRoute).not.toHaveBeenCalled();
  });
  ...
  it("still writes the URL when the user picks a different tool", () => {
    ...
    expect(h.toolRoute).toHaveBeenCalledWith(
      "compress",
      registry,
      expect.anything(),
    );
