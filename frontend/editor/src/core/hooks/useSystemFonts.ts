...
async function getApiBaseUrl(): Promise<string> {
  ...
  if (tauriMode) {
    try {
      console.log(
        "[useSystemFonts] Tauri detected, importing backend service...",
      );
      const { tauriBackendService } =
        await import("../../desktop/services/tauriBackendService");

      if (tauriBackendService) {
        const backendUrl = tauriBackendService.getBackendUrl();
        if (backendUrl) {
          console.log(
            "[useSystemFonts] ✅ Using Tauri backend URL:",
            backendUrl,
          );
          return backendUrl;
        }
  ...
  if (typeof window !== "undefined") {
    ...
    if (!isViteWebDev) {
      console.log(
        "[useSystemFonts] Not in Vite web dev mode, trying localhost:8080...",
      );
  ...
  const fetchFonts = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[useSystemFonts] Failed to refresh backend cache:",
          refreshErr,
        );
      }
      ...
      console.log(
        `[useSystemFonts] Raw API response (${fontNames.length} fonts):`,
        fontNames,
      );
      ...
      if (fontNames.length === 0) {
        console.warn(
          "[useSystemFonts] API returned empty font list, using defaults",
        );
        ...
        console.log(
          `[useSystemFonts] ✅ Successfully loaded ${converted.length} system fonts`,
        );
        console.log(
          "[useSystemFonts] First 10 fonts:",
          converted.slice(0, 10).map((f) => f.label),
        );
      }
    } catch (err) {
