...
async function editorState(page: Page) {
  return page.evaluate(() => {
    const s = (window as unknown as EditorWindow).__v2_editor_store;
    if (!s) return null;
...
async function loadContext(page: Page) {
  return page.evaluate(() => {
    const s = (window as unknown as EditorWindow).__v2_editor_store;
    const idb = (
      window as unknown as { __v2_file_debug?: Record<string, unknown> }
    ).__v2_file_debug;
    return {
      hasDocument: s?.state.hasDocument ?? null,
      loading: s?.state.loading ?? null,
      error: s?.state.error ?? null,
      stage: !!document.querySelector('[data-testid="v2-stage"]'),
      pages: document.querySelectorAll('[data-testid^="v2-page-"]').length,
...
test.describe("v2 editor - it never gets stuck with no document", () => {
  test("the canvas dropping its document does not strand the editor", async ({
    ...
    for (let round = 1; round <= 3; round += 1) {
      await page.evaluate(() => {
        (window as unknown as EditorWindow).__v2_editor_store?.clearDocument();
      });
      expect(
  ...
  test("deselecting and reselecting the file leaves a document open", async ({
    ...
    // must not pin it back (see pdf-text-editor-v2-workbench-files). So the
