...
// Regressions for two selection bugs on the v2 stage:
...
// `[data-testid^="v2-image-"]` also matches the hidden file input; image
// overlays are always `v2-image-p<page>-<obj>`.
const IMG_SEL = '[data-testid^="v2-image-p"]';
const RUN_SEL = '[data-testid^="v2-run-p0-"]';
...
function selectedImageIds(page: Page): Promise<string[]> {
  return page.evaluate(
    () =>
      (window as unknown as V2Win).__v2_editor_store.selection.value.imageIds,
  );
}
...
async function pageBox(page: Page): Promise<Rect> {
  const b = await page.getByTestId("v2-page-0").boundingBox();
  if (!b) throw new Error("page 0 has no bounding box");
  return b;
}

/**
 * Two points in the bare gutter LEFT of the page but still inside `v2-pages`
 * (the stage's clear-on-press target, and the only region the marquee arms in).
 * Outside `v2-pages` neither handler runs and any assertion would pass blind.
 */
async function gutter(page: Page): Promise<{
  from: { x: number; y: number };
  to: { x: number; y: number };
}> {
  const pages = await page.getByTestId("v2-pages").boundingBox();
  const p0 = await pageBox(page);
  if (!pages) throw new Error("v2-pages has no bounding box");
...
test.describe("v2 editor - selection survives resize, marquee never wipes blind", () => {
  ...
  test("a plain marquee replaces the selection with what it caught", async ({
    ...
    const target = await page.evaluate(() => {
      ...
      const id = (el: HTMLElement) =>
        el.dataset.testid!.replace(/^v2-run-/, "");
    ...
    await page.evaluate(
      (id: string) =>
        (window as unknown as V2Win).__v2_editor_store.selection.selectOne(id),
      target.last,
    );
    ...
    const ids = await page
      ...
      .evaluateAll((els) =>
        els
          .slice(0, 3)
          .map((el) =>
            (el as HTMLElement).dataset.testid!.replace(/^v2-run-/, ""),
          ),
      );
    expect(ids.length, "fixture needs at least 3 runs").toBe(3);

    const result = await page.evaluate((rids: string[]) => {
      const sel = (window as unknown as V2Win).__v2_editor_store.selection;
