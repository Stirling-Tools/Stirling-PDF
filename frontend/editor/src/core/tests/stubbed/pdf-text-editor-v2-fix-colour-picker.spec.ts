...
// The fill colour picker (ColorInput, testid v2-colour) had three faults that
...
window.__ink = function (runId) {
  var el = document.querySelector('[data-testid="v2-run-' + runId + '"]');
  if (!el) return null;
  var pg = el.closest("[data-testid^='v2-page-']");
...
function selectedRunIds(
  ...
  return page.evaluate(
    () =>
      ...
      ).__v2_editor_store.selection.value.runIds,
  );
}
...
function topmostAt(
  ...
  return page.evaluate((id) => {
    const el = document.querySelector(`[data-testid="v2-run-${id}"]`);
...
async function selectRun(page: import("@playwright/test").Page, runId: string) {
  ...
  await expect(page.getByTestId("v2-colour")).toBeEnabled();
}
...
test.describe("v2 editor - fill colour picker", () => {
  ...
  test("the picker does not sit over the page after a pick", async ({
    ...
    await page
      .locator(`[data-testid="v2-run-${HEADING}"]`)
      .click({ timeout: 5_000 });
  ...
  test("a dropdown pick still unifies a mixed selection", async ({ page }) => {
    ...
    await page.locator(`[data-testid="v2-run-${HEADING}"]`).click();
    await page
      .locator(`[data-testid="v2-run-${BODY}"]`)
    ...
    expect(await page.getByTestId("v2-colour").inputValue()).toBe("#000000");

    // Open the picker and pick out of the saturation square itself.
    await page.getByTestId("v2-colour").click();
    const overlay = page
  ...
  test("Escape closes the colour dropdown", async ({ page }) => {
    ...
    await page.getByTestId("v2-colour").press("Escape");
    await expect(
      overlay,
