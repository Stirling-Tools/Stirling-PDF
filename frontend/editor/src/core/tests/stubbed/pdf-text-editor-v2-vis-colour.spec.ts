...
async function setOutlineColour(
  ...
  const oc = page.getByTestId("v2-outline-colour");
  await oc.fill(hex);
...
async function setOutlineWidth(
  ...
  const w = page.getByTestId("v2-outline-width");
  await w.fill(width);
  await w.press("Enter");
  await page.waitForTimeout(400);
}
...
test.describe("v2 editor - colour on the page bitmap", () => {
  ...
  test("undo repaints the original ink, not just the model fill", async ({
    ...
    const undo = page.getByTestId("v2-undo");
