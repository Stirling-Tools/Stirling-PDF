...
test.describe("touch pinch", () => {
  ...
  // Mobile emulation (isMobile) is unsupported in Firefox and the touch pinch
  // below drives CDP input events: both are chromium-only. The skip lives here
  // (not in the test body) so no context is created on other browsers.
  test.skip(
