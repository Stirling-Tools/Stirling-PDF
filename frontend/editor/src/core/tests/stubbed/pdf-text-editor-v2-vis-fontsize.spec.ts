...
function measureInk(arg: {
  ...
  const canvas = document.querySelector<HTMLCanvasElement>(
    `[data-testid="v2-page-${arg.pageIndex}"] canvas`,
  );
...
async function selectRun(
  ...
  await expect(page.getByTestId("v2-font-size")).toBeEnabled();
}
...
async function setSize(
  ...
  const input = page.getByTestId("v2-font-size");
...
test.describe("v2 editor - font size, measured on the rendered page", () => {
  ...
  test("doubling the font size doubles the glyph ink on the page bitmap", async ({
    ...
    await selectRun(page, "v2-run-p0-t0");
  ...
  test("the run overlay box grows in step with the ink it covers", async ({
    ...
    const boxBefore = await page.evaluate(overlayInCanvasPx, "v2-run-p0-t0");
    ...
    const boxAfter = await page.evaluate(overlayInCanvasPx, "v2-run-p0-t0");
...
test.describe("v2 editor - font family, measured on the rendered page", () => {
