...
async function selectFirstRun(page: import("@playwright/test").Page) {
  ...
  return (await run.getAttribute("data-testid"))!.replace("v2-run-", "");
}
...
test.describe("v2 editor - the weight control is gone", () => {
...
test.describe("v2 editor - one colour picker by default", () => {
  test("outline colour and width are not in the toolbar", async ({ page }) => {
    await openV2(page, PARAGRAPH_PDF);
    await selectFirstRun(page);

    await expect(page.getByTestId("v2-colour")).toBeVisible();
    await expect(
      page.getByTestId("v2-outline-colour"),
      "outline colour belongs behind the advanced control",
    ).toHaveCount(0);
    await expect(page.getByTestId("v2-outline-width")).toHaveCount(0);
    await expect(page.getByTestId("v2-colour-advanced")).toBeVisible();
  });
