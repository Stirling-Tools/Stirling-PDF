...
async function saveAndReopen(
  ...
  await page.evaluate(() => {
    const w = window as unknown as {
      __v2_editor_store?: { document: unknown };
      __v2_prev_document?: unknown;
    };
    w.__v2_prev_document = w.__v2_editor_store?.document;
  });
  await page.locator('[data-testid="v2-file-input"]').setInputFiles(saved);
  await page.waitForFunction(
    () => {
      ...
      if (!s?.document || s.document === w.__v2_prev_document) return false;
...
test.describe("v2 editor - a second edit is as safe as the first", () => {
  for (const c of CASES) {
    test(c.name, async ({ page }) => {
      test.setTimeout(240_000);
      await page.goto("/pdf-text-editor", { waitUntil: "domcontentloaded" });
      await expect(page.getByTestId("v2-root")).toBeVisible({
        timeout: 15_000,
      });
      await page
