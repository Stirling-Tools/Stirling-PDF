...
async function addTextAt(
  ...
  const runs = page.locator(`[data-testid^="v2-run-p${pageIndex}-"]`);
  ...
  await page.getByTestId(`v2-page-${pageIndex}`).click({ position: { x, y } });
...
async function newestRunTestId(
  ...
  const locator = page.locator(`[data-testid^="v2-run-p${pageIndex}-new-"]`);
...
test.describe("PDF text editor v2 - add text, checked in the bitmap", () => {
  ...
  test("add-text mode ends after one insert - a second page click adds no more ink", async ({
    ...
    await expect(page.getByTestId("v2-add-text")).toHaveText(/add text/i);
    ...
    await page.getByTestId("v2-page-0").click({ position: { x: 150, y: 400 } });
  ...
  test("add-text on a scrolled-to later page inks that page and leaves page one alone", async ({
    ...
    await page.getByTestId("v2-page-2").scrollIntoViewIfNeeded();
    await expect(page.getByTestId("v2-page-2")).toBeVisible();
    await page.waitForTimeout(2500);
    const box = await page.getByTestId("v2-page-2").boundingBox();
