...
async function boxOf(page: Page, src: string) {
  ...
  const locator = page.locator(`[data-testid="v2-run-${id}"]`);
  const box = await locator.boundingBox();
  if (!box) throw new Error(`run /${src}/ has no box`);
  return box;
}

test.describe("v2 editor - edge gestures", () => {
  ...
  test("the insert verbs live in the panel, not the canvas strip", async ({
    ...
    await expect(panel.getByTestId("v2-add-text")).toBeVisible();
    await expect(panel.getByTestId("v2-add-image")).toBeVisible();
    await expect(
      page.getByTestId("v2-toolbar").getByTestId("v2-add-text"),
    ).toHaveCount(0);
  });
});
