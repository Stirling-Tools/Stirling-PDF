...
// synthesises bridging whitespace, which exercises the partialEdit
// ghost-char mapping. Used to guard against "edit a single char and
// the whole line flips to Helvetica" regressions.
const LMROMAN_PDF = path.join(
  __dirname,
  "../test-fixtures/lmroman-assignment.pdf",
);
...
test.describe("PDF text editor v2 - whitespace preservation", () => {
  ...
  test("lmroman-assignment.pdf: typing into a non-base14 line preserves the original font", async ({
    page,
  }) => {
    // Repro: in `dbg-exercise3.pdf` the body bullet "Submissions must
    // be made through Moodle..." uses LMRoman12 (Latin Modern Roman
    // 12pt, an embedded non-base14 serif). LineGrouper merges its
    // sub-objects and synthesises bridging whitespace between them.
    // Before the partialEdit ghost-char fix, the synthesised whitespace
    // chars made `sum(mergedFromTexts) != prevText.length`, so
    // planPartialEdit bailed and the overlay path flipped the entire
    // run to base14:Helvetica - the user saw an LMRoman line suddenly
    // become Helvetica after typing one char.
    await gotoV2(page);
    await page
      .locator('[data-testid="v2-file-input"]')
    ...
    // Find the LMRoman bullet line (uses /Submissions.*Moodle/ as a
    // stable substring that identifies this specific test fixture).
    const target = await page.evaluate(() => {
      const store = (
        window as unknown as {
          __v2_editor_store: {
      ...
      for (const r of runs) {
        if (/Submissions.*Moodle/.test(r.text)) {
    ...
    if (!target) {
      test.skip(true, "lmroman-assignment.pdf missing the Submissions bullet");
      return;
    }
    // Sanity: confirm the source font is non-base14 (the case the bug
    // bit on - base14 lines have their own preservation path).
    expect(target.fontId).not.toMatch(/^base14:/);
    const originalFontId = target.fontId;

    // Type one character at the end of the line.
    await page.evaluate((tid) => {
      const el = document.querySelector<HTMLDivElement>(
    ...
    // After the edit the run's fontId MUST stay on the original
    // LMRoman font - not flip to base14:Helvetica.
    ...
    expect(afterFontId).toBe(originalFontId);
  });

  test("overlay box covers the full line height for tight-bounds runs", async ({
    page,
  }) => {
    // PDFium-reported bounds.height often collapses to the visible
    // glyph extent (~3pt for a 12pt line of lowercase body text). The
    // overlay's hit area and visible mask should cover the full line
    // anyway - if it sits as a thin strip at the baseline, clicking
    // the obvious text doesn't focus it and the contrasting mask
    // doesn't hide the underlying bitmap during edit.
    await gotoV2(page);
    await page
      .locator('[data-testid="v2-file-input"]')
      .setInputFiles(LMROMAN_PDF);
    ...
    const info = await page.evaluate(() => {
      ...
      for (const el of els) {
        ...
        const modelRun = store.doc.page(0).runs.find((r) => r.id === id);
        if (
          modelRun &&
          modelRun.bounds.height < modelRun.fontSize * 0.5 &&
          modelRun.fontSize >= 8
        ) {
          return {
            rectHeight: rect.height,
            modelBoundsHeight: modelRun.bounds.height,
            modelFontSize: modelRun.fontSize,
          };
        }
      }
      return null;
    ...
    if (!info) {
      test.skip(true, "No tight-bounds run found in lmroman-assignment.pdf");
      return;
    }
    // The overlay's CSS height should be AT LEAST a font-size worth of
    // pixels, not the tiny modelBoundsHeight. (We don't pin an exact
    // ratio because page scale is dynamic.)
    expect(info.rectHeight).toBeGreaterThan(info.modelBoundsHeight * 2);
  });
