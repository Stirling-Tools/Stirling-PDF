...
for (const path of ["/editor", "/settings/account", "/processor/policies"]) {
...
test("shows cookie consent on a cold Processor entry and remembers the choice in Editor", async ({
  page,
  context,
}) => {
  ...
  await page.goto("/processor/policies", { waitUntil: "domcontentloaded" });
  const banner = page.locator("#cc-main .cm");
  await expect(banner).toBeVisible();
  await banner.getByRole("button", { name: "No Thanks", exact: true }).click();
