...
import type {
  SearchRedactOptions,
  SearchTextResult,
} from "@app/contexts/RedactionContext";
...
function RedactionAPIBridgeInner({ documentId }: { documentId: string }) {
  ...
  const handleSearchText = useCallback(
    async (
      ...
      const searchResult = await searchProvides
        .searchAllPages(text)
        .toPromise();
      ...
      const foundOnPages = [
        ...new Set(results.map((r: { pageIndex: number }) => r.pageIndex + 1)),
  ...
  const handleRedactText = useCallback(
    async (_text: string, _options?: SearchRedactOptions): Promise<boolean> => {
      if (!annotationScope) {
