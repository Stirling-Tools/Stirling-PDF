...
export interface OcrRuntimePanelProps {
  /** Skip the initial fetch until the panel is actually on screen. */
  active?: boolean;
  /** Lets a language picker refresh itself once the installed set has changed. */
 ...
 *
 * Engine and languages are two separate actions on purpose. The engine is a
 * one-off ~37 MB download that needs a restart before the tool becomes
 * available; languages are small, changeable at any time, and take effect
 * immediately because the backend re-reads them from disk on every request.
 * Presenting them as one step would imply that adding Catalan needs a restart
 * too.
 *
 * Lives in shared/ rather than under the OCR tool because Settings shows the
 * same panel inline while the tool shows it in a dialog.
 */
const OcrRuntimePanel: React.FC<OcrRuntimePanelProps> = ({
  active = true,
  onLanguagesChanged,
}) => {
  ...
  const catalogue = useMemo(() => {
    return Object.entries(status?.availableLanguages ?? {})
      .map(([code, artifact]) => ({
        code,
        size: artifact.size,
  ...
  const pending = useMemo(
    () => ({
      toInstall: [...selected].filter((code) => !installed.has(code)),
      toRemove: [...installed].filter((code) => !selected.has(code)),
    }),
    [selected, installed],
  );
  ...
  const onInstallEngine = async () => {
    ...
    try {
      ...
      if (!result.installed) {
        setError(
          result.error ??
            t("ocr.runtime.engineFailed", "Could not install the OCR engine."),
        );
        return;
      }
  ...
  const onApplyLanguages = async () => {
    ...
    setError(null);
    try {
      const result = await changeOcrLanguages(
        pending.toInstall,
        pending.toRemove,
      );
  ...
  return (
    <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              {catalogue.map(({ code, label, size }) => (
                <Checkbox
                  ...
                  // English is what Tesseract falls back to; removing it leaves
                  // an engine installed and refusing every job.
      ...
      {/*
        The heading is translated; the backend string underneath is diagnostic
        detail ("SHA-256 mismatch", "HTTP 403"), which is worth showing but is
...
export default OcrRuntimePanel;
