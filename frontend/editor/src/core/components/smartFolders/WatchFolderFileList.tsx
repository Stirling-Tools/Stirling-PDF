    ...
    <Box
      ...
      style={{
        ...
        backgroundColor: hovered
          ? `${folder.accentColor}28`
          : `${folder.accentColor}14`,
...
function FileRow({
  ...
  const isInCurrentFolder =
    currentFolderId !== null && memberFolderIds.includes(currentFolderId);
  ...
  return (
    <Box
      ...
      onMouseMove={(e) => {
        if (!mouseDownPos.current || !rowRef.current || isInCurrentFolder)
          return;
      ...
      style={{
        ...
        backgroundColor: isSelected
          ...
          : hovered && !isInCurrentFolder
            ...
            : "transparent",
        transition:
          "background-color 0.1s ease, border-left-color 0.15s ease, opacity 0.15s ease",
        minWidth: 0,
        userSelect: "none",
      }}
    >
      ...
      >
        ...
        {/* Preview button */}
        <Tooltip
          label={t("smartFolders.fileList.preview", "Preview")}
          withArrow
        >
          <ActionIcon
            data-no-select
            ...
            onClick={(e) => {
              e.stopPropagation();
              onPreview(file.id as FileId, file.name);
            }}
          >
        ...
        {isInCurrentFolder ? null : currentFolderId !== null ? (
          <Tooltip
            label={t("smartFolders.fileList.addToFolder", "Add to this folder")}
            withArrow
          >
            <ActionIcon
              data-no-select
          ...
          >
            ...
            <Popover.Dropdown p="xs" style={{ minWidth: "10rem" }}>
              ...
              <Stack gap={2}>
                {folders
                  .filter((f) => !memberFolderIds.includes(f.id))
                  .map((folder) => {
                    ...
                    return (
                      <Button
                        ...
                        leftSection={
                          ...
                          >
                            <FolderIconComp
                              style={{ fontSize: 9, color: folder.accentColor }}
                            />
                          </Box>
                        }
                        onClick={() => {
                ...
                {folders.filter((f) => !memberFolderIds.includes(f.id))
                  .length === 0 && (
                  <Text size="xs" c="dimmed" ta="center" py="xs">
                    {memberFolderIds.length > 0
                      ? t(
                          "smartFolders.fileList.inAllFolders",
...
export function WatchFolderFileList({
  files,
  folderId,
  onSendToFolder,
  onNavigateToFolder,
}: WatchFolderFileListProps) {
  const { t } = useTranslation();
  ...
  const processedFiles = useMemo(() => {
    ...
    if (filterMode === "unassigned")
      result = result.filter((f) => !membership.has(f.id));
    else if (filterMode !== "all") {
      const inFolder = (f: (typeof files)[number]) =>
        membership.get(f.id)?.includes(filterMode) ?? false;
      result = result.filter((f) =>
        filterNegate ? !inFolder(f) : inFolder(f),
      );
    }
  ...
  const unassigned = processedFiles.filter(
    (f) => !membership.has(f.id) && !outputFileIds.has(f.id),
  );
  const inFolders = processedFiles.filter(
    (f) => membership.has(f.id) && !outputFileIds.has(f.id),
  );
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, file: StirlingFileStub) => {
  ...
  const sortOptions = [
    {
      value: "date-desc",
      label: t("smartFolders.fileList.sort.newest", "Newest first"),
    },
    {
      value: "date-asc",
      label: t("smartFolders.fileList.sort.oldest", "Oldest first"),
    },
    {
      value: "name-asc",
      label: t("smartFolders.fileList.sort.nameAZ", "Name A–Z"),
    },
    {
      value: "name-desc",
      label: t("smartFolders.fileList.sort.nameZA", "Name Z–A"),
    },
    {
      value: "size-desc",
      label: t("smartFolders.fileList.sort.largest", "Largest first"),
    },
    {
      value: "size-asc",
      label: t("smartFolders.fileList.sort.smallest", "Smallest first"),
    },
  ];
  ...
  const filterOptions = [
    ...
    {
      value: "unassigned",
      label: t("smartFolders.fileList.filter.unassigned", "Unassigned"),
    },
    ...allFolders.map((f) => ({ value: f.id, label: f.name })),
  ];

  const hasActiveFilters =
    sortKey !== "date-desc" || filterMode !== "all" || filterNegate;
    ...
    >
      ...
      >
        <Group gap="0.375rem" wrap="nowrap" align="center">
          ...
          />
          <Tooltip
            label={t("smartFolders.fileList.uploadFiles", "Upload PDFs")}
            withArrow
          >
            <ActionIcon
              size="sm"
        ...
        {selectedIds.size >= 2 && (
          ...
          >
            ...
            >
              {selectedIds.size}{" "}
              {t("smartFolders.fileList.selected", "selected")}
            </Text>
            <Popover
            ...
            >
              <Popover.Target>
                <Tooltip
                  label={t(
                    "smartFolders.fileList.addToFolder",
                    "Add to folder",
                  )}
                  withArrow
                >
          ...
          {files.length === 0 ? (
            ...
            >
              ...
              <Button
                size="xs"
                variant="light"
                leftSection={<AddIcon style={{ fontSize: "0.875rem" }} />}
                onClick={handleUploadClick}
              >
            ...
            <Text size="xs" c="dimmed" ta="center" py="md" px="sm">
              {search
                ? t(
                    "smartFolders.fileList.noResults",
                    "No files match your search",
                  )
                : t(
                    "smartFolders.fileList.noneInFilter",
