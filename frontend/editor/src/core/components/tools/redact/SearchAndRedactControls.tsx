 ...
 */
export default function SearchAndRedactControls({
  disabled = false,
}: SearchAndRedactControlsProps) {
  const { t } = useTranslation();
  const { searchText, redactText, setManualRedactColor, clearSearch } =
    useRedaction();
  ...
  const [searchResults, setSearchResults] = useState<SearchTextResult | null>(
    null,
  );
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("redact.searchAndRedact.searchFailed", "Search failed"),
      );
    } finally {
  ...
  const handleRedact = useCallback(async () => {
    ...
    try {
      ...
      if (currentResults.totalCount === 0) {
        setError(
          t(
            "redact.searchAndRedact.noMatchesRedacted",
            "No matches found to redact",
          ),
        );
        return;
      }
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("redact.searchAndRedact.redactFailed", "Redaction failed"),
      );
    } finally {
    ...
    <>
      ...
      <Stack gap="md">
        ...
        <Text size="xs" c="dimmed">
          {t(
            "redact.searchAndRedact.instructions",
            "Enter text to find in the PDF. You can then redact all matches at once.",
          )}
        </Text>
        ...
        <TextInput
          ...
          placeholder={t(
            "redact.searchAndRedact.searchPlaceholder",
        ...
        {hasResults && matchCount > 0 && (
          <Text size="sm" c="teal" fw={500}>
            {t(
              "redact.searchAndRedact.matchesFound",
              "{{count}} matches found on {{pages}} page(s)",
              {
                count: matchCount,
                pages: pageCount,
              },
        ...
        {hasResults &&
          matchCount > 0 &&
          searchResults.foundOnPages.length <= 20 && (
            <Text size="xs" c="dimmed">
