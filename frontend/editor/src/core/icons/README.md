...
**It is a brand mark** — put it in `svg/third-party/`, named after the connector `type`/`kind` the API returns, so `<BrandMark id={type} />` resolves it. Brand colours are detected from the geometry; no config needed. Keep the owner's own viewBox: the generator scales every mark onto the 24 grid's 20-unit ink box, so it matches the stroke icons beside it whatever grid it arrived on.
