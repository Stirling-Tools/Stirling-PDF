...
const BRAND = new Set(ALL.filter((name) => ICONS[name].kind === "brand"));
// Read off disk so the gallery cannot disagree with the folder about which drawings are ours.
const CUSTOM = new Set(
  Object.keys(import.meta.glob("@app/icons/svg/stirling/*.svg")).map((p) =>
    p.replace(/^.*\/(.*)\.svg$/, "$1"),
  ),
);
    ...
    <Gallery
      ...
      note={
        <>
          ...
          <code>&lt;Icon name=&quot;…&quot; /&gt;</code>. Rendered at 24px;
          filter to see a name at all three sizes in <em>In use</em>.
        </>
      }
      names={ALL}
      sizes={[24]}
