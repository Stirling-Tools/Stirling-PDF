...
export function FailureActionButtons({
  ...
  const labelFor = (action: FailureActionOffer) =>
    ...
    t(action.labelKey, {
      defaultValue: t(
        `portal.failures.action.${action.id.toLowerCase()}`,
        action.id,
      ),
    });

  return (
    <div className="portal-failures__actions">
      {renderable.map((action) => {
        ...
        return (
              ...
              action.enabled
                ...
                : `${labelFor(action)} — ${t(
                    action.disabledReasonKey ??
                      "portal.failures.disabled.unavailable",
                    "unavailable",
                  )}`
            }
            title={
          ...
          >
            {isConfirming
              ? t("portal.failures.action.confirm", "Are you sure?")
              : labelFor(action)}
          </Button>
        );
      })}
