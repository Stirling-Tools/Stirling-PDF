...
function PolicySetupWizardBody({
  ...
  // For an egress policy these are share channels, not input sources; empty
  // means every channel, so a new one starts unnarrowed.
  const [sources, setSources] = useState<string[]>(
    policy?.state.sources ?? (isEgress ? [] : ["editor"]),
  );
    ...
    >
      ...
      {step === "settings" && (
        <div className="portal-policies__wizard-section">
          ...
          <div className="portal-policies__fields">
            {!isEgress && sources.includes("editor") && (
              <>
                <FormField
