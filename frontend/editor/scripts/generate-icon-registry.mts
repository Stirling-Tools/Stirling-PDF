...
// oxlint-disable-next-line no-restricted-imports -- build script; no alias covers scripts/
import { inkBounds } from "../src/core/icons/svgInkBounds.ts";

/** A node under construction: the generator rewrites ids and paints before it emits. */
type MutableNode = [
  tag: string,
...
const debug = (m: string) => verbose && console.log(m);
...
const camelAttrs = (attrs: Record<string, string>): Record<string, string> =>
  Object.fromEntries(
...
function parseNodes(src: string): MutableNode[] {
  const nodes: MutableNode[] = [];
  const stack: MutableNode[][] = [nodes];
  ...
  while ((m = re.exec(src))) {
    ...
    const node: MutableNode = [tag, parseAttrs(attrSrc || "")];
  ...
  const prune = (list: MutableNode[]): void => {
    for (const n of list) {
      if (n[2]) {
...
function namespaceIds(nodes: MutableNode[], prefix: string): MutableNode[] {
  const walk = (list: MutableNode[]): void => {
...
function hasLiteralColour(nodes: MutableNode[]): boolean {
  const paints = ["fill", "stroke", "stopColor", "floodColor"];
  const walk = (list: MutableNode[]): boolean =>
    list.some(
      (node) =>
        paints.some((p) => {
          const v = node[1][p];
...
function readSvgNotice(raw: string): string | null {
...
function readSvgFile(
  file: string,
  name: string,
): { viewBox: string; nodes: MutableNode[] } {
...
const lucideNodes: Record<string, [string, Record<string, string>][]> =
  JSON.parse(fs.readFileSync(LUCIDE, "utf8"));

function lucideVersion(): string {
  try {
    return JSON.parse(
      fs.readFileSync(
...
const HEADER = (from: string) =>
  `// AUTO-GENERATED by editor/scripts/generate-icon-registry.mts — do not edit.\n` +
  `// Source: ${from}\n` +
...
function emitEntries(entries: Record<string, Entry>): string {
...
const lucideEntries: Record<string, Entry> = {};
