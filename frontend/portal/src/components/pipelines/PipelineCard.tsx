...
export function PipelineCard({ pipeline, onOpen }: PipelineCardProps) {
  ...
  const errorTone =
    m.errorRate >= 0.02 ? "danger" : m.errorRate >= 0.01 ? "warning" : "default";
  ...
  return (
    ...
    >
      <div className="portal-pipelines__card-head">
        ...
        <StatusBadge tone={degraded ? "warning" : "success"} size="sm" pulse={degraded}>
      ...
      <div className="portal-pipelines__card-rail">
        ...
        <span className="portal-pipelines__rail-arrow" aria-hidden>→</span>
        <StageDots stages={pipeline.stages} />
        <span className="portal-pipelines__rail-arrow" aria-hidden>→</span>
        <span className="portal-pipelines__rail-chip">{pipeline.destination}</span>
      </div>
      ...
      <div className="portal-pipelines__metrics">
        ...
        <StatTile label="Error rate" value={pct(m.errorRate, 2)} tone={errorTone} />
