...
const meta: Meta = {
  ...
  decorators: [(S) => <div style={{ width: "28rem" }}><S /></div>],
};
export default meta;
type Story = StoryObj;
...
export const Input_Default: Story = {
  render: () => (
    <FormField label="Pipeline name" required helperText="Lowercase letters and dashes only.">
...
export const Input_WithIcon: Story = {
  render: () => (
    <FormField label="Search">
      <Input leadingIcon={<SearchIcon size={14} />} placeholder="Search Stirling…" />
    </FormField>
  ),
};
...
export const Checkbox_Single: Story = {
  render: () => (
    <Stack gap="2">
      <Checkbox defaultChecked label="Notify on pipeline failure (webhook + email)" />
      <Checkbox label="Send low-confidence docs to review queue" description="Confidence below 0.85 routes to a reviewer." />
...
export const Checkbox_GridOfCategories: Story = {
  render: () => (
    <FormField label="PII categories">
      <Inline gap="2">
        {["SSN", "DOB", "Accounts", "Contacts", "Names", "Addresses"].map((c) => (
...
export const Radio_Group: Story = {
  render: () => {
    function Bound() {
      const [mode, setMode] = useState<"stirling" | "byok" | "hyok">("stirling");
      return (
        <FormField label="Key mode">
          <RadioGroup
            ...
            onChange={setMode}
            options={[
              { value: "stirling", label: "Stirling-managed", description: "Stirling generates and rotates keys." },
              { value: "byok", label: "BYOK (AWS KMS)", description: "Bring your own KMS key." },
              { value: "hyok", label: "HYOK (Enterprise)", description: "Stirling never sees the key material." },
            ]}
          />
        </FormField>
      );
  ...
  render: () => {
    function Bound() {
      ...
      return (
        ...
        >
          <Slider value={v} min={0} max={1} step={0.01} onChange={setV} formatValue={(x) => x.toFixed(2)} />
        </FormField>
      );
    }
    return <Bound />;
    ...
    function Bound() {
      ...
      return (
        <FormField label="Retain artifacts for">
          <Slider value={days} min={7} max={365} step={1} onChange={setDays} formatValue={(d) => `${d} days`} />
        </FormField>
      );
    }
    return <Bound />;
      ...
      return (
        <Stack gap="4">
          <FormField label="Pipeline name" required>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. coi-compliance" />
          </FormField>
          ...
          <FormField label="Key mode">
            <RadioGroup name="km" value={mode} onChange={setMode} options={[
          ...
          <FormField label="Confidence gate" helperText="Default 0.85.">
            <Slider value={conf} min={0} max={1} step={0.01} onChange={setConf} formatValue={(v) => v.toFixed(2)} />
          </FormField>
          <FormField label="Alerts">
            <Stack gap="2">
              <Checkbox checked={notify} onChange={(e) => setNotify(e.target.checked)} label="Notify on failure" />
              <Checkbox checked={review} onChange={(e) => setReview(e.target.checked)} label="Send low-confidence to review" />
            </Stack>
          </FormField>
        </Stack>
      );
