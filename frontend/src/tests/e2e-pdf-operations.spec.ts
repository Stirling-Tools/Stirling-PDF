...
function fixture(filename: string): string {
  const candidates = [
    path.resolve(process.cwd(), 'src', 'core', 'tests', 'test-fixtures', filename),
    path.resolve(process.cwd(), 'frontend', 'src', 'core', 'tests', 'test-fixtures', filename),
  ];
  for (const p of candidates) {
    if (fs.existsSync(p)) return p;
  }
  throw new Error(`Test fixture not found: ${filename} (tried: ${candidates.join(', ')})`);
}
...
function testingFile(filename: string): string {
  ...
  throw new Error(`Testing file not found: ${filename} (tried: ${candidates.join(', ')})`);
}
...
async function uploadFiles(page: import('@playwright/test').Page, filePaths: string[]) {
  ...
  await expect(page.getByText(/selected/i).first()).toBeVisible({ timeout: 15000 });
}

// Helper: click the execute/run button and wait for results
async function executeAndWaitForResults(page: import('@playwright/test').Page, timeout = 60000) {
  const runButton = page.locator('[data-tour="run-button"]');
  await expect(runButton).toBeEnabled({ timeout: 10000 });
  await runButton.click();

  // Wait for the review/results panel to appear
  await expect(page.locator('[data-testid="review-panel-container"]')).toBeVisible({ timeout });
}
...
test.describe('E2E PDF Operations', () => {
  ...
  test.describe('Merge Tool - End to End', () => {
    test('should merge two PDF files and produce a result', async ({ page }) => {
      ...
      const reviewPanel = page.locator('[data-testid="review-panel-container"]');
    ...
    test('should merge three PDF files', async ({ page }) => {
      ...
      await executeAndWaitForResults(page);
      await expect(page.locator('[data-testid="review-panel-container"]')).toBeVisible();
    });
  });
  ...
  test.describe('Split Tool - End to End', () => {
    test('should split a PDF by page numbers', async ({ page }) => {
      ...
      await page.getByText(/Page Numbers/i).first().click();
