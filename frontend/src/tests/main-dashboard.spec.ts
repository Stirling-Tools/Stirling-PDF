...
import { loginAndSetup, ensureCookieConsent, skipOnboarding } from './helpers/login';
...
test.describe('2. Main Dashboard / Home Page', () => {
  ...
  test.describe('2.1 Dashboard Layout and Tool Categories', () => {
    test('should display all navigation elements and tool categories', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: /^Tools$/i }).first()).toBeVisible({ timeout: 10000 });
      await expect(page.locator('[data-testid="read-button"]').first()).toBeVisible();
      await expect(page.locator('[data-testid="automate-button"]').first()).toBeVisible();
      await expect(page.locator('[data-testid="files-button"]').first()).toBeVisible();
      ...
      await expect(page.getByRole('button', { name: /fullscreen|sidebar/i }).first()).toBeVisible();
  ...
  test.describe('2.2 Dashboard - Recommended Tools', () => {
    test('should display recommended tools and navigate to merge', async ({ page }) => {
      ...
      for (const tool of recommendedTools) {
        await expect(page.getByText(tool).first()).toBeVisible({ timeout: 10000 });
      }
  ...
  test.describe('2.3 Dashboard - File Upload Area', () => {
    test('should display file upload area with buttons', async ({ page }) => {
      ...
      const uploadButton = page.getByRole('button', { name: /upload|add files/i }).first();
  ...
  test.describe('2.4 Dashboard - Footer Links', () => {
    test('should display footer links with correct URLs', async ({ page }) => {
      ...
      await expect(page.getByText('Survey').first()).toBeVisible({ timeout: 10000 });
      ...
      await expect(page.getByText('Accessibility').first()).toBeVisible({ timeout: 10000 });

      // Step 2: Verify the GitHub link points to correct URL
      const githubLink = page.locator('a[href*="github.com/Stirling-Tools/Stirling-PDF"]').first();
      await expect(githubLink).toBeVisible();

      // Step 3: Verify the Discord link points to correct URL
      const discordLink = page.locator('a[href*="discord.gg/Cn8pWhQRxZ"]').first();
