...
test.describe('6. Split Tool', () => {
  ...
  test.describe('6.1 Split - Method Selection', () => {
    test('should display all split methods', async ({ page }) => {
      ...
      await expect(page.locator('text=/Choose Method|Method/i').first()).toBeVisible();
      ...
      for (const method of splitMethods) {
        await expect(page.getByText(method).first()).toBeVisible({ timeout: 5000 });
      }

      // Step 3: Verify the "Split" button is disabled
      const splitButton = page.getByRole('button', { name: /split|pisahkan/i }).first();
