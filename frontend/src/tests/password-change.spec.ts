...
import { loginAndSetup, login, dismissWelcomeDialog } from './helpers/login';
...
test.describe('21. Password Change Flow', () => {
  test.describe('21.1 Change Password', () => {
    test('should change password end-to-end', async ({ page }) => {
      ...
      await passwordModal.getByPlaceholder(/Enter a new password/i).fill('admin');
      ...
      const modalSubmitBtn = passwordModal.getByRole('button', { name: /Update password/i });
  ...
  test.describe('21.2 Change Password - Mismatch', () => {
    test('should reject mismatched passwords with error', async ({ page }) => {
      ...
      await passwordModal.getByPlaceholder(/Enter a new password/i).fill('newpassword123');

      // Step 3: Enter a different value in confirm password
      await passwordModal.getByPlaceholder(/Re-enter/i).fill('differentpassword456');
      ...
      await expect(
        page.locator('text=/mismatch|do not match|tidak cocok|tidak sama/i').first()
