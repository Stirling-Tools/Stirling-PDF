...
test.describe('3. Tool Search', () => {
  ...
  test.describe('3.1 Search - Happy Path', () => {
    test('should filter tools in real time based on search input', async ({ page }) => {
      ...
      await expect(page.locator('text=/merge|menggabungkan/i').first()).toBeVisible({ timeout: 5000 });

      // Step 5: Clear the search field
      await searchBox.clear();

      // Step 6: Verify all tools reappear (check for multiple categories)
      await expect(page.locator('text=/recommended|direkomendasikan/i').first()).toBeVisible({ timeout: 5000 });
    });
  });

  test.describe('3.2 Search - No Results', () => {
    test('should handle queries with no matching tools gracefully', async ({ page }) => {
      // Step 1: Click on the search box
      const searchBox = page.getByPlaceholder(/search|cari/i).first();
      await searchBox.click();
      ...
      await expect(page.locator('text=/recommended|direkomendasikan/i')).toHaveCount(0, { timeout: 5000 });
