...
import { login, loginAndSetup, dismissWelcomeDialog } from './helpers/login';
...
test.describe('1. Authentication and Login', () => {
  test.describe('1.1 Login Page - Happy Path', () => {
    test('should login successfully with valid credentials', async ({ page }) => {
      ...
      await expect(page.locator('img[alt*="Stirling"], img[src*="stirling"], img[src*="logo"]').first()).toBeVisible();

      // Step 3: Confirm the heading for "Sign In" / "Login" is visible
      await expect(page.getByRole('heading', { name: /sign in|login|masuk/i })).toBeVisible();
      ...
      await expect(page.locator('.h-screen, .mobile-layout, [data-testid="dashboard"]').first()).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('1.2 Login Page - Invalid Credentials', () => {
    ...
    test.skip('should show error message with invalid credentials', async ({ page }) => {
      ...
      await expect(page.locator('text=/invalid|error|incorrect|locked|salah|terkunci/i')).toBeVisible({ timeout: 10000 });
  ...
  test.describe('1.3 Login Page - Empty Fields Validation', () => {
    test('should keep Sign In button disabled when fields are empty', async ({ page }) => {
      // Starting state: User is logged out; browser on /login
      await page.goto('/login');
      await page.waitForLoadState('domcontentloaded');
  ...
  test.describe('1.5 Login Page - Session Expiry and Redirect', () => {
    test('should redirect back to intended page after re-login', async ({ page }) => {
      ...
      await page.context().addCookies([
        {
          name: 'cc_cookie',
          value: JSON.stringify({ categories: ['necessary'], revision: 0, data: null, rfc_cookie: false }),
          domain: 'localhost',
          path: '/',
        },
      ]);
