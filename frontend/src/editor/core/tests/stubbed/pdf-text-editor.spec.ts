...
// The same Sample.pdf that ships in `frontend/public/samples/`.
...
// PDF text editor regression suite. It runs entirely in the browser via
...
test.describe("PDF text editor - smoke", () => {
  test("the editor mounts at /pdf-text-editor", async ({ page }) => {
    await gotoEditor(page);
    await expect(page.getByTestId("pdf-editor-sidebar-empty")).toBeVisible();
    await expect(page.getByTestId("pdf-editor-toolbar")).toBeVisible();
  });
});

test.describe("PDF text editor - load and render", () => {
  test("loads a PDF in the browser and renders its text runs", async ({
    ...
    await expect(page.getByTestId("pdf-editor-sidebar-status")).toBeVisible();
  });
});

test.describe("PDF text editor - editing", () => {
...
test.describe("PDF text editor - overlay grows to fit typed text", () => {
...
test.describe("PDF text editor - selection + properties", () => {
  test("selecting a run enables the toolbar controls", async ({ page }) => {
    await gotoEditor(page);
    await loadSamplePdf(page);

    const fontSize = page.getByTestId("pdf-editor-font-size");
  ...
  test("changing the font-size control updates the run fontSize", async ({
    ...
    const runId = await firstRun.evaluate((el) =>
      (el.getAttribute("data-testid") ?? "").replace(/^pdf-editor-run-/, ""),
    );
...
test.describe("PDF text editor - save", () => {
...
test.describe("PDF text editor - whitespace preservation", () => {
  ...
  test("user-sample.pdf: deleting one char from middle of a LineGrouper-merged line doesn't corrupt or duplicate sub-runs", async ({
    ...
    await page.evaluate(
      ({ tid, caretAt }) => {
        const el = document.querySelector<HTMLDivElement>(
          `[data-testid="pdf-editor-run-${tid}"]`,
        );
...
test.describe("PDF text editor - colour", () => {
  test("changing the colour control dispatches a SetColour edit", async ({
    ...
    const runId = await firstRun.evaluate((el) => {
      ...
      return tid.replace(/^pdf-editor-run-/, "");
    });
    await firstRun.click();
...
test.describe("PDF text editor - delete + multi-select", () => {
  ...
  test("shift-click selects multiple runs", async ({ page }) => {
    ...
    await expect(page.getByTestId("pdf-editor-undo")).toBeEnabled();
    await page.getByTestId("pdf-editor-undo").click();
    await expect(page.getByTestId("pdf-editor-redo")).toBeEnabled();
  });
});

test.describe("PDF text editor - keyboard shortcuts", () => {
  test("Ctrl+Z undoes the latest edit", async ({ page }) => {
    ...
    await page
      .locator('[data-testid="pdf-editor-stage"]')
...
test.describe("PDF text editor - font family", () => {
...
test.describe("PDF text editor - multi-page", () => {
  ...
  test("edits on a non-first page are saved and round-trip", async ({
    ...
    const runId = runTestId.replace(/^pdf-editor-run-/, "");
...
test.describe("PDF text editor - bold/italic", () => {
  ...
  test("user-sample.pdf: Bold on a LineGrouper-merged tagline removes every per-glyph original (no ghost layers)", async ({
    ...
    const downloadBtn = page.getByTestId("pdf-editor-download");
    const downloadPromise = page.waitForEvent("download");
...
test.describe("PDF text editor - add text box", () => {
  test("Add text mode + page click inserts a new run", async ({ page }) => {
    ...
    const pageEl = page.getByTestId("pdf-editor-page-0");
...
test.describe("PDF text editor - line grouping", () => {
...
test.describe("PDF text editor - glyph fallback", () => {
...
test.describe("PDF text editor - typing fidelity", () => {
...
test.describe("PDF text editor - image manipulation", () => {
  test("image overlays render with pointer events enabled", async ({
    ...
    const image = page.locator('[data-testid^="pdf-editor-image-"]').first();
...
test.describe("PDF text editor - image click-through + delete", () => {
  ...
  test("clicking an image selects it, enabling Delete on the toolbar", async ({
    ...
    await expect(page.getByTestId("pdf-editor-delete")).toBeEnabled();
  });
  ...
  test("Delete on a selected image removes it", async ({ page }) => {
    ...
    const images = page.locator('[data-testid^="pdf-editor-image-"]');
...
test.describe("PDF text editor - render throttling", () => {
  test("off-screen pages show a placeholder until they near the viewport", async ({
    ...
    await expect(page.getByTestId("pdf-editor-page-0-placeholder")).toHaveCount(
      0,
      {
        timeout: 10_000,
      },
...
test.describe("PDF text editor - lazy page loading", () => {
  ...
  test("big-sample.pdf renders within a bounded time, edits, and round-trips", async ({
    ...
    await page.evaluate(() => {
      const el = document.querySelector<HTMLElement>(
        '[data-testid="pdf-editor-page-40"]',
      );
      el?.scrollIntoView({ block: "center" });
    });
    await expect(page.getByTestId("pdf-editor-page-40")).toBeVisible({
      timeout: 30_000,
    });
...
test.describe("PDF text editor - paragraph recognition", () => {
...
test.describe("PDF text editor - text run move (Ctrl+drag)", () => {
...
test.describe("PDF text editor - image transform (absolute)", () => {
...
test.describe("PDF text editor - form xobject recursion", () => {
...
test.describe("PDF text editor - load progress overlay", () => {
  test("loading overlay shows a stage and progress bar while opening", async ({
    ...
    const overlayPromise = page
      .getByTestId("pdf-editor-stage-loading")
    ...
    if (!sawOverlay) {
      ...
      await expect(page.getByTestId("pdf-editor-stage-loading")).toHaveCount(0);
    }
  });
});

test.describe("PDF text editor - fit-to-width", () => {
  test("Fit button updates the zoom percent based on viewport width", async ({
    ...
    const before = await page
      .getByTestId("pdf-editor-zoom-percent")
      .innerText();
    await page.getByTestId("pdf-editor-zoom-fit").click();
    const after = await page.getByTestId("pdf-editor-zoom-percent").innerText();
...
test.describe("PDF text editor - F3 next match", () => {
  test("F3 opens the find bar and steps to the next match", async ({
    ...
    await page.getByTestId("pdf-editor-find-input").fill("documents");
    ...
    await expect(page.getByTestId("pdf-editor-find-count")).toContainText(
      /of \d+/,
    );
  });
});

test.describe("PDF text editor - zoom controls", () => {
  test("zoom in / zoom out / 100% buttons drive renderScale", async ({
    page,
  }) => {
    await gotoEditor(page);
    await loadSamplePdf(page);
    const percent = page.getByTestId("pdf-editor-zoom-percent");
    ...
    await page.getByTestId("pdf-editor-zoom-reset").click();
    await expect(percent).toHaveText("100%");
  });
});

test.describe("PDF text editor - find in document", () => {
  test("Ctrl+F opens the find bar and steps through matches", async ({
    ...
    await page.keyboard.press("Control+f");
    await expect(page.getByTestId("pdf-editor-find-bar")).toBeVisible();
    await page.getByTestId("pdf-editor-find-input").fill("Test");
    const count = page.getByTestId("pdf-editor-find-count");
    await expect(count).toContainText(/of \d+/);
    await page.getByTestId("pdf-editor-find-next").click();
    await expect(count).toContainText(/of \d+/);
    await page.getByTestId("pdf-editor-find-close").click();
    await expect(page.getByTestId("pdf-editor-find-bar")).toHaveCount(0);
  });
});

test.describe("PDF text editor - paragraph soft-wrap", () => {
...
test.describe("PDF text editor - undo restores form-xobject text", () => {
  test("editing form-xobject text then undoing puts the original back visually", async ({
    ...
    const undoneRuns = page.locator('[data-testid^="pdf-editor-run-p0-"]');
...
test.describe("PDF text editor - workbench tab UX", () => {
  ...
  test("Editor workbench pins itself when an external setWorkbench fires", async ({
    ...
    await expect(page.getByTestId("pdf-editor-stage")).toBeVisible();
  });
});

test.describe("PDF text editor - dirty state", () => {
...
test.describe("PDF text editor - toolbar tooltips", () => {
  test("toolbar buttons expose tooltip labels", async ({ page }) => {
    ...
    await expect(page.getByTestId("pdf-editor-add-text")).toBeVisible();
    await expect(page.getByTestId("pdf-editor-add-image")).toBeVisible();
    ...
    await expect(page.getByTestId("pdf-editor-rotate-left")).toHaveCount(0);
    await expect(page.getByTestId("pdf-editor-rotate-right")).toHaveCount(0);
    await expect(page.getByTestId("pdf-editor-print")).toHaveCount(0);
    await expect(page.getByTestId("pdf-editor-reset")).toHaveCount(0);
    await expect(page.getByTestId("pdf-editor-save-workbench")).toHaveCount(0);
  });
});

test.describe("PDF text editor - duplicate selected run", () => {
  test("Ctrl+D clones the selected text run and undo removes the clone", async ({
    ...
    await page.evaluate(() => {
      const store = (window as unknown as { __editor_store?: unknown })
        .__editor_store as { undo: () => void };
      store.undo();
    });
...
test.describe("PDF text editor - Ctrl+wheel zoom", () => {
  test("Ctrl+wheel up on stage increases renderScale", async ({ page }) => {
    ...
    const readScale = () =>
      page.evaluate(() => {
        const store = (
          window as unknown as {
            __editor_store?: { getState: () => { renderScale: number } };
          }
        ).__editor_store!;
...
test.describe("PDF text editor - paragraph line wrap fidelity", () => {
...
test.describe("PDF text editor - marquee + merge", () => {
  ...
  test("Group / Ungroup toolbar buttons merge and split paragraphs", async ({
    ...
    await expect(page.getByTestId("pdf-editor-group")).toHaveCount(0);
    await expect(page.getByTestId("pdf-editor-ungroup")).toHaveCount(0);
    ...
    await expect(page.getByTestId("pdf-editor-group")).toBeEnabled();
    await page.getByTestId("pdf-editor-group").click();
    await page.waitForTimeout(200);

    const merged = await runs.count();
    expect(merged).toBe(initial - 1);

    await expect(page.getByTestId("pdf-editor-ungroup")).toBeEnabled();
    await page.getByTestId("pdf-editor-ungroup").click();
    await page.waitForTimeout(200);
...
test.describe("PDF text editor - help overlay", () => {
  ...
  test("Help button opens the overlay", async ({ page }) => {
    ...
    await page.getByTestId("pdf-editor-help").click();
    await expect(
      page.getByRole("heading", { name: "Keyboard shortcuts" }),
    ).toBeVisible();
  });
...
test.describe("PDF text editor - filename in header", () => {
...
test.describe("PDF text editor - selection count panel", () => {
  test("sidebar shows N runs selected after multi-select", async ({ page }) => {
    ...
    const countNode = page.getByTestId("pdf-editor-selection-count");
    await expect(countNode).toBeVisible();
    await expect(countNode).toContainText(/2 boxes/);
  });
});

test.describe("PDF text editor - PageDown navigation", () => {
...
test.describe("PDF text editor - Ctrl+A select all", () => {
...
test.describe("PDF text editor - stress: whitespace insertion variations", () => {
  ...
  test("whitespace stress: inserting at the START of the tagline shifts content right and keeps separation", async ({
    ...
    await insertAt(page, `pdf-editor-run-${before.id}`, 0, "PRE ");
    const after = await readTagline(page);
    if (!after) throw new Error("tagline vanished");
...
test.describe("PDF text editor - stress: bold / font swap variations", () => {
  async function loadFixture(page: import("@playwright/test").Page) {
    await gotoEditor(page);
    await page
      .locator('[data-testid="pdf-editor-file-input"]')
  ...
  test("font swap stress: Bold then Italic then Bold (cross-axis toggles) preserves text", async ({
    ...
    await page.waitForTimeout(250);
    await selectTagline(page);
    await page.getByTestId("pdf-editor-italic").click();
    await page.waitForTimeout(250);
    await selectTagline(page);
    await selectFontFamily(page, "Helvetica");
    await page.waitForTimeout(250);
...
test.describe("PDF text editor - stress: add / remove cycles (no leaks)", () => {
  async function loadFixture(page: import("@playwright/test").Page) {
    await gotoEditor(page);
    await page
      .locator('[data-testid="pdf-editor-file-input"]')
  ...
  test("undo five edits in a row restores baseline text + bounds", async ({
    ...
    for (let i = 0; i < 6; i++) {
      const undoBtn = page.getByTestId("pdf-editor-undo");
...
test.describe("PDF text editor - stress: save+reopen multi-cycle", () => {
  async function loadFixture(page: import("@playwright/test").Page) {
    await gotoEditor(page);
    await page
      .locator('[data-testid="pdf-editor-file-input"]')
  ...
  test("save+reopen three times in a row (with one edit each) doesn't compound ghost objects", async ({
    ...
    for (let cycle = 0; cycle < 3; cycle++) {
      ...
      const target = page.locator(`[data-testid="pdf-editor-run-${id}"]`);
  ...
  test("save+reopen preserves a fresh add-text run with its full typed content", async ({
    ...
    const lastRun = page.locator('[data-testid^="pdf-editor-run-p0-"]').last();
...
test.describe("PDF text editor - stress: AddText box content fidelity", () => {
  ...
  async function addNewTextBox(
    ...
    await page.getByTestId("pdf-editor-add-text").click();
    await page.getByTestId("pdf-editor-page-0").click({ position });
    await page.waitForTimeout(300);
...
test.describe("PDF text editor - stress: deletion shrinks bounds (no stuck-wide overlay)", () => {
...
test.describe("PDF text editor - stress: overlay box width hugs the text", () => {
  ...
  test("unfocused AddText box: overlay width is within a few pixels of the text width (no +1em buffer)", async ({
    ...
    await page.waitForTimeout(500);
    const overlayW = await overlayCssWidth(page, `pdf-editor-run-${id}`);
    const textW = await cssTextWidth(page, `pdf-editor-run-${id}`);
...
test.describe("PDF text editor - F-duplication regression (Sample.pdf tagline)", () => {
  ...
  test("typing a single F at end of tagline produces exactly one F", async ({
    ...
    const fEmits = await page.evaluate(() => {
      const w = window as unknown as {
        __charcode_events?: Array<{
          outcome: string;
          text: string;
          note: string;
        }>;
      };
      return (w.__charcode_events ?? []).filter((e) => e.text === "F");
    });
    expect(
      fEmits.length,
  ...
  test("two consecutive M edits on 10M+ produce ≤2 emit events for 'M' (no duplicate fire)", async ({
    ...
    await page.evaluate(() => {
      const w = window as unknown as { __charcode_events?: unknown[] };
      if (w.__charcode_events) w.__charcode_events = [];
    });
    ...
    const mEmits = await page.evaluate(() => {
      const w = window as unknown as {
        __charcode_events?: Array<{ outcome: string; text: string }>;
      };
      return (w.__charcode_events ?? []).filter((e) => e.text === "M");
    });
    expect(
      mEmits.length,
