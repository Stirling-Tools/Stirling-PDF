...
} from "@editor/hooks/tools/changeMetadata/useChangeMetadataParameters";
import { TrappedStatus } from "@editor/types/metadata";
...
describe("useChangeMetadataParameters", () => {
  ...
  describe("parameter updates", () => {
    test.each([
      ...
      "should update $paramName parameter",
      ({ paramName, value }) => {
        const { result } = renderHook(() => useChangeMetadataParameters());
  ...
  describe("validation", () => {
    test.each([
      {
        description: "deleteAll is true",
        updates: { deleteAll: true },
        expected: true,
      },
      {
        description: "has title",
        updates: { title: "Test Document" },
        expected: true,
      },
      {
        description: "has author",
        updates: { author: "John Doe" },
        expected: true,
      },
      {
        description: "has subject",
        updates: { subject: "Test Subject" },
        expected: true,
      },
      {
        description: "has keywords",
        updates: { keywords: "test" },
        expected: true,
      },
      {
        description: "has creator",
        updates: { creator: "Test Creator" },
        expected: true,
      },
      {
        description: "has producer",
        updates: { producer: "Test Producer" },
        expected: true,
      },
      {
        description: "has creation date",
        updates: { creationDate: new Date("2025/01/17 14:30:00") },
        expected: true,
      },
      {
        description: "has modification date",
        updates: { modificationDate: new Date("2025/01/17 14:30:00") },
        expected: true,
      },
      {
        description: "has trapped status",
        updates: { trapped: TrappedStatus.TRUE },
        expected: true,
      },
      ...
      "should validate correctly when $description",
      ({ updates, expected }) => {
        const { result } = renderHook(() => useChangeMetadataParameters());
    ...
    test.each([
      {
        description: "valid creation date",
        updates: {
          title: "Test",
      ...
      {
        description: "valid modification date",
        updates: {
          title: "Test",
          modificationDate: new Date("2025/01/17 14:30:00"),
        },
        expected: true,
      },
      {
        description: "empty dates are valid",
        updates: { title: "Test", creationDate: null, modificationDate: null },
        expected: true,
      },
    ])(
      "should validate dates correctly with $description",
      ({ updates, expected }) => {
        const { result } = renderHook(() => useChangeMetadataParameters());
  ...
  describe("custom metadata", () => {
    ...
    test("should update custom metadata by ID", () => {
      ...
      act(() => {
        result.current.updateCustomMetadata(
          customId,
          "Department",
          "Engineering",
        );
    ...
    test("should generate unique IDs for multiple custom entries", () => {
      ...
      const ids = result.current.parameters.customMetadata.map(
        (entry) => entry.id,
      );
