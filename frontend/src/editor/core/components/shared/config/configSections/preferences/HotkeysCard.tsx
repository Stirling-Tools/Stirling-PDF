...
/**
 * One rebindable shortcut per registered tool. While a capture is armed this
 * swallows every keystroke in the window, so it disarms as soon as the pointer
 * or the focus leaves the row being rebound - otherwise the first character of
 * a password typed in a sibling card would be captured as a shortcut.
 */
export function HotkeysCard() {
  ...
  // Pointer, focus or window leaving the row being rebound ends the capture, so
  // no other field ever loses a keystroke to it.
  useEffect(() => {
    if (!editingTool) {
      return;
    }

    // Capture phase: a modal or menu that stops propagation must not keep the
    // capture armed behind it.
    const handleLeave = (event: Event) => {
      const target = event.target;
      if (target instanceof Node && editingRowRef.current?.contains(target)) {
        return;
      }
      cancelCapture();
    };

    window.addEventListener("pointerdown", handleLeave, true);
    window.addEventListener("focusin", handleLeave, true);
    window.addEventListener("blur", cancelCapture);
    return () => {
      window.removeEventListener("pointerdown", handleLeave, true);
      window.removeEventListener("focusin", handleLeave, true);
      window.removeEventListener("blur", cancelCapture);
    };
  }, [editingTool, cancelCapture]);
