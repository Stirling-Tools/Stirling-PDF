...
import { defaultParameters } from "@editor/hooks/tools/changePermissions/useChangePermissionsParameters";
import type { ChangePermissionsParameters } from "@editor/hooks/tools/changePermissions/useChangePermissionsParameters";
...
describe("ChangePermissionsSettings", () => {
  ...
  test("should render all permission checkboxes", () => {
    ...
    expect(
      screen.getByText(
        "mock-changePermissions.permissions.preventAssembly.label",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "mock-changePermissions.permissions.preventPrinting.label",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "mock-changePermissions.permissions.preventModify.label",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "mock-changePermissions.permissions.preventExtractContent.label",
      ),
    ).toBeInTheDocument();
  });
  ...
  test("should render all permission types with correct labels", () => {
    ...
    permissionKeys.forEach((permission) => {
      expect(
        screen.getByText(
          `mock-changePermissions.permissions.${permission}.label`,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  test("should show checkboxes as checked when parameters are true", () => {
    ...
    const assemblyCheckbox = screen.getByLabelText(
      "mock-changePermissions.permissions.preventAssembly.label",
    ) as HTMLInputElement;
    const printingCheckbox = screen.getByLabelText(
      "mock-changePermissions.permissions.preventPrinting.label",
    ) as HTMLInputElement;
    const modifyCheckbox = screen.getByLabelText(
      "mock-changePermissions.permissions.preventModify.label",
    ) as HTMLInputElement;
    const formCheckbox = screen.getByLabelText(
      "mock-changePermissions.permissions.preventFillInForm.label",
    ) as HTMLInputElement;
  ...
  test.each([
    {
      initialValue: false,
      expectedValue: true,
      description: "checking an unchecked box",
    },
    {
      initialValue: true,
      expectedValue: false,
      description: "unchecking a checked box",
    },
  ])(
    "should call onParameterChange with $expectedValue when $description",
  ...
  test.each([
    ...
    "should set checkboxes disabled=$disabled when disabled prop is $disabled",
    ({ disabled, expectedState }) => {
      render(
        <TestWrapper>
          <ChangePermissionsSettings
            parameters={defaultParameters}
  ...
  test("should call translation function with correct keys", () => {
    ...
    permissionKeys.forEach((permission) => {
      expect(mockT).toHaveBeenCalledWith(
        `changePermissions.permissions.${permission}.label`,
        permission,
      );
    });
  });
  ...
  test.each(
    Object.keys(defaultParameters) as Array<keyof ChangePermissionsParameters>,
  )("should handle %s permission type individually", (permission) => {
    const testParameters: ChangePermissionsParameters = {
      ...defaultParameters,
      [permission]: true,
    };
