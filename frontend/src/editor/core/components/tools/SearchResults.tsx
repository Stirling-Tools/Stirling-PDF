...
import ToolButton from "@editor/components/tools/toolPicker/ToolButton";
import { useTranslation } from "react-i18next";
import { useToolSections } from "@editor/hooks/useToolSections";
import SubcategoryHeader from "@editor/components/tools/shared/SubcategoryHeader";
import NoToolsFound from "@editor/components/tools/shared/NoToolsFound";
import "@editor/components/tools/toolPicker/ToolPicker.css";
    ...
    <Stack p="sm" gap="xs" className="tool-picker-scrollable">
      {searchGroups.map((group) => (
        <Box key={group.subcategoryId} w="100%">
          <SubcategoryHeader
            label={getSubcategoryLabel(t, group.subcategoryId)}
          />
          <Stack gap="xs">
            {group.tools.map(({ id, tool }) => {
              ...
              const isSynonymMatch =
                matchedText &&
                tool.synonyms?.some((synonym) =>
                  matchedText.toLowerCase().includes(synonym.toLowerCase()),
                );
              const matchedSynonym = isSynonymMatch
                ? tool.synonyms?.find((synonym) =>
                    matchedText.toLowerCase().includes(synonym.toLowerCase()),
                  )
                : undefined;
