...
import "@app/tools/pdfTextEditor/components/AnnotationOutline.css";
...
export function AnnotationOutline({
  ...
  const label =
    kind === "widget"
      ? t(
          "pdfTextEditor.annotations.widget",
          "Form field - not page text, so it can't be edited here",
        )
      : kind === "freetext"
        ? t(
            "pdfTextEditor.annotations.freetext",
            "Annotation text - not page text, so it can't be edited here",
          )
        : t(
            "pdfTextEditor.annotations.stamp",
            "Stamp annotation - not page text, so it can't be edited here",
          );

  return (
    <div
      className="pdf-editor-annotation-outline"
      data-testid={`pdf-editor-annot-${annotation.id}`}
