...
vi.mock("@editor/contexts/AppConfigContext", () => ({
  useAppConfig: () => ({ config: {}, loading: mocks.configLoading }),
}));
vi.mock("@editor/hooks/useClassificationEnabled", () => ({
...
vi.mock("@editor/services/heuristic/heuristicClassification", () => ({
  classifyFileHeuristically: (file: File) => mocks.classify(file),
}));
vi.mock("@editor/services/classificationMeter", () => ({
  meterClassificationRun: (payload: unknown) => mocks.meter(payload),
}));

import { useClientSideClassification } from "@editor/components/policies/useClientSideClassification";
