    ...
    <Stack gap={4}>
      {images.map((image, idx) => (
        <div key={idx} style={{ wordBreak: "break-word", overflowWrap: "break-word" }}>
          <Text size="sm" c="dimmed">
            {image.Name ? `${image.Name} ` : "Image "}({image.Width}×{image.Height}px
        ...
        <Accordion variant="separated" radius="md" defaultValue="" styles={pdfInfoAccordionStyles}>
          {Object.entries(perPage).map(([pageLabel, pageInfo]: [string, PdfPageInfo]) => (
            <Accordion.Item key={pageLabel} value={pageLabel}>
              ...
              <Accordion.Panel>
                <div
                  style={{ backgroundColor: "var(--bg-raised)", color: "var(--text-primary)", borderRadius: 8, padding: 12 }}
                >
                  <Stack gap="sm">
                    {pageInfo?.Size && (
                      <Stack gap={4}>
                        <Text fw={600} size="sm">
                          {t("getPdfInfo.perPage.size", "Size")}
                        </Text>
                    ...
                    {pageInfo?.Annotations && (
                      <Stack gap={4}>
                        <Text fw={600} size="sm">
                          {t("getPdfInfo.perPage.annotations", "Annotations")}
                        </Text>
                    ...
                    <Stack gap={4}>
                      <Text fw={600} size="sm">
                        {t("getPdfInfo.perPage.links", "Links")}
                      </Text>
                      {renderList(pageInfo?.Links, noneDetected)}
                    </Stack>
                    <Stack gap={4}>
                      <Text fw={600} size="sm">
                        {t("getPdfInfo.perPage.fonts", "Fonts")}
                      </Text>
                    ...
                    {pageInfo?.XObjectCounts && (
                      <Stack gap={4}>
                        <Text fw={600} size="sm">
                          {t("getPdfInfo.perPage.xobjects", "XObject Counts")}
                        </Text>
                    ...
                    <Stack gap={4}>
                      <Text fw={600} size="sm">
                        {t("getPdfInfo.perPage.multimedia", "Multimedia")}
                      </Text>
