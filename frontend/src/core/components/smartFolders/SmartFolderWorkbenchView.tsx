...
import { Box, Text, Stack, Group, Button, ScrollArea, Loader, TextInput, Select } from "@mantine/core";
...
export function SmartFolderWorkbenchView({ data }: SmartFolderWorkbenchViewProps) {
  ...
  useEffect(() => {
    ...
    Promise.all(inputIds.map((id) => fileStorage.getStirlingFile(id as FileId))).then((files) =>
      setInputFiles(files.filter(Boolean) as StirlingFile[]),
    );
    ...
    if (!isServerFolder) {
      ...
      Promise.all(outputIds.map((id) => fileStorage.getStirlingFile(id as FileId))).then((files) =>
        setOutputFiles(files.filter(Boolean) as StirlingFile[]),
      );
    } else {
      setOutputFiles([]);
    }
  ...
  useEffect(() => {
    ...
    if (pendingFileIds && pendingFileIds.length > 0) {
      ...
      Promise.all(pendingFileIds.map((id) => fileStorage.getStirlingFile(id as FileId))).then(async (results) => {
  ...
  const handleServerOutputDownload = useCallback(
    async (serverFilename: string, displayName?: string) => {
      if (!folderId) return;
      try {
  ...
  const handleServerOutputPreview = useCallback(
    async (serverFilename: string, displayName?: string) => {
      if (!folderId) return;
      try {
        const file = await downloadServerFolderOutput(folderId, serverFilename);
  ...
  const handlePauseResume = useCallback(async () => {
    ...
    if (!nowPaused) {
      ...
      for (const fileId of pendingIds) {
        ...
        if (file) {
          runPipeline({ ...folder, isPaused: false }, file, fileId, folderRecord?.files[fileId]?.ownedByFolder ?? false);
        }
      }
    }
  ...
  const dashboardStats = useMemo(() => {
      ...
      statsPeriod === "24h"
        ...
        : statsPeriod === "7d"
          ...
          : statsPeriod === "30d"
            ? now - 30 * 86_400_000
            : 0;
    const entries = Object.entries(folderRecord?.files ?? {}).filter(([, m]) => getTs(m.addedAt) >= cutoff);
    ...
    const buckets: Array<{ processed: number; failed: number }> = Array.from({ length: bucketCount }, () => ({
      processed: 0,
      failed: 0,
    }));
      ...
      await Promise.all(
        ids.map(async (id) => {
          if (withOutputFiles) {
            ...
            await Promise.all(outputIds.map((oid) => fileStorage.deleteStirlingFile(oid as FileId).catch(() => {})));
          }
          await removeFile(id);
        }),
      );
      ...
      for (const id of ids) {
        ...
        const toAdd =
          outputs && outputs.length > 0
            ? outputs
            : ([inputFiles.find((f) => f.fileId === id)].filter(Boolean) as StirlingFile[]);
        files.push(...toAdd);
      }
      return files;
    },
  ...
  const FolderIcon = folder ? iconMap[folder.icon as keyof typeof iconMap] || iconMap.FolderIcon : iconMap.FolderIcon;
  ...
  const daysRunning = folder.createdAt ? Math.floor((Date.now() - new Date(folder.createdAt).getTime()) / 86400000) : null;
    ...
    <Box style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
            ...
            <Stack gap={2}>
                ...
                <Group gap={4} wrap="nowrap">
                  {ops.map((op, i) => (
                    <Group key={i} gap={4} wrap="nowrap" align="center">
                      {i > 0 && <ChevronRightIcon style={{ fontSize: "0.625rem", color: "var(--mantine-color-gray-5)" }} />}
              ...
              {isLocalFolder && (
                <Text size="xs" c={localInputFolderName ? "green" : "yellow"} style={{ fontSize: "0.6875rem" }}>
                  {localInputFolderName ? `Watching: ${localInputFolderName}` : "No input folder — edit to configure"}
                </Text>
              )}
            </Stack>
          </Group>
      ...
      <Box style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        >
          ...
          <Box style={{ padding: "0.5rem 1rem 0", flexShrink: 0 }}>
            ...
            >
              ...
              <StatCard
                ...
                onClick={() => {
                  setActivityStatusFilter(activityStatusFilter === "processed" ? "all" : "processed");
                  setExpandedActivityIds(new Set());
                }}
              />
              ...
              <StatCard
                icon={
                  <ErrorOutlineIcon
                    style={{
                      fontSize: "1.125rem",
                      color: failedFileIds.length > 0 ? "#ef4444" : "var(--mantine-color-dimmed)",
                    }}
                  />
                }
                ...
                onClick={
                  failedFileIds.length > 0
                    ? () => {
                        setActivityStatusFilter(activityStatusFilter === "error" ? "all" : "error");
                        setExpandedActivityIds(new Set());
                      }
                    : undefined
                }
              ...
              {hasCompressStep && (
                <StatCard
                  icon={
                    <DownloadIcon
                      style={{ fontSize: "1.125rem", color: dataSavedBytes > 0 ? "#22c55e" : "var(--mantine-color-dimmed)" }}
                    />
                  }
          ...
          {(processingFileIds.length > 0 ||
            fileIds.filter((id) => folderRecord?.files[id]?.status === "pending").length > 0) &&
            (() => {
              ...
              return (
                <Box style={{ padding: "0 1rem 0.5rem", flexShrink: 0 }}>
                  <Box style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.25rem" }}>
                    {activeCount > 0 && (
                      <Text style={{ fontSize: "0.625rem", color: "var(--mantine-color-blue-filled)" }}>
          ...
          <Box style={{ padding: "1rem 1rem 0.5rem", flexShrink: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            {activityStatusFilter === "processed" && filteredActivityIds.length > 0 && (
              <>
                <button
                  onClick={() => void handleBatchDownload(filteredActivityIds)}
                  style={{
                    background: "none",
                    border: "0.0625rem solid var(--mantine-color-blue-filled)",
                    borderRadius: "0.25rem",
                ...
                <button
                  onClick={() => void handleBatchDownloadSeparate(filteredActivityIds)}
                  style={{
                    background: "none",
                    border: "0.0625rem solid var(--mantine-color-blue-filled)",
                    borderRadius: "0.25rem",
          ...
          {selectedActivityIds.size > 0 &&
            (() => {
              const hasAnyFailed = [...selectedActivityIds].some(
                (id) => folderRecord?.files[id]?.status === "error" && inputFiles.some((f) => f.fileId === id),
              );
              const hasDownloadable = [...selectedActivityIds].some(
                (id) => (activityOutputMap.get(id)?.length ?? 0) > 0 || inputFiles.some((f) => f.fileId === id),
              );
              ...
              return (
                ...
                >
                  <Text size="xs" fw={600} style={{ color: "var(--mantine-color-blue-filled)" }}>
                    {selectedActivityIds.size} selected
                  </Text>
                  {selectedActivityIds.size < filteredActivityIds.length && (
                    <button
                      onClick={() => setSelectedActivityIds(new Set(filteredActivityIds))}
                      style={{
                        background: "none",
                        border: "none",
                        cursor: "pointer",
                    ...
                    >
                      Select all {filteredActivityIds.length}
                    </button>
                  )}
                  <Box style={{ flex: 1 }} />
                  {showRetry && (
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {status === "error" && meta?.nextRetryAt && (
                            <ReplayIcon style={{ fontSize: "0.875rem", color: "#f59e0b", flexShrink: 0 }} />
                          )}
                          {status === "pending" && (
                            <Box
                              style={{
                          ...
                          {status === "error" && (meta?.failedAttempts ?? 0) > 0 && (
                            ...
                            >
                              <Text
                                style={{ fontSize: "0.625rem", fontWeight: 700, color: "#ef4444", letterSpacing: "0.03em" }}
                              >
                                {meta!.failedAttempts}×
                              </Text>
                            </Box>
                            ...
                            <Box style={{ display: "flex", alignItems: "center", gap: "0.125rem", flexShrink: 0 }}>
                              ...
                              {!isExpanded && !isServerFolder && primaryFile && (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    void handleDownload(primaryFile, primaryFile.name);
                                  }}
                                  title="Export"
                                >
                              ...
                              {!isExpanded && isServerFolder && hasPrimaryServerOutput && (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    const fn = serverOutputNames[0];
                                    const ext = fn.includes(".") ? fn.substring(fn.lastIndexOf(".")) : "";
                                    const base = filename.includes(".")
                                      ? filename.substring(0, filename.lastIndexOf("."))
                                      : filename;
                                    void handleServerOutputPreview(fn, base + ext);
                                  }}
                                  title="Preview"
                                >
                                  <VisibilityIcon style={{ fontSize: "0.875rem" }} />
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    ...
                                    const base = filename.includes(".")
                                      ? filename.substring(0, filename.lastIndexOf("."))
                                      : filename;
                                    void handleServerOutputDownload(fn, base + ext);
                                  }}
                                  title="Export from server"
                                >
                                  <DownloadIcon style={{ fontSize: "0.875rem" }} />
                                </button>
                              )}
                              <button
                                ...
                                title="Delete"
                              >
                                <DeleteOutlineIcon style={{ fontSize: "0.875rem" }} />
                              </button>
                            </Box>
                          )}
                        </Box>
                            ...
                            {inputFile && (
                              <Box
                                style={{
                                  ...
                                  gap: "0.5rem",
                                  padding: "0.2rem 0",
                                  marginBottom: outputs.length > 0 || status === "error" ? "0.25rem" : 0,
                                }}
                              >
                                <Text
                                  style={{
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      ...
                                      const base = filename.includes(".")
                                        ? filename.substring(0, filename.lastIndexOf("."))
                                        : filename;
                                      void handleServerOutputPreview(fname, base + ext);
                                    }}
                                    title="Preview"
                                  >
                                    <VisibilityIcon style={{ fontSize: "0.875rem" }} />
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      ...
                                      const base = filename.includes(".")
                                        ? filename.substring(0, filename.lastIndexOf("."))
                                        : filename;
                                      void handleServerOutputDownload(fname, base + ext);
                                    }}
                              ...
                              <Box style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem", paddingTop: "0.25rem" }}>
                                ...
                                {inputFile && (
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      updateFileMetadata(fileId, { status: "pending", errorMessage: undefined });
                                      runPipeline(
                                        folder!,
                                        inputFile,
                                        fileId,
              ...
              {(["24h", "7d", "30d", "all"] as const).map((p) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: statsPeriod === p ? "var(--mantine-color-blue-light-hover)" : "transparent",
                    color: statsPeriod === p ? "var(--mantine-color-blue-filled)" : "var(--mantine-color-dimmed)",
                    fontSize: "0.6875rem",
      ...
      >
        <Stack gap="lg" style={{ flex: 1, minHeight: 0 }}>
          ...
          <Group gap="xl">
            <Box>
              <Text style={{ fontSize: "0.625rem", letterSpacing: "0.05em" }} c="dimmed" tt="uppercase" fw={600}>
                Done
              </Text>
            ...
            return (
              <Box style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ flex: 1, minHeight: 0, display: "flex", gap: "0.5rem" }}>
                  ...
                  >
                    {yTicks.map((v, i) => (
                      <Text key={i} style={{ fontSize: "0.625rem", lineHeight: 1 }} c="dimmed">
                        {v}
                      </Text>
                    ))}
                  </div>
                      ...
                      {dashboardStats.buckets.map((b, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {b.processed > 0 && (
                              <div style={{ flex: b.processed, backgroundColor: "#22c55e", minHeight: "2px" }} />
                            )}
                          </div>
                        );
                      })}
                ...
                {/* X-axis labels */}
                <div
                  style={{ display: "flex", gap: "2px", marginTop: "0.25rem", paddingLeft: `calc(${yAxisWidth} + 0.5rem)` }}
                >
                  {dashboardStats.buckets.map((_, i) => (
                    <div key={i} style={{ flex: 1, textAlign: "center", overflow: "hidden" }}>
                      {i % stride === 0 && (
                        <Text style={{ fontSize: "0.6875rem", whiteSpace: "nowrap" }} c="dimmed">
                          {bucketLabel(i)}
                        </Text>
                      )}
                    </div>
                ...
                <Group gap="md" mt="0.625rem">
                  <Group gap="0.3rem">
                    <div style={{ width: "0.5rem", height: "0.5rem", borderRadius: "2px", backgroundColor: "#22c55e" }} />
                    <Text size="xs" c="dimmed">
                      Complete
                    </Text>
                  </Group>
                  <Group gap="0.3rem">
                    <div style={{ width: "0.5rem", height: "0.5rem", borderRadius: "2px", backgroundColor: "#ef4444" }} />
                    <Text size="xs" c="dimmed">
                      Failed
                    </Text>
                  </Group>
          ...
          >
            ...
            >
              <Text fw={600} mb="0.25rem">
                Remove {deleteConfirm.ids.length === 1 ? "entry" : `${deleteConfirm.ids.length} entries`}
              </Text>
              <Text size="sm" c="dimmed" mb="1rem">
                Remove notifications only clears the activity log. Delete outputs also removes the processed files from
                storage. Your original input files are never touched.
              </Text>
