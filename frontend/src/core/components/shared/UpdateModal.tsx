...
const UpdateModal: React.FC<UpdateModalProps> = ({
  ...
  const sortedVersions = fullUpdateInfo?.new_versions
    ? [...fullUpdateInfo.new_versions]
        .sort((a, b) => updateService.compareVersions(b.version, a.version))
    : [];
  // Show max 10 initially to keep the modal manageable
  const [showAllVersions, setShowAllVersions] = useState(false);
  const visibleVersions = showAllVersions ? sortedVersions : sortedVersions.slice(0, 10);
  ...
  return (
    ...
    >
      ...
      <Box style={{ padding: '24px 28px 16px', flexShrink: 0 }}>
        <Group justify="space-between" align="flex-start" wrap="nowrap">
          <Group gap="md" align="flex-start" wrap="nowrap">
            ...
            <Box>
              <Text fw={700} size="xl" lh={1.3}>{t('update.modalTitle', 'Update Available')}</Text>
              <Text size="sm" c="dimmed" mt={2}>{t('update.modalSubtitle', 'A new version of Stirling-PDF is ready to install.')}</Text>
            </Box>
          </Group>
          {canClose && (
            <CloseButton onClick={onClose} size="lg" variant="subtle" aria-label="Close update modal" />
          )}
        </Group>
      </Box>
      ...
      <Box style={{ flex: 1, overflowY: 'auto', padding: '0 28px 16px' }}>
        <Stack gap="lg">
          ...
          <Box style={{
            ...
            background: 'color-mix(in srgb, var(--mantine-color-body) 100%, transparent)',
          }}>
            <Group justify="center" align="center" wrap="nowrap" gap="xl">
              <Stack gap={2} align="center">
                <Text size="xs" c="dimmed" tt="uppercase" fw={600} lh={1}>{t('update.current', 'Current Version')}</Text>
                <Text fw={800} fz={32} lh={1.1}>{currentVersion}</Text>
              </Stack>
              <ArrowForwardIcon style={{ fontSize: 28, color: 'var(--mantine-color-dimmed)', flexShrink: 0 }} />
              <Stack gap={2} align="center">
                <Text size="xs" c="dimmed" tt="uppercase" fw={600} lh={1}>{t('update.latest', 'Latest Version')}</Text>
          ...
          {/* What's New card */}
          <Box style={{
            background: 'color-mix(in srgb, var(--mantine-color-blue-filled) 8%, transparent)',
            borderRadius: 12, padding: '16px 20px',
            border: '1px solid color-mix(in srgb, var(--mantine-color-blue-filled) 15%, transparent)',
          }}>
            <Group justify="space-between" align="center">
              <Group gap={8}>
                <StarIcon style={{ fontSize: 18, color: 'var(--mantine-color-blue-filled)' }} />
                <Text fw={600} size="sm">{t('update.whatsNewIn', "What's new in")} {updateSummary.latest_version}</Text>
              </Group>
              <Group gap="sm">
                <Text size="sm" component="a"
                  href={`https://github.com/Stirling-Tools/Stirling-PDF/releases/tag/v${updateSummary.latest_version}`}
                  target="_blank" c="blue"
                  style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  {t('update.releaseNotes', 'Release Notes')} <OpenInNewIcon style={{ fontSize: 14 }} />
                </Text>
                <Text size="sm" component="a"
                  href="https://github.com/Stirling-Tools/Stirling-PDF/releases"
                  target="_blank" c="dimmed"
                  style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  {t('update.allReleases', 'All Releases')} <OpenInNewIcon style={{ fontSize: 14 }} />
                </Text>
              </Group>
            </Group>
          </Box>
          ...
          {updateSummary.any_breaking && (
            <Alert variant="light" color="orange" radius="md"
              ...
              {t('update.breakingChangesMessage', 'Some versions contain breaking changes. Please review the migration guides below before updating.')}
            </Alert>
          )}
          ...
          {updateSummary.migration_guides && updateSummary.migration_guides.length > 0 && (
            <Box>
              <Text fw={600} size="sm" mb={4}>{t('update.migrationGuides', 'Migration Guides')}</Text>
              <Text size="xs" c="dimmed" mb="sm">{t('update.migrationGuidesDesc', 'Review important changes before updating.')}</Text>
              <Stack gap={0}>
                {updateSummary.migration_guides.map((guide, idx) => (
                  <Box key={idx} style={{
                    borderTop: idx === 0 ? '1px solid var(--border-subtle, var(--mantine-color-default-border))' : undefined,
                    borderBottom: '1px solid var(--border-subtle, var(--mantine-color-default-border))',
                    padding: '10px 12px',
                  }}>
                    <Group justify="space-between" align="center" wrap="nowrap">
                      <Group gap="sm" style={{ flex: 1 }}>
                        <Box style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--mantine-color-green-filled)', flexShrink: 0 }} />
                        <Text fw={600} size="sm">{guide.version}</Text>
                        <Text size="xs" c="dimmed" lineClamp={1}>{guide.notes}</Text>
                      </Group>
                      <Button component="a" href={guide.url} target="_blank" variant="default" size="xs"
                        rightSection={<OpenInNewIcon style={{ fontSize: 12 }} />}>
          ...
          {loading ? (
            <Center py="lg">
              <Group gap="sm">
                <Loader size="sm" />
                <Text size="sm" c="dimmed">{t('update.loadingDetailedInfo', 'Loading version details...')}</Text>
              </Group>
            </Center>
          ) : visibleVersions.length > 0 ? (
            <Box>
              <Group justify="space-between" align="center" mb="sm">
                <Text fw={600} size="sm">{t('update.versionHistory', 'Version History')}</Text>
                <Text size="xs" c="dimmed">{sortedVersions.length} {sortedVersions.length === 1 ? 'version' : 'versions'}</Text>
              </Group>
              <Stack gap={0}>
                {visibleVersions.map((version, index) => {
                  ...
                  return (
                    <Box key={index} style={{
                      borderTop: index === 0 ? '1px solid var(--border-subtle, var(--mantine-color-default-border))' : undefined,
                      borderBottom: '1px solid var(--border-subtle, var(--mantine-color-default-border))',
                    }}>
                      <Group
                      ...
                      >
                        <Group gap="sm" style={{ flex: 1 }}>
                          <Text fw={700} size="sm" style={{ minWidth: 50 }}>{version.version}</Text>
                          <Badge color={getPriorityColor(version.priority)} size="xs" variant="light">
                            {getPriorityLabel(version.priority)}
                          </Badge>
                          {version.compatibility.breaking_changes && (
                            <Badge color="orange" size="xs" variant="light">{t('update.breaking', 'Breaking')}</Badge>
                          )}
                          {!isExpanded && version.announcement?.title && (
                            <Text size="xs" c="dimmed" lineClamp={1} style={{ flex: 1 }}>{version.announcement.title}</Text>
                          )}
                        </Group>
                        <Group gap={4}>
                          <Button component="a"
                            href={`https://github.com/Stirling-Tools/Stirling-PDF/releases/tag/v${version.version}`}
                            target="_blank" variant="subtle" size="xs" px={6}
                            onClick={(e) => e.stopPropagation()}
                            rightSection={<OpenInNewIcon style={{ fontSize: 11 }} />}>
                            {t('update.notes', 'Notes')}
                          </Button>
                          {isExpanded
                            ? <ExpandLessIcon style={{ fontSize: 18, color: 'var(--mantine-color-dimmed)' }} />
                            : <ExpandMoreIcon style={{ fontSize: 18, color: 'var(--mantine-color-dimmed)' }} />}
                        </Group>
                      </Group>
                      <Collapse in={isExpanded}>
                        <Box px="sm" pb="sm" style={{ borderTop: '1px solid var(--border-subtle, var(--mantine-color-default-border))' }}>
                          <Stack gap="sm" mt="sm">
                            {version.announcement?.message && (
                              <Text size="sm" c="dimmed" style={{ lineHeight: 1.6 }}>{version.announcement.message}</Text>
                            )}
                            {version.compatibility.breaking_changes && (
                              <Alert variant="light" color="orange" radius="sm"
                                icon={<WarningAmberIcon style={{ fontSize: 16 }} />}
                                title={t('update.breakingChanges', 'Breaking Changes')}>
                                <Text size="sm">
                                  {version.compatibility.breaking_description || t('update.breakingChangesDefault', 'This version contains breaking changes.')}
                                </Text>
                                {version.compatibility.migration_guide_url && (
                                  <Button component="a" href={version.compatibility.migration_guide_url} target="_blank"
                                    variant="light" color="orange" size="xs" mt="xs"
              ...
              {sortedVersions.length > 10 && (
                <Center mt="sm">
                  <Button variant="subtle" size="xs" onClick={() => setShowAllVersions(!showAllVersions)}>
                    {showAllVersions
                      ? t('update.showLess', 'Show fewer versions')
                      : t('update.showMore', { defaultValue: 'Show all {{count}} versions', count: sortedVersions.length })}
                  </Button>
                </Center>
              )}
            </Box>
          ...
          {desktopInstall && desktopInstall.state !== 'idle' && (
            <Box style={{
              ...
              {(desktopInstall.state === 'downloading' || desktopInstall.state === 'installing') && (
                <Stack gap="sm">
                  <Group justify="space-between" align="center">
                    <Text size="sm" fw={600}>
                    ...
                    {desktopInstall.progress && desktopInstall.progress.total !== null && (
                      <Text size="xs" c="dimmed">
                        {formatBytes(desktopInstall.progress.downloaded)} / {formatBytes(desktopInstall.progress.total)}
                      </Text>
                    )}
                  </Group>
                  <Progress
                    value={desktopInstall.state === 'installing' ? 100 : (desktopInstall.progress?.percent ?? 0)}
                    size="lg" animated radius="xl"
                  />
                  {desktopInstall.state === 'installing' && (
                    <Alert variant="light" color="blue" radius="sm" py="xs">
                      <Text size="xs">{t('desktopUpdate.installingWarning', 'The app will close automatically to complete the installation.')}</Text>
                    </Alert>
                  )}
                </Stack>
              )}
              {desktopInstall.state === 'ready-to-restart' && (
                <Alert icon={<CheckCircleOutlineIcon />} color="green" variant="light" radius="md"
                  title={t('desktopUpdate.readyToRestart', 'Update Ready')}>
                  {t('desktopUpdate.restartMessage', 'The update has been installed. Restart the app to finish.')}
                </Alert>
              )}
              {desktopInstall.state === 'error' && (
                <Alert icon={<ErrorOutlineIcon />} color="red" variant="light" radius="md"
                  title={t('desktopUpdate.updateFailed', 'Update Failed')}>
                  {desktopInstall.errorMessage ?? t('desktopUpdate.updateFailedMessage', 'Failed to download or install the update.')}
                </Alert>
              )}
            </Box>
          )}
      ...
      {/* ── Sticky footer ──────────────────────────────────────────────────── */}
      <Box style={{
        borderTop: '1px solid var(--border-subtle, var(--mantine-color-default-border))',
        padding: '16px 28px', flexShrink: 0,
      }}>
        <Group justify="flex-end" gap="sm">
            <Button variant="default" onClick={handleLater} disabled={!canClose} radius="md" size="md">
