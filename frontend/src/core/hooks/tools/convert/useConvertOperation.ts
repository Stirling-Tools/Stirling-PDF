...
export const shouldProcessFilesSeparately = (selectedFiles: File[], parameters: ConvertParameters): boolean => {
  return (
    ...
    (((isImageFormat(parameters.fromExtension) || parameters.fromExtension === "image") &&
      ...
      ((isWebFormat(parameters.fromExtension) || parameters.fromExtension === "web") && parameters.toExtension === "pdf") ||
...
export const convertProcessor = async (
  ...
  if (isSeparateProcessing) {
    ...
    const baseFilename = selectedFiles.length === 1 ? selectedFiles[0].name : "converted_files";
...
export const useConvertOperation = (parameters?: ConvertParameters) => {
  ...
  const customConvertProcessor = useCallback(
    async (parameters: ConvertParameters, selectedFiles: File[]): Promise<CustomProcessorResult> => {
