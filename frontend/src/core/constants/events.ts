...
export function requestStartTour(tourType: TourType): void {
  ...
  window.dispatchEvent(new CustomEvent<StartTourPayload>(START_TOUR_EVENT, { detail: { tourType } }));
}
...
export function dispatchTourState(isOpen: boolean): void {
  if (typeof window === "undefined") return;
  window.dispatchEvent(new CustomEvent<TourStatePayload>(TOUR_STATE_EVENT, { detail: { isOpen } }));
}
