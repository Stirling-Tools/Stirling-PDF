...
const SIGNATURE_TOOL_DEFAULT_SOURCES: SignatureSource[] = ['canvas', 'image', 'text', 'saved'];

export const createSignatureTool = (config: SignatureToolConfig) => {
  const {
    toolId,
    translationScope = toolId,
    allowedSignatureSources = SIGNATURE_TOOL_DEFAULT_SOURCES,
    defaultSignatureSource,
    defaultSignatureType,
  } = config;

  const SignatureTool = (props: BaseToolProps) => {
    ...
    const getSteps = () => {
      ...
      if (base.selectedFiles.length > 0) {
        steps.push({
          title: translateTool('steps.configure', 'Configure Signature'),
          isCollapsed: false,
    ...
    return createToolFlow({
      ...
      review: {
        ...
        title: translateTool('results.title', 'Signature Results'),
  ...
  const SignatureToolComponent = SignatureTool as ToolComponent;
  SignatureToolComponent.tool = () => useSignOperation;
  SignatureToolComponent.getDefaultParameters = () => ({
    ...DEFAULT_PARAMETERS,
    signatureType: config.defaultSignatureType ?? DEFAULT_PARAMETERS.signatureType,
  });

  return SignatureToolComponent;
};

const Sign = createSignatureTool({
  toolId: 'sign',
