...
test.describe('18. Cookie Preferences', () => {
  ...
  test.describe('18.1 Cookie Banner', () => {
    test('should open and configure cookie preferences from footer', async ({ page }) => {
      // Step 1: Locate the "Cookie Preferences" button in the footer
      const cookieButton = page.locator('#cookieBanner, button:has-text("Preferensi Cookie"), button:has-text("Cookie Preferences")').first();

      await page.waitForTimeout(1000);
      const isVisible = await cookieButton.isVisible().catch(() => false);
      if (!isVisible) {
        test.skip(true, 'Cookie Preferences button not visible — analytics may be disabled');
        return;
      }
      ...
      const okeBtn = ccMain.locator('button:has-text("Oke"), button:has-text("OK")').first();
      const noThanksBtn = ccMain.locator('button:has-text("Tidak, terima kasih"), button:has-text("No Thanks")').first();
      const manageBtn = ccMain.locator('button:has-text("Kelola preferensi"), button:has-text("Manage preferences")').first();
      ...
      if (hasManage) {
        ...
        const necessaryCategory = ccMain.locator('text=/Cookie yang Sangat Diperlukan|Strictly Necessary|necessary/i').first();
        const analyticsCategory = ccMain.locator('text=/Analitik|Analytics/i').first();
        await expect(necessaryCategory.or(analyticsCategory).first()).toBeVisible({ timeout: 3000 });

        // Click "Terima semua" (Accept all) or "Simpan preferensi" (Save preferences) to close
        const acceptAllBtn = ccMain.locator('button:has-text("Terima semua"), button:has-text("Accept all")').first();
        const savePrefBtn = ccMain.locator('button:has-text("Simpan preferensi"), button:has-text("Save preferences")').first();
