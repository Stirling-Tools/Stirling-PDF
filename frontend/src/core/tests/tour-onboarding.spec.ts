...
test.describe('15. Tour/Onboarding', () => {
  ...
  test.describe('15.2 Tour Tooltip', () => {
    test('should show and dismiss tour tooltip', async ({ page }) => {
      ...
      const tooltipContent = page.locator('[data-radix-popper-content-wrapper], [role="tooltip"]').first();
      ...
      await expect(page.getByPlaceholder(/search|cari/i).first()).toBeVisible({ timeout: 10000 });
    });
  });
});
