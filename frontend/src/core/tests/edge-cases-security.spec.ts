...
import { loginAndSetup, ensureCookieConsent, dismissWelcomeDialog, dismissCookieConsent } from '@app/tests/helpers/login';
...
test.describe('20. Edge Cases and Security', () => {
  test.describe('20.1 Concurrent Sessions', () => {
    test('should invalidate session across tabs on logout', async ({ browser, baseURL }) => {
      ...
      await page1.locator('#email').waitFor({ state: 'visible', timeout: 15000 });
      ...
      await page1.getByRole('button', { name: /settings/i }).first().click();
      ...
      if (await accountNav.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        // Click Log out
        await page1.getByText(/Log out/i).first().click();
  ...
  test.describe('20.5 Empty File Upload', () => {
    test('should handle empty files gracefully', async ({ page }) => {
      ...
      try {
        ...
        try { fs.unlinkSync(emptyFile); } catch { /* ignore */ }
      }
    });
  });

  test.describe('20.6 API Documentation Link', () => {
    test('should navigate to Swagger API documentation', async ({ page, context }) => {
      await loginAndSetup(page);
