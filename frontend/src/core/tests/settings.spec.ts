...
test.describe('12. Settings', () => {
  ...
  test.describe('12.1 Settings - General Preferences', () => {
    test('should open settings dialog with all configuration categories', async ({ page }) => {
  ...
  test.describe('12.2 Settings - Account Management', () => {
    test('should display current user info and management options', async ({ page }) => {
      ...
      if (await accountNav.first().isVisible({ timeout: 3000 }).catch(() => false)) {
  ...
  test.describe('12.3 Settings - Logout', () => {
    test('should invalidate session and protect routes after logout', async ({ page }) => {
  ...
  test.describe('12.4 Settings - Close Dialog', () => {
    test('should close settings dialog cleanly', async ({ page }) => {
      ...
      await expect(page.locator('[data-tour="quick-access-bar"]').first()).toBeVisible();
    });
  });
  ...
  test.describe('12.5 Settings - Search Within Settings', () => {
    test('should filter settings based on search input', async ({ page }) => {
      ...
      const settingsSearch = page.locator('.mantine-Modal-content input[role="searchbox"], .mantine-Modal-content input[type="search"], .mantine-Modal-content .mantine-Select-input, .mantine-Modal-content .mantine-Combobox-input').first();

      // Step 2: If a search field is visible, type a search term and verify filtering
      if (await settingsSearch.isVisible({ timeout: 3000 }).catch(() => false)) {
