...
test.describe('5. Merge Tool', () => {
  ...
  test.describe('5.1 Merge - Page Structure', () => {
    test('should display the correct multi-step workflow', async ({ page }) => {
      ...
      await expect(page.locator('text=/merge|menggabungkan/i').first()).toBeVisible();

      // Step 2: Verify a 3-step workflow is displayed (Files, Sort Files, Settings)
      await expect(page.locator('text=/^Files$|^File$/i').first()).toBeVisible({ timeout: 10000 });
      await expect(page.locator('text=/Sort Files|Sort/i').first()).toBeVisible();
      await expect(page.locator('text=/Settings|Pengaturan/i').first()).toBeVisible();

      // Step 3: Verify the "Merge" button is present and disabled
      const mergeButton = page.getByRole('button', { name: /merge|gabungkan/i }).first();
