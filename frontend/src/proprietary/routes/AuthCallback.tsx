...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      // Check if we're still stuck in validating state
      if (status === 'validating' && !session) {
        console.error('[AuthCallback] Validation timeout - no session after 10 seconds');
        setStatus('error');
        ...
        {status === 'error' ? (
          ...
          <>
            ...
            <div className="text-gray-600">
              {status === 'validating' ? 'Validating session...' : 'Completing authentication...'}
            </div>
          </>
        )}
      </div>
