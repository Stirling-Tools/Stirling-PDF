...
// it is only typechecked standalone via src/editor/cloud/tsconfig.json
...
export default defineConfig(async ({ mode, command }) => {
  ...
  // Load env files relative to this config (frontend/), regardless of
  ...
  // frontend/ the cwd-based lookup would miss .env*.
  ...
  return {
    ...
    // root (frontend/) and would create a second node_modules there.
    cacheDir: resolve(
      import.meta.dirname,
      "node_modules",
      `.vite-${effectiveMode}`,
      ...
      viteStaticCopy({
        targets: [
          ...
          },
          {
            ...
            src: "src/editor/core/assets/brand/modern-logo/*",
            dest: "modern-logo",
          },
          {
