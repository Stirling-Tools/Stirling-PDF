/** Which icons source references. A heuristic, and only ever a report: nothing is dropped on its say-so. */
...
/** The names src/editor/core/icons/icons.ts maps. */
export function iconNames(file: string): string[] {
  const src = fs.readFileSync(file, "utf8");
  return [...src.matchAll(/^ {2}"?([a-z0-9][a-z0-9-]*)"?: \{/gm)].map(
    (m) => m[1],
  );
}
...
export function scanUsedIcons(
  ...
  const iconsDir = path.join(srcDir, "editor/core/icons");
