...
//
// --icon is a registry icon name (a lucide icon, or one of src/editor/core/icons/svg/stirling),
...
import { STROKE_WIDTH } from "../src/editor/core/icons/icons.config.ts";
...
const ICON_DIRS = [
  ...
  path.join(ROOT, "src/editor/core/icons/svg/stirling"),
];
...
export async function resolveIcon(icon) {
  ...
  if (!file)
    throw new Error(
      `icon not found in the registry (lucide or src/editor/core/icons/svg/stirling): "${icon}"`,
    );
...
function readRegistryStrings() {
  const src = readFileSync(
    path.join(ROOT, "src/editor/core/data/useTranslatedToolRegistry.tsx"),
    "utf8",
  );
  const STR = '"((?:[^"\\\\]|\\\\.)*)"';
  const titles = {},
...
export async function generateAll(theme = THEME) {
  ...
  const ogMap = JSON.parse(
    readFileSync(
      path.join(ROOT, "src/editor/core/data/ogImageMap.json"),
      "utf8",
    ),
  );
  const results = [];
