...
export {
  ...
  },
  {
    ...
    replace: [
      ...
      "  // budget is enforced while reading instead of after the whole output exists.",
      ...
      "    if (total > maxBytes) {",
      "      await reader.cancel();",
      '      throw new Error("Layer inflate budget exceeded");',
      "    }",
      "    chunks.push(value);",
      "  }",
      ...
      "      try {",
      "        decoded = await __stirlingInflate(",
      "          raw,",
      "          __STIRLING_LAYER_INFLATE_BUDGET - inflatedBytes",
      "        );",
      "      } catch {",
      "        return null;",
      "      }",
  ...
  },
  {
    ...
    replace: [
      ...
      "        return 1;",
      "        } catch (readError) {",
      "          // FPDF_FILEACCESS reports a failed read with 0; throwing would",
      "          // unwind through the PDFium frames with no C++ cleanup.",
      "          return 0;",
      "        }",
