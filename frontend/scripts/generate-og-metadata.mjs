...
//
// Outputs:
//   src/editor/core/data/ogImageMap.json      - { toolId: imageBasename }  (imported by the client)
//   src/editor/core/data/urlSeoOverrides.json - { path: { title, description } } (client per-URL meta, English only)
...
// --- parse tool ids ---------------------------------------------------------
const idSrc = read("src/editor/core/types/toolId.ts");
...
// --- parse URL aliases ------------------------------------------------------
const mapSrc = read("src/editor/core/utils/urlMapping.ts");
const urlToTool = {};
for (const m of mapSrc.matchAll(/"([^"]+)":\s*"([^"]+)"/g))
  urlToTool[m[1]] = m[2];

// --- parse English title/description fallbacks ------------------------------
const regSrc = read("src/editor/core/data/useTranslatedToolRegistry.tsx");
const titleById = {};
const descById = {};
const STR = '"((?:[^"\\\\]|\\\\.)*)"';
...
const navKeys = (
  read("src/editor/core/components/shared/config/types.ts")
...
const mapPath = "src/editor/core/data/ogImageMap.json";
const manifestPath = "public/og-metadata.json";
const saasManifestPath = "public/og-metadata.saas.json";
const seoOverridesPath = "src/editor/core/data/urlSeoOverrides.json";

const outputs = [
  [mapPath, mapJson],
  [manifestPath, manifestJson],
  [saasManifestPath, saasManifestJson],
