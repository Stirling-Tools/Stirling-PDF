...
const ICONS_DIR = path.join(SRC, "editor/core/icons");
...
const SVG_ALLOWED = [
  ICON_SVG_DIR,
  path.join(SRC, "editor/core/assets"),
  path.join(SRC, "assets"),
  path.join(SRC, "editor/core/tests"),
];
...
if (citing.length && !fs.existsSync(licenceFile)) {
  problems.push(
    `src/editor/core/icons/LICENSE-lucide.txt is missing, but ${citing.join(", ")} ` +
...
if (lucidePath.size) {
  for (const svg of fs
    ...
    if (copied.length && !raw.includes("LICENSE-lucide.txt")) {
      problems.push(
        `src/editor/core/icons/svg/stirling/${svg}: undeclared lucide geometry ` +
          `(from ${[...new Set(copied.map((d) => lucidePath.get(d)))].join(", ")}). ` +
          `Add a comment citing src/editor/core/icons/LICENSE-lucide.txt, or redraw it.`,
      );
    }
  }
}
...
for (const file of files) {
  ...
  if (file.endsWith(".svg")) {
    if (!SVG_ALLOWED.some((dir) => file.startsWith(dir))) {
      problems.push(
        `${rel(file)}: .svg outside src/editor/core/icons/svg/. Icons belong there so ` +
  ...
  lines.forEach((line, i) => {
    ...
    if (MIGRATION_COMPLETE && /<svg[\s>]/.test(line) && !inIconsDir) {
      ...
      if (!OPT_OUT.test(context)) {
        problems.push(
          `${rel(file)}:${i + 1}: inline <svg>. Move it to src/editor/core/icons/svg/ and render ` +
    ...
    if (!known.has(m[1])) {
      problems.push(
        `${rel(file)}: <Icon name="${m[1]}" /> is not in the icon map. Add it to ` +
          `src/editor/core/icons/icons.ts, or fix the name.`,
      );
    }
  }
