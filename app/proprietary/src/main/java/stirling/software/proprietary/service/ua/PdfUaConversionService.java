...
public class PdfUaConversionService {
    ...
    public PdfUaConversionOutcome convert(byte[] input, TaggingOptions options) throws IOException {
        if (input.length > MAX_INPUT_BYTES) {
            throw new IOException("This PDF is "
        ...
        if (validation.compliant() && honest) {
            ...
            return new PdfUaConversionOutcome(declared, true, validation, summary(taggingResult), warnings);
        }
        ...
        if (!validation.compliant()) {
            warnings.add("The document could not be made "
                    + profile.displayName()
        ...
        log.info(
                ...
                validation.totalFailures(),
                !honest);
        return new PdfUaConversionOutcome(undeclared, false, validation, summary(taggingResult), warnings);
    }
    ...
    private static PdfUaConversionOutcome.TaggingSummary summary(TaggingResult result) {
        return new PdfUaConversionOutcome.TaggingSummary(
                result.isRebuilt(), result.getTaggedElements(), result.getArtifacts(), result.getFiguresNeedingAlt());
    }
        ...
        try {
            ...
            if (mentionsPassword(e)) {
                throw new IOException(
                        "This PDF is encrypted. Remove the password before converting it to" + " PDF/UA.", e);
            }
            throw e;
        }
    }
    ...
    private byte[] keepTagsOverFonts(
            byte[] input, byte[] embedded, SourceFacts facts, TaggingOptions options, List<String> warnings)
            throws IOException {
        ...
        warnings.add("Embedding the missing fonts would have deleted the document's existing tags, so"
    ...
    private static void rejectUnsupportedSource(PDDocument document) throws IOException {
        if (document.getNumberOfPages() > MAX_PAGES) {
            throw new IOException("This PDF has "
        ...
        if (form != null && form.xfaIsDynamic()) {
            throw new IOException("Dynamic XFA forms are not permitted by PDF/UA. Flatten the form first.");
        }
    }
    ...
    private static void rejectEncrypted(PDDocument document) throws IOException {
        if (document.isEncrypted()) {
            throw new IOException("Encrypted PDFs cannot be converted to PDF/UA. Remove the password first.");
        }
    }
