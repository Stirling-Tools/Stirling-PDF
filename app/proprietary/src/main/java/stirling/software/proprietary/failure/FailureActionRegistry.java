...
public class FailureActionRegistry {
    ...
    public FailureActionRegistry(List<FailureAction> actions) {
        for (FailureAction action : actions) {
            ...
            if (clash != null) {
                throw new IllegalStateException("Two handlers registered for action "
                        + action.id()
                        + ": "
    ...
    void verifyEveryDeclaredActionHasAHandler() {
        List<String> gaps = Arrays.stream(FailureKind.values())
                .flatMap(kind -> kind.getActions().stream()
                        .filter(FailureActionId::runsOnServer)
                        .filter(action -> !byId.containsKey(action))
                        .map(action -> kind.getId() + " -> " + action))
                .toList();
        if (!gaps.isEmpty()) {
            throw new IllegalStateException("Failure kinds declare actions with no registered handler: " + gaps);
        }
        log.debug(
