...
@Repository
public interface ApiKeyDailyUsageRepository extends JpaRepository<ApiKeyDailyUsage, ApiKeyDailyUsageId> {

    /** Atomically bump today's tally; returns 0 when no row exists yet (caller then inserts). */
    @Modifying
    @Query("UPDATE ApiKeyDailyUsage u SET u.count = u.count + 1 "
    ...
    @Query("SELECT COALESCE(SUM(u.count), 0) FROM ApiKeyDailyUsage u "
            + "WHERE u.apiKeyId = :apiKeyId AND u.epochDay >= :fromDayInclusive")
    long sumSince(@Param("apiKeyId") Long apiKeyId, @Param("fromDayInclusive") long fromDayInclusive);

    @Query("SELECT u.count FROM ApiKeyDailyUsage u " + "WHERE u.apiKeyId = :apiKeyId AND u.epochDay = :epochDay")
    ...
    @Query("SELECT u.apiKeyId AS apiKeyId, u.count AS total FROM ApiKeyDailyUsage u "
            + "WHERE u.apiKeyId IN :ids AND u.epochDay = :epochDay")
    List<ApiKeyUsageSum> countForDayByIds(@Param("ids") Collection<Long> ids, @Param("epochDay") long epochDay);

    /** Batched trailing-window sum for many keys in one query. */
    @Query("SELECT u.apiKeyId AS apiKeyId, SUM(u.count) AS total FROM ApiKeyDailyUsage u "
