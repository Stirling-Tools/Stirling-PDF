...
public class TaggingOptions {
    ...
    @Builder.Default
    private PdfUaProfile profile = PdfUaProfile.UA1;
    ...
    @Builder.Default
    private boolean overrideLanguage = false;
    ...
    @Builder.Default
    private boolean preservePdfVersion = false;

    @Builder.Default
    private ExistingTags existingTags = ExistingTags.AUTO;

    @Builder.Default
    private FigurePolicy figurePolicy = FigurePolicy.REQUIRE_ALT;

    /** Alternative descriptions supplied by the caller, keyed by "pageIndex:ordinal". */
    @Builder.Default
    private Map<String, String> altTextByFigure = Map.of();

    /** What the document said before font embedding rewrote it; see {@link SourceFacts}. */
    @Builder.Default
    private SourceFacts sourceFacts = SourceFacts.NONE;
