...
public class StorageEncryptionAdminController {
    ...
    public StorageEncryptionStatusResponse status() {
        ...
        try {
            ...
            throw new ResponseStatusException(
                    HttpStatus.SERVICE_UNAVAILABLE, "The storage encryption key registry could not be read", e);
        }
    }

    private StorageEncryptionStatusResponse buildStatus() {
        List<StorageEncryptionStatusResponse.KeyInfo> keys = keyRepository.findAll(Sort.by("createdAt")).stream()
                .map(k -> new StorageEncryptionStatusResponse.KeyInfo(
                        k.getKeyId(),
                        k.getScopeType().name(),
                        k.getScopeId(),
                        k.getKeyVersion(),
        ...
        long pendingRotationRows =
                masterKeyVersion == null ? 0 : keyRepository.countByMasterKeyVersionLessThan(masterKeyVersion);
    ...
    public StorageEncryptionStatusResponse.KeyInfo disableKey(@PathVariable UUID keyId) {
        ...
        auditService.audit(
                AuditEventType.STORAGE_ENCRYPTION, Map.of("action", "key.disabled", "keyId", keyId.toString()));
    ...
    public StorageEncryptionStatusResponse.KeyInfo enableKey(@PathVariable UUID keyId) {
        FileEncryptionKey existing = keyRepository
                .findById(keyId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No such encryption key"));
    ...
    public MigrationStatusResponse migrationStatus() {
        return migrationService.status().map(MigrationStatusResponse::from).orElse(MigrationStatusResponse.IDLE);
    }
    ...
    private FileEncryptionKeyService requireKeyService() {
        try {
            ...
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT, "Storage encryption is not configured: " + e.getMessage(), e);
        }
    }
    ...
    public record MigrationStatusResponse(
        ...
        static MigrationStatusResponse from(StorageEncryptionMigrationService.MigrationStatus s) {
            return new MigrationStatusResponse(
                    s.state().name(), s.total(), s.processed(), s.skipped(), s.failed(), s.startedAt(), s.finishedAt());
        }
    }
}
