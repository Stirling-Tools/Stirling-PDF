...
public interface FileRunEventRepository extends JpaRepository<FileRunEventEntity, String> {
     ...
     */
    @Query("select e from FileRunEventEntity e where ((:teamId is null and e.teamId is null) or"
    ...
    @Transactional
    @Query("update FileRunEventEntity e set e.occurrences = e.occurrences + 1,"
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query("update FileRunEventEntity e set"
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query("update FileRunEventEntity e set e.status = :target, e.statusActor = :actor,"
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query("update FileRunEventEntity e set e.status ="
            + " stirling.software.proprietary.failure.FileRunEventStatus.FILE_REMOVED,"
    ...
    @Query("select e from FileRunEventEntity e where e.id = :id and ((:teamId is null and e.teamId"
            + " is null) or e.teamId = :teamId)")
    Optional<FileRunEventEntity> findByIdAndTeam(@Param("id") String id, @Param("teamId") Long teamId);
}
