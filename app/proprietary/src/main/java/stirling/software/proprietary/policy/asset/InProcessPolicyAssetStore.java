...
public class InProcessPolicyAssetStore implements PolicyAssetStore {
    ...
    public PolicyAsset save(PolicyAsset asset, byte[] content) {
        String id =
                asset.id() == null || asset.id().isBlank() ? UUID.randomUUID().toString() : asset.id();
        PolicyAsset stored = new PolicyAsset(
                id,
                asset.fileName(),
                asset.contentType(),
                content.length,
    ...
    private static Comparator<PolicyAsset> newestFirst() {
        return Comparator.comparingLong(PolicyAsset::createdAt).reversed().thenComparing(PolicyAsset::id);
    }
}
