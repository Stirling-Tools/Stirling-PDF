...
public class AccountLinkController {
    ...
    public ResponseEntity<?> connectStart(
        ...
        try {
            return ResponseEntity.ok(connectService.start(req != null ? req.name() : null, callbackHint(req, http)));
        } catch (AccountLinkClient.UpstreamException e) {
            log.warn("Account-link connect rejected upstream: HTTP {}", e.status());
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY).body(java.util.Map.of("error", "CONNECT_FAILED"));
        } catch (IOException e) {
    ...
    private static ConnectService.CallbackHint callbackHint(ConnectStartRequest req, HttpServletRequest http) {
