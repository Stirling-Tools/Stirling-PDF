...
public class FileEncryptionKeyService {
    ...
    public FileEncryptionKeyService(FileEncryptionKeyRepository repository, FileEncryptionMasterKey masterKey) {
        this(repository, masterKey, StorageEncryptionAuditListener.NOOP, TransactionOperations.withoutTransaction());
    }
    ...
    public byte[] kekForDecrypt(UUID keyId) throws StorageEncryptionException {
        ...
        FileEncryptionKey row = repository.findById(keyId).orElseThrow(() -> {
            auditListener.decryptDenied(keyId, "key not found");
            return new StorageEncryptionException("No encryption key "
                    + keyId
    ...
    public FileEncryptionKey setKeyStatus(UUID keyId, FileEncryptionKey.Status status, String actor)
        ...
        FileEncryptionKey row = repository
                .findById(keyId)
                .orElseThrow(() -> new StorageEncryptionException("No encryption key " + keyId));
    ...
    public FileEncryptionKey enable(UUID keyId, String actor) throws StorageEncryptionException {
        FileEncryptionKey row = repository
                .findById(keyId)
                .orElseThrow(() -> new StorageEncryptionException("No encryption key " + keyId));
        boolean scopeHasAnotherActiveKey = activeForScope(row.getScopeType(), row.getScopeId())
                .filter(other -> !other.getKeyId().equals(keyId))
                .isPresent();
        FileEncryptionKey.Status target =
                scopeHasAnotherActiveKey ? FileEncryptionKey.Status.RETIRED : FileEncryptionKey.Status.ACTIVE;
    ...
    public int rotateMasterKey() throws StorageEncryptionException {
        int rewrapped = 0;
        for (FileEncryptionKey row : repository.findByMasterKeyVersionLessThan(masterKey.currentVersion())) {
            byte[] kek = unwrapRow(row);
    ...
    private Optional<FileEncryptionKey> activeForScope(FileEncryptionKey.ScopeType scopeType, long scopeId) {
