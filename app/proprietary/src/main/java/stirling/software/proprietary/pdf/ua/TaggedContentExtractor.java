...
public class TaggedContentExtractor {
    ...
    private static boolean isGraphicOperator(String name, List<COSBase> operands, PDResources resources) {
        if ("BI".equals(name)) {
            return true;
        }
    ...
    static BBox toBBox(PDRectangle rect) {
        return new BBox(rect.getLowerLeftX(), rect.getLowerLeftY(), rect.getUpperRightX(), rect.getUpperRightY());
    }
    ...
    private static MarkableOp classify(
            String name, List<COSBase> operands, PDResources resources, Matrix ctm, BBox pathBox, int ordinal) {
        ...
        COSName resourceName = operands.size() == 1 && operands.get(0) instanceof COSName n ? n : null;
        ...
        try {
            ...
            if (xobject instanceof PDImageXObject) {
                return new MarkableOp(ordinal, MarkableOp.Kind.IMAGE, unitSquare(ctm), resourceName.getName());
            }
            if (xobject instanceof PDFormXObject form) {
                return new MarkableOp(ordinal, MarkableOp.Kind.FORM, formBox(form, ctm), resourceName.getName());
            }
        } catch (IOException e) {
            log.debug("Could not resolve XObject {}: {}", resourceName.getName(), e.getMessage());
        }
        return new MarkableOp(ordinal, MarkableOp.Kind.FORM, unitSquare(ctm), resourceName.getName());
    }
    ...
    private static Float numberAt(List<COSBase> operands, int index) {
        return index < operands.size() && operands.get(index) instanceof org.apache.pdfbox.cos.COSNumber number
                ? number.floatValue()
                : null;
    }
    ...
    private static final class LineCollector extends PDFTextStripper {
        ...
        protected void processOperator(Operator operator, List<COSBase> operands) throws IOException {
            ...
            if (nestedDepth == 0 && syntheticDepth == 0) {
                if (isPathConstruction(name)) {
                    pathBox = extendPath(
                            pathBox, name, operands, getGraphicsState().getCurrentTransformationMatrix());
                }
        ...
        protected void showType3Glyph(
                Matrix textRenderingMatrix, PDType3Font font, int code, org.apache.pdfbox.util.Vector displacement)
                throws IOException {
            nestedDepth++;
            try {
        ...
        @Override
        protected void processChildStream(org.apache.pdfbox.contentstream.PDContentStream contentStream, PDPage page)
                throws IOException {
            nestedDepth++;
            try {
        ...
        private WordInfo buildWord(String text, List<TextPosition> positions) {
            ...
            return new WordInfo(text, bounds.box(), bounds.start, bounds.end, bounds.dominantSize(), bounds.bold);
        }
    ...
    private static final class Bounds {
        ...
        private static boolean isBold(TextPosition tp) {
            ...
            return descriptor != null && (descriptor.getFontWeight() >= 600 || descriptor.isForceBold());
        }
    }
}
