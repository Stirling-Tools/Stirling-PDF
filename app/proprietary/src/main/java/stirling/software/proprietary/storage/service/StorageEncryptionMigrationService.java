...
public class StorageEncryptionMigrationService {
    ...
    public MigrationStatus start() {
        if (!encryptionState.isWriteEnabled()) {
            throw new IllegalStateException("storage.encryption.enabled must be on before migrating existing files");
        }
    ...
    private State migratePages(Run run) throws InterruptedException {
        long lastId = 0;
        while (true) {
            List<StoredFile> page = storedFileRepository.findMigratableAfter(lastId, PageRequest.of(0, PAGE_SIZE));
            if (page.isEmpty()) {
            ...
            for (StoredFile file : page) {
                ...
                try {
                    ...
                    log.error("Failed to encrypt stored file {} (key {})", file.getId(), file.getStorageKey(), e);
                }
            }
    ...
    private void migrateFile(StoredFile file, Run run) throws IOException {
        ...
        StoredObject encrypted =
                reencrypt(owner, oldKey, file.getOriginalFilename(), file.getContentType(), file.getSizeBytes());
        ...
        if (storedFileRepository.swapMainBlob(
                        file.getId(), oldKey, encrypted.getStorageKey(), encrypted.getEncryptionKeyId())
                == 1) {
            deleteQuietly(oldKey);
    ...
    private StoredObject reencrypt(User owner, String storageKey, String filename, String contentType, Long sizeBytes)
            throws IOException {
        ...
        return storageProvider.store(owner, new ResourceUpload(plaintext, filename, contentType, size));
    }
    ...
    private static final class Run {
        ...
        private MigrationStatus snapshot() {
            return new MigrationStatus(
                    state, total, processed.get(), skipped.get(), failed.get(), startedAt, finishedAt);
        }
    }
}
