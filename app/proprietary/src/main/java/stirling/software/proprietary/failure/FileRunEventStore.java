...
public class FileRunEventStore {
    ...
    public FileRunEvent record(RecordFailure command) {
        ...
        Optional<FileRunEvent> folded =
                findByDedupKey(command.teamId(), dedupKey).flatMap(row -> absorb(row.getId(), command, now));
        if (folded.isPresent()) {
            return folded.get();
        }
    ...
    private Optional<FileRunEventEntity> findByDedupKey(Long teamId, String dedupKey) {
        return repository.findByTeamAndDedupKey(teamId, dedupKey, NEWEST).stream()
                .findFirst();
    }
    ...
    public List<FileRunEvent> list(Long teamId, FileRunEventStatus status, String kindId, String actor, int limit) {
        Pageable page = PageRequest.of(0, Math.max(1, limit));
        List<FileRunEventEntity> rows = status == null
                ? repository.findByTeamAndStatusIn(teamId, FileRunEventStatus.open(), kindId, actor, page)
     ...
     */
    @Transactional
    public FileRunEvent applyStatus(String id, Long teamId, FileRunEventStatus target, String actor) {
    ...
    public int markFilesRemoved(Long teamId, String actor, Collection<String> fileIds) {
        ...
        return repository.markFilesRemoved(teamId, actor, fileIds, Instant.now(), FileRunEventStatus.open());
    }
    ...
    private FailureActionException refusalFor(String id, Long teamId) {
        return repository.findByIdAndTeam(id, teamId).isPresent()
                ? new FailureActionException(
                        FailureActionException.Reason.ALREADY_CLOSED, "Event " + id + " is already closed")
                ...
                : new FailureActionException(FailureActionException.Reason.EVENT_NOT_FOUND, "No such event: " + id);
    }
}
