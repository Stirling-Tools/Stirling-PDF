...
public class PdfToPdfUaRequest extends PDFFile {
    ...
    @Schema(
            description = "Document language as a BCP-47 tag, for example en-GB. Applied only when the"
    ...
    @Schema(
            description = "Replace the language the document already declares. Off by default, so a"
    ...
    @Schema(
            description = "How to treat images with no description. require-alt leaves them undescribed so"
    ...
    @Schema(
            description = "Embed fonts the document references but does not carry. Required for"
                    + " conformance and needs Ghostscript.",
            defaultValue = "true")
    private Boolean embedFonts;

    @Schema(
            description = "Alternative descriptions for figures, as key=text pairs separated by newlines."
