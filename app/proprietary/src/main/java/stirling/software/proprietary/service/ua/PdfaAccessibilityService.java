...
public class PdfaAccessibilityService implements PdfaLevelAServiceInterface {
     ...
     */
    @Override
    public Result upgradeToLevelA(byte[] pdfBytes, int part, String language, String title, boolean alsoDeclareUa) {
        ...
        try {
            ...
            try (PDDocument document = Loader.loadPDF(pdfBytes)) {
                ...
                if (document.getNumberOfPages() > MAX_TAGGABLE_PAGES) {
                    warnings.add("This document has "
                ...
                TaggingOptions options = TaggingOptions.builder()
            ...
            if (taggingResult.getTaggedElements() == 0 && taggingResult.isRebuilt()) {
                warnings.add("No taggable content was found, so the file cannot claim PDF/A level A."
                        + " It remains valid at level B.");
                return new Result(pdfBytes, false, warnings);
            }
            if (taggingResult.isContentSuppressed()) {
                warnings.add("Some text could not be tagged reliably and was marked as an artifact, so"
            ...
            if (!validationService.validatesAsPdfaLevelA(declared, part)) {
                warnings.add("The document was tagged but does not validate as PDF/A-"
                        + part
            ...
            if (alsoDeclareUa) {
                ...
                warnings.add("PDF/UA was requested alongside PDF/A but "
            ...
            warnings.add(
                    "Level A upgrade failed (" + e.getMessage() + "), so the file was left at conformance level B.");
            return new Result(pdfBytes, false, warnings);
        }
    }
    ...
    private static void addPdfUaExtensionSchema(XMPMetadata xmp) {
        PDFAExtensionSchema extension = (PDFAExtensionSchema) xmp.getSchema(PDFAExtensionSchema.class);
        if (extension == null) {
        ...
        ArrayProperty properties = xmp.getTypeMapping()
                .createArrayProperty(
                        schema.getNamespace(), schema.getPrefix(), PDFASchemaType.PROPERTY, Cardinality.Seq);
    ...
    private static void addField(XMPMetadata xmp, AbstractStructuredType target, String name, String value) {
        target.getContainer()
                .addProperty(xmp.getTypeMapping().createText(target.getNamespace(), target.getPrefix(), name, value));
    }
    ...
    private static byte[] save(PDDocument document, int part) throws IOException {
        ...
        document.save(out, part == 1 ? CompressParameters.NO_COMPRESSION : new CompressParameters());
