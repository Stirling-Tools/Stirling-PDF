...
public interface StoredFileRepository extends JpaRepository<StoredFile, Long> {
     ...
     */
    @Query("SELECT f FROM StoredFile f JOIN FETCH f.owner "
    ...
    @Modifying
    @Transactional
    @Query("UPDATE StoredFile f SET f.storageKey = :newKey, f.encryptionKeyId = :keyId "
    ...
    @Modifying
    @Transactional
    @Query("UPDATE StoredFile f SET f.historyStorageKey = :newKey "
            + "WHERE f.id = :id AND f.historyStorageKey = :oldKey")
    int swapHistoryBlob(@Param("id") Long id, @Param("oldKey") String oldKey, @Param("newKey") String newKey);

    @Modifying
    @Transactional
    @Query("UPDATE StoredFile f SET f.auditLogStorageKey = :newKey "
            + "WHERE f.id = :id AND f.auditLogStorageKey = :oldKey")
    int swapAuditLogBlob(@Param("id") Long id, @Param("oldKey") String oldKey, @Param("newKey") String newKey);
}
