...
public class AccessibilityAuditService {
    ...
    private static final List<String> HUMAN_CHECKS = List.of(
            "Is the reading order correct for someone who cannot see the layout?",
            "Does each alternative description convey what the image is for, not just what" + " it looks like?",
    ...
    private static void enforceLimits(byte[] pdfBytes) {
        ...
        if (pages > MAX_PAGES) {
            throw ExceptionUtils.createIllegalArgumentException(
                    "error.tooManyPages",
                    "This PDF has {0} pages. The accessibility report is limited to {1} pages;" + " split it first.",
                    pages,
                    MAX_PAGES);
        }
    }

    private void populateSummary(PDDocument document, AccessibilityReport report) throws IOException {
        ...
        summary.setHasLanguage(
                catalog.getLanguage() != null && !catalog.getLanguage().isBlank());
    ...
    private List<FigureDescriptor> figuresNeedingDescription(PDDocument document) {
        try {
            ...
            for (StructBlock block : structure.figuresWithoutAlt()) {
                int ordinal = block.getRanges().isEmpty()
                        ? -1
                        : block.getRanges().get(0).start();
                BBox box = block.getBbox();
                figures.add(new FigureDescriptor(
    ...
    private static boolean declaresUa(PDDocument document) {
        try {
            ...
            String xmp = new String(metadata.toByteArray(), java.nio.charset.StandardCharsets.UTF_8);
