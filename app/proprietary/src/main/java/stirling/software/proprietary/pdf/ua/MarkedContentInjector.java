...
public class MarkedContentInjector {
    ...
    public int inject(PDDocument document, PDPage page, List<StructBlock> blocks, int nextMcid, boolean stripExisting)
            throws IOException {
        ...
        for (Object token : tokens) {
            ...
            if (stripExisting && isMarkedContentOperator(name)) {
                boolean keep;
                if (opensMarkedContent(name)) {
                    keep = isOptionalContent(name, operands) || carriesReplacementText(name, operands);
                    if (keep) {
            ...
            if (MarkableOp.isMarkableOperator(name)) {
                ...
                if (!open || owner != openBlock) {
                    ...
                    if (open) {
                        ...
                        output.add(insidePath ? pathStart : output.size(), Operator.getOperator("EMC"));
                        if (insidePath) {
                            pathStart++;
                        }
                    }
                ...
                for (StructBlock.OrdinalRange range : node.getRanges()) {
                    for (int i = range.start(); i <= range.end(); i++) {
                        ...
                        if (existing != null && existing != node) {
                            log.debug("Ordinal {} claimed by both {} and {}; keeping the first", i, existing, node);
                        }
                    }
                }
            });
    ...
    private static void write(PDDocument document, PDPage page, List<Object> tokens) throws IOException {
