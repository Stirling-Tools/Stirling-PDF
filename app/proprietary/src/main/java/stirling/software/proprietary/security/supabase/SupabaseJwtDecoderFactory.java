 ...
 *
 * <p>TODO: Migration required - this class previously produced a Spring Security
 * {@code org.springframework.security.oauth2.jwt.JwtDecoder} bean (Nimbus-based) via
 * {@code @Configuration}/{@code @Bean}, conditionally registered with
 * {@code @ConditionalOnProperty(security.supabase.user-login.enabled=true)}. Quarkus has no
 * {@code JwtDecoder} abstraction; bearer/JWT validation for the Supabase issuer must be wired via
 * quarkus-oidc (resource-server) using {@code quarkus.oidc.<tenant>.auth-server-url}/
 * {@code quarkus.oidc.<tenant>.jwks-path}, or via quarkus-smallrye-jwt
 * ({@code mp.jwt.verify.publickey.location} pointing at the JWKS URL computed by
 * {@link #jwksUri()}). The conditional registration becomes a build/runtime profile guard
 * (e.g. {@code @io.quarkus.arc.profile.IfBuildProfile} or a {@code quarkus.oidc.<tenant>.tenant-enabled}
...
public class SupabaseJwtDecoderFactory {
    ...
    /**
     * Computes the Supabase JWKS endpoint, or returns {@code null} when no issuer is configured
     * (in which case token verification must fail closed - reject every token).
     */
