...
public final class McpConfigValidator {
    ...
    public static List<Finding> validate(ApplicationProperties.Mcp mcp) {
        ...
        if ("apikey".equalsIgnoreCase(auth.getMode())) {
            findings.add(info("auth mode = apikey - clients send a Stirling API key via X-API-KEY (or"
        ...
        if (mode != null && !mode.isBlank() && !"oauth".equalsIgnoreCase(mode.trim())) {
            findings.add(warn("mcp.auth.mode='"
                    + mode
        ...
        if (isBlank(auth.getIssuerUri())) {
            findings.add(warn("mcp.auth.issuer-uri is not set: the JWT decoder fails closed and rejects"
            ...
            findings.add(warn("mcp.auth.issuer-uri='" + auth.getIssuerUri() + "' does not look like an http(s) URL."));
        }

        boolean hasResourceId = !isBlank(auth.getResourceId());
        boolean hasAcceptedAudiences = auth.getAcceptedAudiences().stream().anyMatch(a -> !isBlank(a));

        if (!hasResourceId && !hasAcceptedAudiences) {
            findings.add(warn("neither mcp.auth.resource-id nor mcp.auth.accepted-audiences is set: the"
            ...
            if (hasResourceId && !looksLikeUrl(auth.getResourceId())) {
                findings.add(warn("mcp.auth.resource-id='"
                        + auth.getResourceId()
            ...
            if (hasAcceptedAudiences) {
                findings.add(info("mcp.auth.accepted-audiences="
                ...
                findings.add(info("audience binding is strict (token aud must equal"
        ...
        if (isBlank(auth.getJwksUri())) {
            findings.add(info("mcp.auth.jwks-uri not set - signing keys are auto-discovered from the"
                    + " issuer's OpenID configuration."));
        }

        if ("sub".equalsIgnoreCase(auth.getUsernameClaim()) && auth.isRequireExistingAccount()) {
            findings.add(warn("mcp.auth.username-claim='sub' with require-existing-account=true: many"
        ...
        if (!auth.isRequireExistingAccount()) {
            findings.add(warn("mcp.auth.require-existing-account=false: any token your IdP signs can"
        ...
        if (mcp.isScopesEnabled()) {
            findings.add(info("mcp.scopes-enabled=true - the IdP must mint 'mcp.tools.read' and"
        ...
        if (allowed != null && !allowed.isEmpty()) {
            findings.add(info("mcp.allowed-operations is a strict allow-list of "
                    + allowed.size()
                    + " operation(s); every other tool is hidden, so a wrong or"
                    + " typo'd id silently exposes nothing."));
            List<String> shadowed = blocked == null
                    ? List.of()
                    : allowed.stream().filter(blocked::contains).toList();
            if (!shadowed.isEmpty()) {
                findings.add(warn("mcp operation(s) "
                        + shadowed
