...
import stirling.software.proprietary.audit.ProcessorAuditEventRow;
import stirling.software.proprietary.model.api.documents.ProcessorDocAuditEventDto;
import stirling.software.proprietary.model.api.documents.ProcessorDocumentsResponseDto;
import stirling.software.proprietary.model.api.documents.ProcessorDocumentsSummaryDto;
import stirling.software.proprietary.model.api.documents.ProcessorReviewDocumentDto;
...
public class ProcessorDocumentsService {

    private static final int RETURN_LIMIT = 40;

    private final ProcessorAuditReadService auditReadService;
    private final ObjectMapper objectMapper;

    /** Whole-server view (admins). */
    public ProcessorDocumentsResponseDto serverDocuments() {
        return build(auditReadService.serverEvents());
    }

    /** Team-scoped view: only files touched by {@code principals}. */
    public ProcessorDocumentsResponseDto scopedDocuments(String cacheKey, List<String> principals) {
        return build(auditReadService.scopedEvents(cacheKey, principals));
    }

    private ProcessorDocumentsResponseDto build(List<ProcessorAuditEventRow> events) {
        ...
        List<ProcessorReviewDocumentDto> documents = new ArrayList<>();
        int processedToday = 0;
        for (ProcessorAuditEventRow event : events) {
        ...
        ProcessorDocumentsSummaryDto summary =
                ProcessorDocumentsSummaryDto.builder()
        ...
        return ProcessorDocumentsResponseDto.builder()
                .summary(summary)
                .documents(documents)
                .build();
    }
    ...
    private ProcessorReviewDocumentDto toDocument(
            String rowId,
            String name,
            String contentType,
            String product,
            ...
            String source,
            Instant timestamp) {
        ProcessorDocAuditEventDto op =
                ProcessorDocAuditEventDto.builder()
                        .id(rowId + "-op")
        ...
        return ProcessorReviewDocumentDto.builder()
                .id("doc-" + rowId)
                .name(name)
    ...
    private Map<String, Object> parseData(ProcessorAuditEventRow event) {
