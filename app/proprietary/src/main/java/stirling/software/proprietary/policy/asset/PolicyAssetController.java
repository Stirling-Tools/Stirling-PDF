...
public class PolicyAssetController {
    ...
    @Operation(
            ...
            description = "Stores a supporting file (multipart field 'file') for pipeline steps to"
                    + " reference from their fileParameters as 'asset:<id>', and returns"
                    + " its metadata including the assigned id.")
    public ResponseEntity<PolicyAsset> upload(@RequestPart("file") MultipartFile file) throws IOException {
        ...
        PolicyAsset meta = new PolicyAsset(
                null,
                fileName,
                file.getContentType(),
                file.getSize(),
    ...
    @Operation(
            ...
            description = "Lists the supporting files visible to the caller's team (metadata only), so"
    ...
    @Operation(
            ...
            description = "Returns the asset's bytes with its stored content type and filename. Gated"
    ...
    public ResponseEntity<Resource> content(@PathVariable String assetId) {
        ...
        byte[] bytes = assetStore
                .content(assetId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No asset: " + assetId));
    ...
    @Operation(
            ...
            description = "Removes an asset no pipeline references. An asset still referenced by a"
    ...
    public ResponseEntity<Void> delete(@PathVariable String assetId) {
        ...
        boolean referenced = policyAccessGuard.visibleFrom(policyStore).stream()
                .anyMatch(policy ->
                        PolicyAssetRefs.referencedAssetIds(policy.steps()).contains(assetId));
        if (referenced) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Asset is still referenced by a pipeline step");
        }
