...
public class PolicyRunner {
    ...
    public PolicyRunHandle runWith(
            Policy policy, PolicyInputs inputs, PolicyProgressListener listener, String documentReference) {
        PolicyRunHandle handle = policyEngine.runPolicy(policy, inputs, listener, null, documentReference);
        docCounter.record(
    ...
    private List<String> pullAndRun(Policy policy, String sourceId, InputSpec spec, PolicySweep context) {
        ...
        for (ResolvedInput unit : work) {
            runIds.add(startRun(policy, sourceId, unit.fileIdentity(), unit.inputs(), unit.onComplete()));
    ...
    private String startRun(
            Policy policy, String sourceId, String fileIdentity, PolicyInputs inputs, Consumer<Boolean> onComplete) {
        log.info("Running policy {} ({})", policy.id(), policy.name());
        PolicyRunHandle handle =
                policyEngine.runPolicy(policy, inputs, PolicyProgressListener.NOOP, sourceId, fileIdentity);
