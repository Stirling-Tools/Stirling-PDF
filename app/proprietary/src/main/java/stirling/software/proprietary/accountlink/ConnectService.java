...
public class ConnectService {
    ...
    public record ConnectStatus(Phase phase, String authorizeUrl, Long secondsRemaining, Long teamId) {
    ...
    public record CallbackHint(String requestedCallbackUrl, String browserOrigin, String derivedBaseUrl) {}
    ...
    public ConnectStatus startReauth(CallbackHint hint) throws IOException {
        DeviceCredential credential = credentialStore
                .get()
                .orElseThrow(() ->
                        new IOException("This server is not linked, so there is no session" + " to re-establish"));
        return open(credential.getDeviceId(), hint, credential);
    }

    private ConnectStatus open(String name, CallbackHint hint, DeviceCredential credential) throws IOException {
        String callbackUrl = resolveCallbackUrl(hint);
        if (callbackUrl == null) {
            throw new IOException("Cannot determine where to send the admin back to; set system.frontendUrl");
        }
        ...
        state.setExpiresAt(now.plusSeconds(created.expiresInSeconds() > 0 ? created.expiresInSeconds() : 900));
    ...
    public ConnectStatus complete(String nonce) {
        ...
        if (nonce == null || !nonceMatches(nonce, state.getNonce())) {
            log.warn("Account-link connect: callback for handshake {} had a bad nonce", state.getRequestId());
            return ConnectStatus.of(Phase.REJECTED);
        }

        AccountLinkClient.ConnectClaimResult claim = client.connectClaim(state.getRequestId(), state.getClaimSecret());
        return switch (claim.outcome()) {
            ...
            case CONFIRMED -> {
                stateRepo.delete(state);
                log.info("Account-link connect: session re-established for team {}", claim.teamId());
                yield new ConnectStatus(Phase.LINKED, null, null, claim.teamId());
            }
            case PENDING ->
    ...
    private static ConnectStatus pendingStatus(ConnectState state, LocalDateTime now) {
        ...
        return new ConnectStatus(Phase.PENDING, state.getAuthorizeUrl(), Math.max(remaining, 0), null);
    }
    ...
    private static String originOf(String candidate) {
        ...
        return defaultPort ? scheme + "://" + uri.getHost() : scheme + "://" + uri.getHost() + ":" + port;
    }
