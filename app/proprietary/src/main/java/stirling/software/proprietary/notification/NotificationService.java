...
/**
 * Assembles the caller's notifications from whatever produces them, and routes an action back to
 * whichever source produced the one being acted on. Derived on read rather than stored: there is
 * one source today, and a table would need a write path, a retention story and a per-user read
 * model before it earned itself.
 *
 * <p>Who sees what, and who may do what, is decided by each source rather than here. {@link
 * FileRunEventService} already scopes its reads and resolves each row's actions against that
 * reader, so this cannot widen either by accident.
 */
@Service
...
public class NotificationService {
    ...
    /**
     * Apply an action to the row behind one notification, and return the notification as it now
     * stands. {@code notificationId} is the prefixed id the client was given: the bell holds no raw
     * failure id, so it cannot reach a failure endpoint even by accident.
     *
     ...
     *     the action, exactly as it would on its own surface
     */
     ...
     * stands. Takes the prefixed id for the same reason {@link #act} does, even though nobody
     * pressed a button: it is still a call made from the bell.
     *
     * @throws IllegalArgumentException if the id names no source this build has
     * @throws stirling.software.proprietary.failure.FailureActionException if the source refuses,
    ...
    private NotificationView fromFailure(FileRunEvent event) {
        return new NotificationView(
                ...
                event.createdAt(),
                event.lastSeenAt(),
                // The failure surface's own projection, not a copy: a bell offering
