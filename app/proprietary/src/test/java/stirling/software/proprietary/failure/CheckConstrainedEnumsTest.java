...
class CheckConstrainedEnumsTest {
    ...
    void everyPersistedEnumStillMatchesTheShippedCheckConstraints() {
        assertThat(names(FileRunEventStatus.values()))
                .containsExactlyInAnyOrder("NEW", "ACKNOWLEDGED", "DISMISSED", "RESOLVED", "FILE_REMOVED");
        assertThat(names(FailureOrigin.values())).containsExactlyInAnyOrder("TOOL", "POLICY", "PIPELINE");
        assertThat(names(FailureStage.values()))
                .containsExactlyInAnyOrder("INPUT", "INTERNAL", "OUTPUT", "BLOCKED", "NEVER_RAN");
        assertThat(names(FailureSeverity.values())).containsExactlyInAnyOrder("ERROR", "WARNING", "INFO");
        assertThat(names(FailureScope.values())).containsExactlyInAnyOrder("FILE", "RUN", "POLICY", "SOURCE", "SERVER");
    }
    ...
    void nothingAddedToTheModelReachedTheTable() throws Exception {
        ...
        List<Class<?>> persisted = Arrays.stream(FileRunEventEntity.class.getDeclaredFields())
