...
class AccountLinkControllerTest {
    ...
    void setUp() {
        ...
        controller = new AccountLinkController(service, connectService, mock(LocalUsageService.class), syncProvider);
    }
    ...
    void connectStart_upstreamFailure_maps502() throws Exception {
        when(connectService.start(any(), any())).thenThrow(new AccountLinkClient.UpstreamException(500, "boom"));
    ...
    void connectStart_transportFailure_maps502WithoutLeakingTheHost() throws Exception {
        when(connectService.start(any(), any())).thenThrow(new IOException("connection refused to saas.internal:8081"));
