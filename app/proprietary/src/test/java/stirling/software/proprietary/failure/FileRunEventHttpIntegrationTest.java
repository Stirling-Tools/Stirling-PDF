...
class FileRunEventHttpIntegrationTest {
    ...
    private String seed(FailureKind kind, Long teamId, String fileId, String detail) {
        ...
        return store.record(new RecordFailure(
                        kind,
    ...
    class ReadPath {
        ...
        void serialisesAnEventWithItsFacetsCopyKeysAndResolvedActions() throws Exception {
            ...
            assertThat(row.get("titleKey").asString()).isEqualTo("portal.failures.kind.inputPasswordProtected.title");
            ...
            assertThat(actions.get(0).get("labelKey").asString()).isEqualTo("portal.failures.action.viewInProcessor");
            assertThat(actions.get(0).get("defaultLabel").asString()).isEqualTo("View in processor");
        ...
        void doesNotReturnAnotherTeamsRows() throws Exception {
            ...
            JsonNode events =
                    mapper.readTree(get("/api/v1/file-run-events").body()).get("events");
        ...
        void ignoresAClientSuppliedTeamId() throws Exception {
            ...
            JsonNode events = mapper.readTree(
                            get("/api/v1/file-run-events?teamId=999").body())
                    .get("events");
        ...
        void coercesQueryParametersAndFiltersOnThem() throws Exception {
            ...
            JsonNode dismissed = mapper.readTree(
                            get("/api/v1/file-run-events?status=DISMISSED").body())
                    .get("events");
            assertThat(dismissed).hasSize(1);

            JsonNode byKind = mapper.readTree(
                            get("/api/v1/file-run-events?status=DISMISSED" + "&kindId=INPUT_PASSWORD_PROTECTED")
                                    .body())
                    .get("events");
        ...
        void rejectsAnUnknownStatusFilter() throws Exception {
            assertThat(get("/api/v1/file-run-events?status=BANANA").statusCode())
                    .isEqualTo(400);
        }
    ...
    class Reporting {
        ...
        void bindsAReportBodyAndAnswersNoContent() throws Exception {
            ...
            assertThat(events.get(0).get("kindId").asString()).isEqualTo("INPUT_PASSWORD_PROTECTED");
        }

        @Test
        void acceptsAReportWithNoCodeOrFiles() throws Exception {
            HttpResponse<String> response =
                    post("/api/v1/file-run-events/reports", "{\"operation\":\"compress\",\"detail\":\"network died\"}");
        ...
        void rejectsAReportWithNoOperation() throws Exception {
            assertThat(post("/api/v1/file-run-events/reports", "{\"errorCode\":\"E004\",\"detail\":\"boom\"}")
                            .statusCode())
                    .isEqualTo(400);
        }
        ...
        void rejectsAnOversizedReportWithoutRecordingAnyOfIt() throws Exception {
            ...
            String ids = java.util.stream.IntStream.range(0, EditorFailureReport.MAX_FILE_IDS + 1)
                    .mapToObj(i -> "\"f-" + i + "\"")
    ...
    class Dispatch {
        ...
        void acceptsAPopulatedInputsMap() throws Exception {
            ...
            HttpResponse<String> response = post(
                    "/api/v1/file-run-events/" + id + "/actions/DISMISS", "{\"inputs\":{\"password\":\"hunter2\"}}");
        ...
        void acceptsAnAbsentBody() throws Exception {
            ...
            assertThat(post("/api/v1/file-run-events/" + id + "/actions/DISMISS", null)
                            .statusCode())
                    .isEqualTo(200);
        }
        ...
        void mapsAnUnknownActionToBadRequest() throws Exception {
            ...
            assertThat(post("/api/v1/file-run-events/" + id + "/actions/APPROVE", "{\"inputs\":{}}")
                            .statusCode())
                    .isEqualTo(400);
        }
        ...
        void mapsAnActionTheClientRunsToBadRequest() throws Exception {
            ...
            assertThat(post("/api/v1/file-run-events/" + id + "/actions/VIEW_FILE", "{\"inputs\":{}}")
                            .statusCode())
                    .isEqualTo(400);
        }
        ...
        void mapsAnotherTeamsRowToNotFound() throws Exception {
            ...
            assertThat(post("/api/v1/file-run-events/" + id + "/actions/DISMISS", "{\"inputs\":{}}")
                            .statusCode())
                    .isEqualTo(404);
        }
        ...
        void aDismissedRowComesBackWithItsActionsDisabled() throws Exception {
            ...
            JsonNode row = mapper.readTree(post("/api/v1/file-run-events/" + id + "/actions/DISMISS", null)
                    .body());

            assertThat(row.get("status").asString()).isEqualTo("DISMISSED");
            for (JsonNode action : row.get("actions")) {
                assertThat(action.get("enabled").asBoolean()).isFalse();
                assertThat(action.get("disabledReasonKey").asString()).isEqualTo("portal.failures.disabled.closed");
            }
        }
    }
    ...
    static class TestApp {
        ...
        PolicyManagementAuthority policyManagementAuthority() {
            PolicyManagementAuthority authority = org.mockito.Mockito.mock(PolicyManagementAuthority.class);
