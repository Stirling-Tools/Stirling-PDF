    ...
    class AuditGating {
        ...
        void recordsDocumentEventsWithoutEE() {
            ...
            assertThat(captor.getAllValues())
                    ...
                    .containsExactly(AuditEventType.PDF_PROCESS.name(), AuditEventType.FILE_OPERATION.name());
        }
        ...
        void typeAwareShouldAuditWithoutEE() throws Exception {
            ...
            assertThat(nonEe.shouldAudit(AuditEventType.PDF_PROCESS, m, auditConfig))
                    .isTrue();
            assertThat(nonEe.shouldAudit(AuditEventType.FILE_OPERATION, m, auditConfig))
                    .isTrue();
            assertThat(nonEe.shouldAudit(AuditEventType.USER_LOGIN, m, auditConfig))
                    .isFalse();
            // With EE, non-doc events at/under the configured level audit too.
            assertThat(service.shouldAudit(AuditEventType.USER_LOGIN, m, auditConfig))
                    .isTrue();
        }
