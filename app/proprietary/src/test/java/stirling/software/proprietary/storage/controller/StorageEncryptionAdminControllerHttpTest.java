...
class StorageEncryptionAdminControllerHttpTest {
    ...
    void setUp() {
        ...
        StorageEncryptionAdminController controller = new StorageEncryptionAdminController(
                props,
    ...
    void disable_unknownKey_returns404() throws Exception {
        mockMvc.perform(post("/api/v1/admin/storage-encryption/keys/" + UUID.randomUUID() + "/disable"))
    ...
    void migrate_serialisesRunningSnapshotIncludingInstants() throws Exception {
        when(migrationService.start())
                .thenReturn(new StorageEncryptionMigrationService.MigrationStatus(
    ...
    void migrate_alreadyRunning_returns409() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/admin/storage-encryption/migrate")).andExpect(status().isConflict());
    }
    ...
    void controller_isAdminOnly() {
        PreAuthorize preAuthorize = StorageEncryptionAdminController.class.getAnnotation(PreAuthorize.class);
    ...
    void noDeleteEndpointExists() {
        ...
        boolean anyDelete = java.util.Arrays.stream(StorageEncryptionAdminController.class.getMethods())
