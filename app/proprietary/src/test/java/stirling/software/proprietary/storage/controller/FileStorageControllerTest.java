...
class FileStorageControllerTest {
    ...
    @BeforeEach
    void setUp() {
        FileStorageController controller = new FileStorageController(fileStorageService, storageProvider, auditService);
    ...
    void downloadFile_encryptedContent_auditsPlaintextExportAsAttachment() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/storage/files/{fileId}/download", 77L)).andExpect(status().isOk());
        ...
        assertThat(exportEvents()).singleElement().satisfies(event -> {
            ...
            assertThat(event).containsEntry("keyId", "cafe1234-0000-0000-0000-000000000001");
        });
    }
    ...
    void downloadFile_inlineView_marksTheExportInline() throws Exception {
        ...
        // apart.
        assertThat(exportEvents()).singleElement().satisfies(e -> assertThat(e).containsEntry("inline", true));
    }
    ...
    private void streamedDownload(StoredFile file) throws Exception {
        when(fileStorageService.requireAuthenticatedUser()).thenReturn(file.getOwner());
        when(fileStorageService.getAccessibleFile(file.getOwner(), 77L)).thenReturn(file);
        when(storageProvider.signedDownloadUrl(anyString(), any(Duration.class), anyBoolean(), anyString()))
                .thenReturn(Optional.empty());
        when(fileStorageService.loadFile(file)).thenReturn(new ByteArrayResource("decrypted bytes".getBytes(UTF_8)));
    }
