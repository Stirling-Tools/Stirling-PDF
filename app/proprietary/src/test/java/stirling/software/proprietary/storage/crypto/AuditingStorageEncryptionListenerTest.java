...
class AuditingStorageEncryptionListenerTest {
    ...
    private List<Map<String, Object>> auditedEvents() {
        ...
        verify(auditService, atLeastOnce()).audit(eq(AuditEventType.STORAGE_ENCRYPTION), data.capture());
    ...
    void auditReadsOn_recordsDecryptWithKeyAndStorageKey() {
        ...
        assertThat(auditedEvents()).singleElement().satisfies(event -> {
    ...
    void auditReadsOff_dropsDecryptButKeepsEverythingElse() {
        AuditingStorageEncryptionListener listener = new AuditingStorageEncryptionListener(auditService, false);
    ...
    void decryptDenied_carriesTheReason() {
        new AuditingStorageEncryptionListener(auditService, false).decryptDenied(KEY_ID, "key not found");
