...
/** Bulk processor-access must match per-user canAccessProcessor for every policy. */
@ExtendWith(MockitoExtension.class)
class ResourceAccessProcessorBulkParityTest {
    ...
    void setUp(DefaultAccessPolicy policy) {
        ...
        ReflectionTestUtils.setField(service, "processorDefaultPolicy", policy);
    ...
    void orgAllDoesNotLeakDeploymentWideWhenResolverForbidsIt() {
        ...
        ReflectionTestUtils.setField(
                service, "processorDefaultPolicy", DefaultAccessPolicy.ORG_ALL);
        lenient()
                .when(grantRepository.findByResourceTypeAndResourceId(ResourceType.PORTAL, ""))
                .thenReturn(List.of());
        ...
        Set<Long> bulk =
                service.usersWithProcessorAccess(List.of(adminUser, plainMember), Set.of());

        assertThat(bulk).contains(1L).doesNotContain(5L);
        assertThat(service.canAccessProcessor(plainMember))
                .as("ORG_ALL must not grant a plain member deployment-wide on a SaaS-like resolver")
                .isFalse();
        assertThat(service.canAccessProcessor(adminUser)).isTrue();
    }

    private void assertParity(DefaultAccessPolicy policy) {
        setUp(policy);
        Set<Long> bulk = service.usersWithProcessorAccess(everyone, leaderUserIds);
        for (User user : everyone) {
            boolean authoritative = service.canAccessProcessor(user);
            assertThat(bulk.contains(user.getId()))
                    .as(
                            "policy=%s user=%d bulk should equal canAccessProcessor(%s)",
