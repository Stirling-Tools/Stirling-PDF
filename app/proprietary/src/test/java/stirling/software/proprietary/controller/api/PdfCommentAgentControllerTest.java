...
class PdfCommentAgentControllerTest {
    ...
    void setUp() {
        PdfCommentAgentController controller =
                new PdfCommentAgentController(orchestrator, JsonMapper.builder().build(), aiFeatureGate);
        mockMvc = MockMvcBuilders.standaloneSetup(controller)
                ...
                .setHandlerExceptionResolvers(
                        new ResponseStatusExceptionResolver(), new DefaultHandlerExceptionResolver())
                .build();
    }
    ...
    void acceptsValidPdfAndReturnsAnnotatedBytes() throws Exception {
        MockMultipartFile pdfFile = new MockMultipartFile(
                "fileInput", "input.pdf", MediaType.APPLICATION_PDF_VALUE, "%PDF-1.4\n%%EOF".getBytes());
        ...
        mockMvc.perform(multipart("/api/v1/ai/tools/pdf-comment-agent")
                        .file(pdfFile)
                        .param("prompt", "flag dates"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_PDF))
                .andExpect(header().string(
                                "Content-Disposition", org.hamcrest.Matchers.containsString("input-commented.pdf")))
    ...
    void propagatesOrchestratorBadRequestForNonPdfUpload() throws Exception {
        ...
        MockMultipartFile notPdf =
                new MockMultipartFile("fileInput", "input.txt", MediaType.TEXT_PLAIN_VALUE, "hello".getBytes());
