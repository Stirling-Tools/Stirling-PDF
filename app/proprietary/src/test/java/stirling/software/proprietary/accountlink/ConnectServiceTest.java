...
class ConnectServiceTest {
    ...
    @Mock
    private ConnectStateRepository stateRepo;
    ...
    void setUp() {
        ...
        service = new ConnectService(client, stateRepo, credentialStore, entitlementCache, applicationProperties);
    }
    ...
    void start_advertisesTheConfiguredFrontendUrlInPreferenceToTheRequest() throws Exception {
        ...
        verify(client)
                .connectRequest(
                        ...
                        org.mockito.ArgumentMatchers.eq("https://pdf.example.com" + ConnectService.CALLBACK_PATH),
                        anyString(),
                        anyString(),
    ...
    void start_fallsBackToTheAddressTheRequestArrivedOn() throws Exception {
        ...
        assertThat(callback.getValue()).isEqualTo("https://pdf.internal:8443/stirling" + ConnectService.CALLBACK_PATH);
    }

    @Test
    void start_withNoAddressAtAllFailsRatherThanGuessing() {
        assertThat(catchIo(() -> service.start(null, fromRequest(null)))).hasMessageContaining("system.frontendUrl");
    ...
    void resolveCallback_honoursThePortalsOwnCallbackWhenTheBrowserOriginAgrees() {
        ...
        assertThat(service.resolveCallbackUrl(
                        new ConnectService.CallbackHint(requested, "http://localhost:5173", "http://localhost:8080")))
                .isEqualTo(requested);
    }

    @Test
    void resolveCallback_ignoresACallbackFromADifferentOrigin() {
        assertThat(service.resolveCallbackUrl(new ConnectService.CallbackHint(
                        "https://evil.example.com/steal", "http://localhost:5173", "http://localhost:8080")))
    ...
    void resolveCallback_prefersTheBrowserOriginOverTheApiRequest() {
        ...
        assertThat(service.resolveCallbackUrl(
                        new ConnectService.CallbackHint(null, "http://localhost:5173", "http://localhost:8080")))
                .isEqualTo("http://localhost:5173" + ConnectService.CALLBACK_PATH);
    }
    ...
    void resolveCallback_ignoresAnUnusableOriginHeader() {
        ...
        assertThat(service.resolveCallbackUrl(new ConnectService.CallbackHint(null, "null", "http://localhost:8080")))
    ...
    void start_sendsTheAdminWhereverSaaSSaidToSendThem() throws Exception {
        ...
        ConnectService.ConnectStatus status = service.start(null, fromRequest("https://pdf.example.com"));
    ...
    void complete_withNoNonceAtAllIsRejected() {
        when(stateRepo.findById(ConnectState.SINGLETON_ID)).thenReturn(Optional.of(openHandshake()));
    ...
    void startReauth_presentsTheCredentialSoSaaSCanPinTheTeam() throws Exception {
        ...
        verify(client)
                .connectRequest(
                        ...
                        anyString(),
                        anyString(),
                        org.mockito.ArgumentMatchers.argThat(c -> c != null && "dev".equals(c.getDeviceId())));
    }
