...
class PolicyValidatorTest {
    ...
    void acceptsAStepBindingThatReferencesATeamAsset() {
        ...
        when(outputSink.supports(any())).thenReturn(true);
        PolicyAsset asset =
                assetStore.save(new PolicyAsset(null, "logo.png", null, 0, "owner", null, 1L), new byte[] {1});
    ...
    void rejectsAStepBindingToAnUnknownAsset() {
        ...
        IllegalArgumentException ex = assertThrows(
                IllegalArgumentException.class,
                () -> validator.validate(withFileBinding(PolicyAssetRefs.PREFIX + "missing-asset", null)));
    ...
    void rejectsAStepBindingToAnotherTeamsAsset() {
        when(inputSource.supports(any())).thenReturn(true);
        PolicyAsset foreign =
                assetStore.save(new PolicyAsset(null, "secret.p12", null, 0, "owner", 99L, 1L), new byte[] {1});

        // Policy has no team; the asset belongs to team 99 - must read as unknown, not leak.
        IllegalArgumentException ex = assertThrows(
                IllegalArgumentException.class,
                () -> validator.validate(withFileBinding(PolicyAssetRefs.PREFIX + foreign.id(), null)));
        assertTrue(ex.getMessage().contains("unknown stored file"));
    }
    ...
    private Policy withFileBinding(String assetKey, Long teamId) {
        PipelineStep step =
                new PipelineStep("/api/v1/security/add-watermark", Map.of(), Map.of("watermarkImage", assetKey));
        return new Policy(
                "p1",
                "p",
                "owner",
