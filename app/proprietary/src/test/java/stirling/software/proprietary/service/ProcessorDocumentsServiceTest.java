...
class ProcessorDocumentsServiceTest {

    @Mock private ProcessorAuditReadService auditReadService;

    private ProcessorDocumentsService service;

    @BeforeEach
    void setUp() {
        service = new ProcessorDocumentsService(auditReadService, JsonMapper.builder().build());
    }

    private static ProcessorAuditEventRow row(long id, String data) {
        return new ProcessorAuditEventRow(
                id, "con.yoh13@gmail.com", AuditEventType.PDF_PROCESS.name(), data, Instant.now());
    }

    private ProcessorReviewDocumentDto onlyDoc(String data) {
    ...
    void policyStepDocumentIsLabelledAutomation() {
        ProcessorReviewDocumentDto doc =
                onlyDoc(
