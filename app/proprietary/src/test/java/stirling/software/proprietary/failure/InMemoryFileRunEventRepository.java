...
class InMemoryFileRunEventRepository implements FileRunEventRepository {
    ...
    public List<FileRunEventEntity> findByTeamAndStatus(
            Long teamId, FileRunEventStatus status, String kindId, String actor, Pageable pageable) {
        return page(
                newestFirst(rows.values().stream()
                        .filter(e -> sameTeam(e, teamId)
                                && e.getStatus() == status
                                && sameKind(e, kindId)
                                && sameActor(e, actor))
                        .toList()),
    ...
    public int applyStatusIf(
        ...
        FileRunEventEntity entity = rows.get(id);
        if (entity == null || !sameTeam(entity, teamId) || !allowedFrom.contains(entity.getStatus())) {
            return 0;
        }
    ...
    public List<FileRunEventEntity> findByTeamAndStatusIn(
            Long teamId, List<FileRunEventStatus> statuses, String kindId, String actor, Pageable pageable) {
        return page(
                newestFirst(rows.values().stream()
                        .filter(e -> sameTeam(e, teamId)
    ...
    @Override
    public List<FileRunEventEntity> findByTeamAndDedupKey(Long teamId, String dedupKey, Pageable pageable) {
        return newestFirst(rows.values().stream()
    ...
    private boolean clashesOnDedupKey(FileRunEventEntity candidate) {
        ...
        return rows.values().stream()
                .anyMatch(row -> candidate.getTeamId().equals(row.getTeamId())
    ...
    @Override
    public <S extends FileRunEventEntity> List<S> findAll(org.springframework.data.domain.Example<S> example) {
        throw new UnsupportedOperationException();
    }
    ...
    @Override
    public <S extends FileRunEventEntity> Optional<S> findOne(org.springframework.data.domain.Example<S> example) {
        throw new UnsupportedOperationException();
    }
    ...
    @Override
    public <S extends FileRunEventEntity> long count(org.springframework.data.domain.Example<S> example) {
        throw new UnsupportedOperationException();
    }

    @Override
    public <S extends FileRunEventEntity> boolean exists(org.springframework.data.domain.Example<S> example) {
        throw new UnsupportedOperationException();
    }
