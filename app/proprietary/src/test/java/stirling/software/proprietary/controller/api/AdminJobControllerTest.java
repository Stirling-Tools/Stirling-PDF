...
class AdminJobControllerTest {
    ...
    void cleanupWithForceIgnoresTheRetentionWindow() throws Exception {
        when(taskManager.cleanupFinishedJobsNow(any())).thenReturn(new TaskManager.CleanupSummary(5, 9, 0));
    ...
    void forcedAdminCleanupSweepsEveryUsersJobs() throws Exception {
        when(taskManager.cleanupFinishedJobsNow(any())).thenReturn(new TaskManager.CleanupSummary(1, 1, 0));
