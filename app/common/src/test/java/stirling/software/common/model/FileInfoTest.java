...
public class FileInfoTest {
    ...
    void testGetFormattedFileSize(long fileSize, String expectedFormattedSize) {
        FileInfo fileInfo =
                new FileInfo("example.txt", "/path/to/example.txt", FIXED_NOW, fileSize, FIXED_NOW.minusDays(1));
    ...
    class GetFilePathAsPathTests {
        ...
        void shouldConvertStringToPath() {
            FileInfo fi = new FileInfo("example.txt", "/path/to/example.txt", FIXED_NOW, 123, FIXED_NOW.minusDays(1));

            Path path = fi.getFilePathAsPath();

            // Basic sanity checks
            assertNotNull(path, "Path should not be null");
            assertEquals(Path.of("/path/to/example.txt"), path, "Converted Path should match input string");
        }
    }
    ...
    class DateFormattingTests {
        ...
        void shouldFormatModificationDate() {
            ...
            FileInfo fi =
                    new FileInfo("example.txt", "/path/to/example.txt", mod, 1, LocalDateTime.of(2024, 1, 1, 0, 0, 0));
        ...
        void shouldFormatCreationDate() {
            ...
            FileInfo fi = new FileInfo(
                    "example.txt", "/path/to/example.txt", LocalDateTime.of(2025, 1, 1, 0, 0, 0), 1, created);
        ...
        void shouldThrowWhenModificationDateNull() {
            ...
            assertThrows(
                    ...
                    "Formatting a null modificationDate should throw NPE with current" + " implementation");
        }
        ...
        void shouldThrowWhenCreationDateNull() {
            // Assumption: Current implementation does not guard null -> NPE is expected.
            FileInfo fi = new FileInfo("example.txt", "/path/to/example.txt", FIXED_NOW, 1, null); // creationDate null
    ...
    class AdditionalSizeFormattingTests {
        ...
        void shouldRepresentTerabytesInGb() {
            ...
            FileInfo fi = new FileInfo("example.txt", "/path/to/example.txt", FIXED_NOW, twoTB, FIXED_NOW.minusDays(1));
