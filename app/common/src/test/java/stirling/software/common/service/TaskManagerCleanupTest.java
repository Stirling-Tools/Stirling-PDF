...
class TaskManagerCleanupTest {
    ...
    void forcedCleanupSkipsJobsTheFilterRejects() {
        ...
        TaskManager.CleanupSummary summary = taskManager.cleanupFinishedJobsNow(id -> id.startsWith("alice:"));
    ...
    void cleanupSurvivesAFileStorageFailure() {
        ...
        taskManager.registerInputFile(jobId, "input-file");
        completeWithFile(jobId, "result-file");
        when(fileStorage.deleteFileAsSystem("result-file")).thenThrow(new RuntimeException("disk on fire"));
    ...
    void multiFileResultsAndTheInputCopyAreAllDeleted() {
        ...
        taskManager.registerInputFile(jobId, "input-file");
        JobResult result = taskManager.getJobResult(jobId);
        result.completeWithFiles(List.of(
                ResultFile.builder()
                        .fileId("page-1")
                        .fileName("1.pdf")
