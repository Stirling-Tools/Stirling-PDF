...
public class ToolIOOperationCustomizer implements GlobalOperationCustomizer, GlobalOpenApiCustomizer {
    ...
    private static Map<String, Object> toCase(ToolIOCase rule, Method handler) {
        ...
        entry.put(
                "when",
                Arrays.stream(rule.when()).map(c -> toCondition(c, handler)).toList());
    ...
    private static Map<String, Object> toCondition(ToolIOWhen condition, Method handler) {
        ...
        ToolIOParameterDefaults.resolve(handler, condition.param()).ifPresent(value -> entry.put("default", value));
        return entry;
    }
