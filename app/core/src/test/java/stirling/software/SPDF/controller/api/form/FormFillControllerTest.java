...
class FormFillControllerTest {
    ...
    class AddFields {
        ...
        void validPayload() throws Exception {
            ...
            String json = "[{\"name\":\"NewField\",\"type\":\"text\",\"pageIndex\":0,"
    ...
    class EditFields {
        ...
        void emptyBatch() {
            assertThatThrownBy(() -> controller.editFields(
                            pdfFile(), "{\"add\":[],\"modify\":[],\"delete\":[]}".getBytes(), false))
                    .isInstanceOf(IllegalArgumentException.class);
        }
        ...
        void validBatch() throws Exception {
            ...
            String json = "{\"add\":[{\"name\":\"f\",\"type\":\"text\",\"pageIndex\":0,\"x\":50,"
        ...
        void refusesPeriodInName() throws Exception {
            String json = "{\"add\":[{\"name\":\"Customer.Name\",\"type\":\"text\",\"pageIndex\":0,"
                    + "\"x\":50,\"y\":700,\"width\":200,\"height\":20}]}";
        ...
        void allowsUnchangedQualifiedName() throws Exception {
            ...
            String json = "{\"modify\":[{\"targetName\":\"Customer.Name\",\"name\":\"Customer.Name\","
                    + "\"x\":10,\"y\":10}]}";
            ResponseEntity<Resource> response = controller.editFields(file, json.getBytes(), false);
            ...
            String encoded = response.getHeaders().getFirst(FormFillController.SKIPPED_EDITS_HEADER);
        ...
        void noHeaderOnCleanBatch() throws Exception {
            ...
            String json = "{\"add\":[{\"name\":\"clean\",\"type\":\"text\",\"pageIndex\":0,\"x\":50,"
    ...
    class FieldBundle {
        ...
        private java.util.Map<String, java.util.zip.ZipEntry> entriesOf(byte[] zipBytes) throws IOException {
        ...
        void bundlesBoth() throws Exception {
            ...
            assertThat(new String(payloads.get("fields.json"), java.nio.charset.StandardCharsets.UTF_8))
                    .contains("bundled");
        }
        ...
        void matchesTheSecondCallItReplaces() throws Exception {
            ...
            MockMultipartFile saved = new MockMultipartFile("file", "test.pdf", "application/pdf", bundledPdf);
            try (PDDocument reloaded = org.apache.pdfbox.Loader.loadPDF(bundledPdf)) {
                ...
                ResponseEntity<java.util.List<stirling.software.common.model.FormFieldWithCoordinates>> refetched =
                        controller.listFieldsWithCoordinates(saved);

                String viaRefetch = realObjectMapper.writeValueAsString(refetched.getBody());
                String viaBundle = new String(payloads.get("fields.json"), java.nio.charset.StandardCharsets.UTF_8);
    ...
    class SkipHeaderBudget {
        ...
        void staysWithinBudget() throws Exception {
            ...
            ResponseEntity<Resource> response =
                    (ResponseEntity<Resource>) method.invoke(controller, streamingOk(new byte[] {1}), skipped);
            ...
            String decoded =
                    new String(java.util.Base64.getDecoder().decode(header), java.nio.charset.StandardCharsets.UTF_8);
            ...
            assertThat(response.getHeaders().getFirst(FormFillController.SKIPPED_EDITS_TOTAL_HEADER))
                    .isEqualTo("40");
        }
    }
}
