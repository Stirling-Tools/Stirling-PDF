...
public class PdfBoxFontEncodingProbeTest {
    ...
    private static final Path SAMPLE = PROJECT_ROOT.resolve("frontend/editor/public/samples/Sample.pdf");
    ...
    public void probeRenderInjectedSample() throws IOException {
        Path liberation = PROJECT_ROOT.resolve("app/core/src/main/resources/static/fonts/LiberationSans-Regular.ttf");
        ...
        try (PDDocument check = Loader.loadPDF(out.toByteArray())) {
            ...
            Path png = Paths.get(System.getProperty("user.dir"), "build", "probe-output")
                    .resolve("pdfbox-probe-q4b-rendered.png");
            Files.createDirectories(png.getParent());
            ImageIO.write(img, "PNG", png.toFile());
            System.out.println("Rendered injected sample to " + png + " - " + img.getWidth() + "x" + img.getHeight());
        }
    }
    ...
    public void probeType0CIDFontType2Subset() throws IOException {
        ...
        try (PDDocument doc = Loader.loadPDF(subsetPdf)) {
            ...
            for (COSName fn : res.getFontNames()) {
                ...
                System.out.println("  Subset font in saved PDF: "
                        + f.getName()
                        + " ("
                ...
                for (int p = 0; p < doc.getNumberOfPages(); p++) {
                    ...
                    for (COSName name : res.getFontNames()) {
                        ...
                        try {
                            ...
                            System.out.println(
                                    "  page " + p + " " + name.getName() + " load failed: " + e.getMessage());
                        }
                    }
                }
            }
    ...
    public void probeAllQuestions() throws IOException {
        ...
        try (PDDocument doc = Loader.loadPDF(pdfBytes)) {
            ...
            for (int p = 0; p < doc.getNumberOfPages(); p++) {
                ...
                for (COSName name : res.getFontNames()) {
                    ...
                    try {
                        ...
                        System.out.println("  (skipped " + name.getName() + " - " + e.getMessage() + ")");
                    }
                }
            }
            System.out.println("Discovered " + allFonts.size() + " unique fonts across Sample.pdf:");
            ...
            for (PDFont f : allFonts) {
                ...
                for (String ch : candidates) {
                    try {
                        ...
                        System.out.println("  encode('" + ch + "') UNSUPPORTED: " + uoe.getMessage());
                    } catch (IllegalArgumentException iae) {
                        System.out.println("  encode('" + ch + "') MISSING: " + iae.getMessage());
                    } catch (IOException ioe) {
                        System.out.println("  encode('" + ch + "') IO ERR: " + ioe.getMessage());
            ...
            for (PDFont f : allFonts) {
                ...
                if (f instanceof PDType3Font) {
                    System.out.println("    -> Type3 has CharProc streams, NOT a FontFile binary."
                            + " getFontStream() returns null. Round-trip rehab is impossible:");
                    System.out.println("       each glyph is a mini content stream, not a glyph outline in a"
        ...
        if (!Files.exists(liberation)) {
            ...
            try (PDDocument out = new PDDocument()) {
                ...
                System.out.println("  Wrote fresh-TTF PDF to " + tmp + " (" + baos.size() + " bytes) - opens cleanly.");
                ...
                try (PDDocument check = Loader.loadPDF(baos.toByteArray())) {
                    ...
                    for (COSName fn : res.getFontNames()) {
                        PDFont f = res.getFont(fn);
                        System.out.println("    embedded font: "
                                + f.getName()
                                + " ("
                                + f.getClass().getSimpleName()
                                + ", embedded="
        ...
        try (PDDocument doc = Loader.loadPDF(pdfBytes)) {
            ...
            try (PDPageContentStream cs = new PDPageContentStream(
                    doc, page, PDPageContentStream.AppendMode.APPEND, true /* compress */, true /* resetContext */)) {
                cs.beginText();
                cs.setFont(ttf, 12);
            ...
            Files.write(tmp, baos.toByteArray());
            System.out.println("  Wrote injected-text PDF to " + tmp + " (" + baos.size() + " bytes).");
            ...
            try (PDDocument check = Loader.loadPDF(baos.toByteArray())) {
                ...
                for (COSName fn : res.getFontNames()) {
                    PDFont f = res.getFont(fn);
                    count++;
                    System.out.println("    page-0 font: "
                            + fn.getName()
                            + " -> "
                            + f.getName()
                            + " ("
        ...
        for (Standard14Fonts.FontName fn : Standard14Fonts.FontName.values()) {
            ...
            System.out.println("  Standard14 " + fn.getName() + " -> mapped='" + mapped + "' name=" + f.getName());
        }
        System.out.println("  (PDFBox bundles ONLY LiberationSans-Regular.ttf as a binary; the AFMs cover"
