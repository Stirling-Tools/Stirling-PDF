...
class JobControllerTest {
    ...
    void testCleanupFinishedJobs_ReportsWhatWasReleased() {
        when(taskManager.cleanupFinishedJobsNow(any())).thenReturn(new TaskManager.CleanupSummary(2, 5, 1));
    ...
    void testCleanupFinishedJobs_OnlySweepsJobsTheCallerOwns() {
        ...
        when(jobOwnershipService.validateJobAccess("someone-else:job")).thenThrow(new SecurityException("not yours"));
        when(taskManager.cleanupFinishedJobsNow(any())).thenReturn(new TaskManager.CleanupSummary(1, 1, 1));
        ...
        assertFalse(filter.getValue().test("someone-else:job"), "A job the caller cannot access must be left in place");
    }

    @Test
    void testCleanupFinishedJobs_SweepsEverythingWhenSecurityIsDisabled() {
        when(taskManager.cleanupFinishedJobsNow(any())).thenReturn(new TaskManager.CleanupSummary(3, 3, 0));
