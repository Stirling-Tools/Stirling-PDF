 ...
 *
 * <p>Java is always the caller; Python never initiates connections back to Java. The engine URL is
 * configured via the {@code STIRLING_AI_ENGINE_URL} environment variable, which defaults to {@code
 * http://localhost:5001} for local development.
 *
 * <p>Two endpoints are called:
 *
 * <ul>
 *   <li>{@code POST /api/ledger/examine} — send a {@link FolioManifest}, receive a {@link
 *       Requisition}
...
public class AiEngineClient {
    ...
    @Value("${stirling.ai.engine.url:http://localhost:5001}")
    private String engineBaseUrl;

    /**
     * Round 1: present the FolioManifest to the Examiner. Returns the Requisition declaring what
     * Java must extract.
     */
    public Requisition examine(FolioManifest manifest) {
        String url = engineBaseUrl.stripTrailing() + "/api/ledger/examine";
        log.info(
                "[ledger] POST {} session={} round={}",
                url,
                manifest.sessionId(),
                manifest.round());
    ...
    /**
     * Round 2: present fulfilled Evidence to the Auditor. Returns an AgentTurn containing the
     * Verdict — the Auditor always commits after receiving its evidence.
     *
    ...
    public AgentTurn deliberate(Evidence evidence, BigDecimal tolerance) {
        String url =
                UriComponentsBuilder.fromUriString(
                                engineBaseUrl.stripTrailing() + "/api/ledger/deliberate")
