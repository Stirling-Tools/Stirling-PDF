...
public class SignPDFWithCertRequest extends PDFFile {
    ...
    @Schema(
            description = "The type of the digital certificate. WINDOWS_STORE and PKCS11 are"
    ...
    @Schema(
            description = "The private key for the digital certificate (required for PEM type"
                    + " certificates, supports .pem, .der, or .key files)")
    private MultipartFile privateKeyFile;

    @Schema(
            description = "The digital certificate (required for PEM type certificates, supports"
    ...
    @Schema(
            description = "The password for the keystore / private key, or the token PIN for PKCS11",
            format = "password")
    private String password;

    @Schema(
            description = "The alias of the certificate to sign with. Required for WINDOWS_STORE and"
                    + " recommended for PKCS11 tokens holding multiple certificates.")
    private String alias;

    @Schema(
            description = "Absolute path to the PKCS#11 driver library (required for PKCS11 type). Must"
                    + " be an allowed driver - a detected one or configured via"
                    + " STIRLING_PKCS11_LIBRARIES.")
    private String pkcs11LibraryPath;

    @Schema(description = "Optional PKCS#11 slot index. When omitted the first slot with a token is" + " used.")
    ...
    @Schema(
            description = "The page number where the signature should be visible. This is required if"
