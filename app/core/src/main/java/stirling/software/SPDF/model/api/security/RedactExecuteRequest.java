...
public class RedactExecuteRequest extends PDFFile {
    ...
    @Schema(
            description = "Exact strings to find and black out. One entry per phrase to redact."
                    + " Best for known names, identifiers, and specific text found in the document.")
    private List<String> textValues = new ArrayList<>();

    @Schema(
            description = "Regex patterns to match and redact. Each match anywhere in the document is blacked out."
    ...
    @Schema(description = "1-indexed page numbers to wipe entirely (all content removed from those pages).")
    private List<Integer> wipePages = new ArrayList<>();

    @Schema(
            description = "Text ranges to redact by specifying a start and end anchor phrase. All"
    ...
    @Schema(description = "Rectangular areas to black out, each defined by a page number and bounding box coordinates.")
    ...
    public record TextRange(
            @Schema(
                    description = "A short, distinctive phrase (5–15 words) that marks where"
    ...
    public record ImageBox(
            ...
            @Schema(
                    description = "Left x coordinate of the redaction rectangle in PDF user-space points.",
                    requiredMode = Schema.RequiredMode.REQUIRED)
            float x1,

            @Schema(
                    description = "Top y coordinate of the redaction rectangle in PDF user-space points.",
                    requiredMode = Schema.RequiredMode.REQUIRED)
            float y1,

            @Schema(
                    description = "Right x coordinate of the redaction rectangle in PDF user-space points.",
                    requiredMode = Schema.RequiredMode.REQUIRED)
            float x2,

            @Schema(
                    description = "Bottom y coordinate of the redaction rectangle in PDF user-space points.",
                    requiredMode = Schema.RequiredMode.REQUIRED)
            float y2) {}
    ...
    public static class RedactStyle {
        ...
        @Schema(description = "Extra padding around each box in points", type = "number", defaultValue = "0")
        ...
        @Schema(description = "Execution strategy hint for the redaction pipeline", defaultValue = "AUTO")
