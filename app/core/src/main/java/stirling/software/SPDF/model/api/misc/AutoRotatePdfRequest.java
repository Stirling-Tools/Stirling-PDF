...
public class AutoRotatePdfRequest extends PDFFile {
    ...
    @Schema(
            description = "Detection method. 'auto' tries embedded-text direction first and falls back"
    ...
    @Schema(
            description = "Minimum Tesseract OSD orientation confidence required before a correction is"
    ...
    @Schema(
            description = "If true, no rotation is applied; returns a JSON report of the per-page"
                    + " detection results instead of a PDF")
    private boolean dryRun;

    @Schema(
            description = "When a page cannot be decided on its own but the pages that could be decided"
    ...
    @Schema(
            description = "Optional pre-computed corrections to apply without running detection. Pages"
