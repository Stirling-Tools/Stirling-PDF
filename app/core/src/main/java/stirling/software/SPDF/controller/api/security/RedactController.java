...
public class RedactController {
    ...
    public ResponseEntity<Resource> redactPdf(@ModelAttribute RedactPdfRequest request) {
        ...
        List<String> terms = Arrays.stream(rawListOfText.split("\n"))
                .map(String::trim)
        ...
        try (PDDocument document = pdfDocumentFactory.load(request.getFileInput())) {
            ...
            try (TempFile tempInput = tempFileManager.createManagedTempFile(".pdf")) {
                ...
                try {
                    ...
                    log.debug("Calling JPDFium PdfRedactor.redact in RedactController (terms={})", terms);
                    RedactResult result = PdfRedactor.redact(tempInput.getFile().toPath(), options);
                    log.debug("JPDFium auto-redact complete (matches={})", result != null ? result.totalMatches() : -1);
                    if (result == null) {
                        throw new IOException("JPDFium auto-redact returned null result");
                    }
                    try {
                        result.save(tempOutput.getFile().toPath());
                        log.info("JPDFium auto-redact: {} matches processed into {}", result.totalMatches(), filename);
                    ...
                    tempOutput.close();
                    log.warn("JPDFium native redaction fell back to manual redaction service: {}", e.getMessage());
                    Map<Integer, List<PDFText>> foundTexts = textRedactionService.findTextToRedact(
                            document, terms.toArray(new String[0]), useRegex, wholeWordSearchBool);
