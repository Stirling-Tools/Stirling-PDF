...
class TextRedactionService {
    ...
    boolean performTextReplacement(
        ...
        List<String> terms = Arrays.stream(listOfText)
                .map(String::trim)
                .filter(s -> !s.isEmpty() && s.length() <= 4096)
                .toList();
        ...
        try {
            ...
            log.debug("JPDFium PdfRedactor.redact complete (matches={})", result != null ? result.totalMatches() : -1);
            if (result == null) {
                log.warn("JPDFium PdfRedactor.redact returned null result, falling back to box-only redaction mode");
                return true;
            }
