...
class TextRedactionService {
    ...
    boolean performTextReplacement(
        ...
        List<String> terms =
                Arrays.stream(listOfText).map(String::trim).filter(s -> !s.isEmpty()).toList();
        ...
        try {
            ...
            try (PDDocument redactedDoc = org.apache.pdfbox.Loader.loadPDF(tempOut)) {
                while (document.getNumberOfPages() > 0) {
                    document.removePage(0);
                }
                for (org.apache.pdfbox.pdmodel.PDPage page : redactedDoc.getPages()) {
