...
public class PdfTextEditorCharcodeController {
    ...
    private static final class BoundedReverseMapCache
        ...
        @Override
        protected boolean removeEldestEntry(java.util.Map.Entry<String, java.util.Map<String, Long>> eldest) {
    ...
    @Operation(summary = "Encode Unicode → font charcodes for the v2 PDF text editor", description = """
    ...
    public ResponseEntity<EncodeCharcodesResponse> encodeCharcodes(@RequestBody EncodeCharcodesRequest request) {
        ...
        try (PDDocument doc = pdfDocumentFactory.load(pdfBytes, true)) {
            ...
            ResourceFont located = findFontByToUnicode(
                    page, request.getLocatorChar(), request.getFontName(), request.getFontSha256(), doc);
            if (located == null) {
                resp.setError("no font on page "
                        + request.getPageIndex()
                        + " renders locatorChar="
                        + request.getLocatorChar()
                        + (request.getFontName() != null ? " (fontName=" + request.getFontName() + ")" : ""));
            ...
            java.util.Map<String, Long> reverseMap = buildReverseUnicodeMap(pdfBytes, located, request.getPageIndex());
            ...
            while (i < text.length()) {
                ...
                try {
                    ...
                    packed = p;
                } catch (IOException | IllegalArgumentException | UnsupportedOperationException encodeEx) {
    ...
    private static ResourceFont findFontByToUnicode(
        ...
        try {
            ...
            if (fontName != null && !fontName.isEmpty()) {
                ResourceFont exact = selectUnambiguous(fonts, wantChar, f -> fontName.equals(f.getName()), "exact");
                if (exact != null) return exact;
                String wantStripped = stripSubsetTag(fontName);
                ResourceFont stripped = selectUnambiguous(
                        fonts, wantChar, f -> wantStripped.equals(stripSubsetTag(f.getName())), "stripped");
                ...
                for (org.apache.pdfbox.cos.COSName xn : res.getXObjectNames()) {
                    try {
                        ...
                        if (xo instanceof org.apache.pdfbox.pdmodel.graphics.form.PDFormXObject form) {
                            ...
                            if (fr != null) {
                                queue.add(new PendingResources(fr, pending.path() + "/" + xn.getName()));
                            }
                        }
