...
public class JwtSaml2AuthenticationRequestRepository
    ...
    // In-memory store for JWT tokens keyed by RelayState UUID
    ...
    public void saveAuthenticationRequest(
        ...
        // Generate a short UUID for RelayState
        // fixme: RelayState is the AuthnRequest ID
        String relayState = authRequest.getRelayState();
        // todo: check relay state in the request, use that if available

        // Store the JWT token in memory keyed by the RelayState UUID
        // fixme: Storing the saml request token is pointless, the response only has the saml
        // response and RelayState. try ARQID instead.
        tokenStore.put(relayState, token);

        // Store the RelayState ID in request attributes for the SAML request resolver
        request.setAttribute(SAML_REQUEST_TOKEN, relayState);
        response.addHeader(SAML_REQUEST_TOKEN, relayState);

        log.debug("Stored SAML request token with RelayState: {}", relayState);
    }
    ...
    public Saml2PostAuthenticationRequest loadAuthenticationRequest(HttpServletRequest request) {
        ...
        try {
            ...
            log.error("Failed to load SAML authentication request from token", e);
            return null;
        }
    }
    ...
    public Saml2PostAuthenticationRequest removeAuthenticationRequest(
        ...
        // Clean up any remaining tokens for this RelayState
        String relayStateId = request.getParameter("RelayState");
        if (relayStateId != null) {
            tokenStore.remove(relayStateId);
            log.debug("Removed SAML request token for RelayState ID: {}", relayStateId);
        }
    ...
    private String extractTokenFromStore(HttpServletRequest request) {
        // When the IdP posts back, the SAMLResponse contains our AuthnRequestID
        String authnRequestId = request.getParameter("RelayState");
        String samlResponse = request.getParameter("SAMLResponse");

        if (authnRequestId != null && !authnRequestId.isEmpty()) {
            // Look up the JWT token using the RelayState UUID
            String token = tokenStore.get(authnRequestId);

            if (token != null) {
                // Clean up the token from memory after use
                tokenStore.remove(authnRequestId);
                log.debug(
                        "Retrieved and removed SAML request token for RelayState ID: {}",
                        authnRequestId);
                return token;
            } else {
                log.warn("No SAML request token found for RelayState ID: {}", authnRequestId);
            }
        }
