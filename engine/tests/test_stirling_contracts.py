...
from stirling.models.tool_models import OperationId, RotatePdfParams
...
def test_edit_plan_response_has_typed_steps() -> None:
    steps = [ToolOperationStep(tool=OperationId.ROTATE_PDF, parameters=RotatePdfParams(angle=90))]
    response = EditPlanResponse(
        summary="Rotate the input PDF by 90 degrees.",
        steps=steps,
    )

    assert response.steps[0].tool == OperationId.ROTATE_PDF
