...
def write_output(out_path: Path, tools: list[ToolSpec], models_code: str) -> None:
    ...
    if pydantic_names:
        header.append(f"from pydantic import {', '.join(sorted(pydantic_names))}")
        header.append("")
    header.append("from stirling.models.base import ApiModel")
    ...
    # Assemble: header + two blank lines + model classes + union + enum + ops
    parts = [
        "\n".join(header),
        "\n\n" + models_code.lstrip("\n"),
        "\n".join(union_lines),
