...
class MyAgent:
    def __init__(self, runtime: AppRuntime) -> None:
        capabilities = []
        if runtime.rag_capability is not None:
            capabilities.append(runtime.rag_capability)
...
That's it. When RAG is enabled, the agent gets a `search_knowledge` tool it can call autonomously.
...
**`sqlite`** (default) — Embedded sqlite-vec. Single `.db` file, zero ops, ~1MB of deps. Ideal for dev and self-hosted deployments.

**`pgvector`** — External PostgreSQL with the `vector` extension. Point `STIRLING_RAG_PGVECTOR_DSN` at your Postgres instance. Requires installing the pgvector extra:
```bash
...
|--------|----------|---------|
| GET | `/api/v1/rag/status` | Check if RAG is enabled |
