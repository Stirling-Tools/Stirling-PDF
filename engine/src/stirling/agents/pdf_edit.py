    ...
    def __init__(self, runtime: AppRuntime) -> None:
        ...
        self.selection_agent = Agent(
            ...
            system_prompt=(
                ...
                "\n\nIMPORTANT RULES:\n"
                "- PREFER returning a plan with sensible defaults over asking for clarification. "
                "Assume clockwise for rotation, all pages unless specified, standard quality, etc.\n"
                "- ONLY ask for clarification when there is genuine critical ambiguity that would "
                "lead to a wrong result (e.g. 'rotate' with no angle at all, 'merge' with no order info "
                "when order matters).\n"
                "- When the user provides enough info to make a reasonable plan, RETURN THE PLAN. "
                "Do not ask follow-up questions about details that have obvious defaults.\n"
                "- If files are loaded, assume ALL files unless the user specifies otherwise.\n"
                "- When returning need_clarification, you MUST populate the suggestions array "
                "with 2-4 short clickable options. Do NOT put the options in the question text. "
                "Example: question='What rotation angle?', suggestions=[{label:'90° clockwise'}, "
                "{label:'180°'}, {label:'270° clockwise'}, {label:'Custom angle', is_other:true}].\n"
